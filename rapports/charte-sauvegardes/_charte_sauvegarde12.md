...
## Principe cardinal — des données fiables et pratiques

Le corpus ne porte que des données **fiables** et **pratiques**, jamais décoratives ni inventées.

- **Rien d'inventé.** On ne fabrique jamais une donnée qu'on ne tient pas d'une source sûre — date, ville d'édition, nom complet, référence. Le vide vaut mieux que l'à-peu-près : un champ laissé vide coûte moins cher qu'une valeur plausible mais fausse (même logique que le **§25.0** pour les liens : une donnée absente coûte moins cher qu'une donnée fausse).
- **Nom complet quand il est établi.** Éditeurs, traducteurs, auteurs : privilégier la forme complète et exacte (« Louis Guérin », « Éditions du Cerf ») dès qu'on la connaît sûrement ; sinon, garder la forme rencontrée sans la deviner.
- **Variantes de titre attestées.** Plusieurs formes d’un titre original peuvent être conservées lorsqu’elles sont réellement attestées (*Adnotationes in Iob* / *Annotationes in Job*, par exemple). Une forme est choisie comme forme préférée dans `titre_original` ; les autres sont documentées comme variantes dans le champ ou la table prévus à cet effet. Tant que ce référentiel n’existe pas, on ne supprime ni ne corrige silencieusement une variante attestée : on la conserve dans la notice ou dans sa note bibliographique en explicitant son statut.
- **Tables de référence, données brutes intactes.** Pour normaliser sans jamais réécrire la source, on tient des tables de curation (`livres_alias` / `resoudre_livre`, `editeurs` / `resoudre_editeur`, et à venir `traducteurs`) : la donnée brute reste dans l'œuvre, la forme canonique est résolue **à l'affichage**. Ces tables se remplissent **au fil du catalogage**, jamais d'un bloc : on ajoute un éditeur ou un traducteur quand on le rencontre, avec ses variantes, et ses dates uniquement sur source sûre.
- **Pratique avant exhaustif.** Une donnée n'a de valeur que si elle sert la lecture ou la recherche ; on ne remplit pas un champ pour le remplir.
...
Le segment est une unité de sens. La segmentation opère sur un texte **déjà propre** (l'océrisation et le nettoyage sont une phase antérieure et distincte) : elle **découpe, elle ne réécrit pas**. Elle épouse le style et la logique propres à chaque auteur et ne dénature jamais le texte.

**Identité et reconstruction.** Chaque segment porte : un `id` ; un `segment_numero` **unique** (ordre global) ; un **`paragraphe`** (n° du § dans la source, **répétable** — jamais affiché, il sert à joindre les segments d'un même paragraphe) ; un **`rang`** (place du segment dans son §, 1…k). L'ordre-segment est `paragraphe.rang` (le § 1 découpé en cinq → 1.1 à 1.5). En **mode livre** (principal), on regroupe par `paragraphe`, on concatène les segments avec **un espace simple**, et l'on sépare les paragraphes comme la source (saut de ligne / alinéa) ; les segments sont stockés **sans espace de bord**. Un **mode segmenté** (lecture) affiche un saut de ligne entre chaque segment. Un changement de page dans la source ne crée PAS de paragraphe : c'est l'alinéa qui fait foi.

**Cible de taille.** Environ **10 à 15 mots** — un petit ensemble qu'on embrasse d'un coup d'œil. Mais **la logique prime** sur la taille, et il n'y a **pas de plafond dur**.

**Où couper — hiérarchie.**
- *Coupes forcées (toujours)* : frontière de **paragraphe** ; frontière de **vers** (saut de ligne) ; ouverture et fermeture d'une **citation balisée**.
- *Coupes logiques* (du marqueur le plus fort au plus faible ; on ne descend d'un cran que si le segment dépasse la cible ET qu'un marqueur plus bas existe) :
  1. Ponctuation **finale** de phrase : `.` `!` `?` `…` — en excluant les faux positifs (abréviations « cf. », « etc. », « S. », « Mgr. », « v. », ordinaux, initiales, décimales).
  2. Ponctuation **forte** interne : `:` `;` `—` `–`.
  3. **Frontière de proposition** : couper **avant** une conjonction ou un relatif (mais, car, or, donc, ni, qui, que, dont, où, parce que, afin que, lorsque, si, bien que…) — le mot ouvre le segment suivant, le lien logique reste visible.
- *Conventions* : la ponctuation finale reste avec le segment qu'elle clôt. Une **incise** forme un segment à part **seulement si elle est longue** ; sinon elle reste rattachée. Une **énumération** reste **un seul segment**. La **virgule n'est pas** un niveau de coupe. **En l'absence de tout marqueur, on ne coupe pas** — sauf frontière de sens **manifeste à la lecture** ; on privilégie alors des segments **syntaxiquement complets** (proposition avec sujet + verbe, ou syntagme autonome).

**Citations.** Toute citation **balisée** (guillemets, italique, retrait), **courte comme longue**, forme un segment de nature **`citation`**, **texte introducteur inclus** — exemple : « L'Apôtre dit (Rm 1, 20) : "Les perfections invisibles de Dieu…" » reste **un seul segment**. Défaut : **une citation balisée = un segment** ; on ne la redécoupe qu'exceptionnellement, à ses marqueurs internes, si elle est très longue (jamais en coupant un mot). Les citations **non balisées** (allusions, paraphrases) suivent la segmentation ordinaire ; leur nature de lien se décide à l'étape de liaison, pas ici.

**Vers et poésie.** Un saut de ligne = une frontière de segment : **un segment ne traverse jamais deux vers** (un vers long peut compter plusieurs segments ; deux vers ne se réunissent jamais). La disposition en vers se reproduit alors naturellement, chaque vers étant son propre paragraphe.

**Enrichissement (mise en forme).** La mise en forme de la source (italique, gras, exposant, petites capitales, latin/grec, liens) est **transcrite fidèlement** dans le balisage existant du site : `*italique*`, `**gras**`, `^^exposant^^`, `++petites capitales++`, `<i>…</i>` (italique éditorial de Sacy), `[texte](lien)`. C'est une **notation** de la forme, non une réécriture du texte.

**Interdits absolus.**
- Couper **un mot**.
- Couper **à l'intérieur d'une balise** d'enrichissement (une coupe ne traverse une portion mise en forme qu'en **refermant puis rouvrant** la balise de part et d'autre).
- Couper une **citation balisée** (hors redécoupe encadrée ci-dessus).
- Couper une **phrase sans marqueur clair**.
- **Transformer le texte nu** : à la segmentation, aucun caractère du texte nu (lettres, mots, ponctuation, espaces internes) n'est **ajouté, retiré ni modifié**.

**Invariant vérifiable (non-transformation).** En dépouillant chaque segment de son balisage (`texteSansEnrichissement`) et en concaténant les segments d'un même paragraphe avec un espace simple, on retrouve **exactement** le texte-source dépouillé de sa mise en forme. À contrôler automatiquement au moment de la segmentation.

**Natures.** `texte` · `titre` (intégré aux niveaux de structure `niv1…`, ce n'est pas un segment à proprement parler) · `apparat_critique` (segmenté selon **les mêmes règles**, avec sa **propre numérotation de paragraphes**, distincte du texte principal, pour éviter tout mélange) · `dialogue` · `separateur` · `texte absent` (lacune matérielle : `segment_texte = « Texte manquant »`, seule insertion tolérée).

**Dialogue.** Nature `dialogue` : un **tour de parole** (objection prêtée à un interlocuteur, réplique de l'auteur), fréquent chez les apologètes. Modèle : **chaque tour = un `paragraphe`** distinct (comme un vers), ses segments portant `nature='dialogue'` ; la frontière de tour épouse la frontière de paragraphe (coupe forcée). En rendu, un paragraphe de dialogue s'affiche sur sa propre ligne, en léger retrait (alinéa pendant), les tours consécutifs formant l'échange. **La nature ne pilote que le saut de ligne et le retrait : elle n'injecte aucun caractère.** Le tiret d'ouverture est celui de la source (normalisé « – ») s'il existe ; on n'en invente pas — l'invariant de non-transformation reste vérifié. Si un interlocuteur cite l'Écriture, la citation balisée reste un segment `citation` à l'intérieur du paragraphe-tour (elle conserve ses liens), les segments alentour restant `dialogue`.

**Après la segmentation — relecture et contrôle.** Relire l'œuvre pour repérer les **problèmes systématiques** (typographiques surtout : guillemets « », espaces insécables, tirets, capitales, apostrophes courbes, siècles et ordinaux…), les **corriger en lot**, **puis** exécuter une **passe de contrôle** de l'œuvre via la page *Contrôle des œuvres* (rangs bon/moyen/critique, natures, apparat) avant de la considérer prête.
...
### 6.1 — `paragraphe` et `ref_niv` : fonctions distinctes

La colonne `paragraphe` assure le **regroupement technique** des segments issus d’un même paragraphe de la source. Elle ne remplace pas automatiquement un niveau éditorial utile. Les colonnes `ref_niv1` à `ref_niv5` conservent les divisions qui servent réellement la lecture, le sommaire ou la navigation : chapitre, question, article, section, numéro de verset commenté, rubrique, etc.

Une même information peut donc apparaître sous deux formes lorsque leurs fonctions diffèrent. Dans une œuvre de type *annotationes*, telle que les *Annotations sur le livre de Job* :
- `ref_niv1` porte le chapitre ;
- `ref_niv2` peut conserver le libellé visible `§ n`, utile à la navigation éditoriale ;
- `paragraphe` porte la valeur numérique `n`, utilisée pour réunir techniquement les segments du même paragraphe ;
- `rang` donne l’ordre des segments à l’intérieur de ce groupe.

Cette coexistence est **intentionnelle** et ne doit pas être signalée comme une duplication fautive. On ne vide un `ref_niv` que s’il ne porte réellement aucune structure ou fonction d’affichage utile dans l’œuvre concernée.
...
### 9.0 — Ordre des passes (à suivre systématiquement)

La constitution des liens d'une œuvre suit TOUJOURS le même ordre, du plus sûr au plus interprétatif, du moins coûteux au plus coûteux. Chaque passe s'applique à toute œuvre : celle qui ne trouve rien — le balisage manque — ne retarde pas la suivante. On ne saute pas une passe et on ne l'inverse pas. Ce qui varie d'un texte à l'autre, c'est le rendement de chaque passe, jamais l'ordre.

**Passe 0 — Diagnostic du balisage.** Repérer les conventions de l'édition : références explicites ? guillemets ? rien ? Elles décident du rendement des passes 1 et 2.

**Passe 1 — Références de l'éditeur.** Extraire les références que l'auteur ou l'éditeur a lui-même écrites (« (Rm 1, 20) », « S. Matth., ch. 5 »). Identifier le livre par la table d'alias (`resoudre_livre`), résoudre le `canon_id`. **Toute référence de l'éditeur DEVIENT un lien** — c'est un marqueur précieux, jamais écarté : la cible (le verset) est attestée. Mais la référence donne la CIBLE, non le TYPE : **chaque situation s'examine** pour établir le rapport de l'auteur au verset. Le type 1 exige une reproduction LITTÉRALE (§9.1) : ne le poser QUE si le segment reproduit le texte visé — citation marquée « … », formule d'annonce (« il est écrit », « dit l'Apôtre »). Sinon l'auteur renvoie au verset sans le citer : il le COMMENTE (type 3) ou en fait un ÉCHO (type 4), ce qui s'établit **par la lecture du segment**, jamais mécaniquement. En attendant cet examen, poser le lien vers la cible en fiabilité « à constituer » + `arbitrage_requis`, JAMAIS en type 1 par défaut. Provenance « editeur ». EXTRAIRE le lien AVANT de nettoyer la référence du corps du texte (voir plus haut). Traiter les intervalles et le renvoi au chapitre seul (→ un lien 3 sur le chapitre, §9.3). Convertir la numérotation quand la source diffère de l'ossature (Vulgate : Psaumes, Siracide).

**Passe 2 — Citations marquées sans référence.** Guillemets ou italiques sans référence : apparier au texte biblique — jamais par simple comparaison de chaînes (§9.1) — en écartant les citations de Pères, de philosophes ou de conciles. Type 1 ou 2 selon la littéralité (§9.1-9.2), fiabilité faible, soumise à validation.

**Passe 3 — Termes consacrés.** Péricopes désignées par leur nom traditionnel (Béatitudes, Décalogue, Notre Père… — liste ci-dessus) → type 3 ou 4, jamais type 1.

**Passe 4 — Lecture : commentaires (type 3) et citations fondues (type 2).** Là où l'auteur explique un verset ou un chapitre déterminé (type 3 : il doit le VISER, §9.3 ; commentaire d'un chapitre → AUSSI un lien 3 sur le chapitre), ou absorbe un verset dans sa propre voix (type 2, §9.2). Curation, jamais extraction mécanique.

**Passe 5 — Lecture : échos thématiques (type 4).** Sur les segments doctrinalement les plus denses, les reprises d'épisode, d'image ou de motif, défendables par un motif précis commun aux deux textes (§9.4). Fiabilité faible en cas de doute.

**Passe 6 — Nettoyage et cohérence.** Une fois les liens extraits : retirer du corps les références devenues redondantes (règles ci-dessus, APRÈS extraction seulement). Aligner l'ensemble plutôt que d'apparier isolément, dédoublonner, rafraîchir les vues.

Les passes 0 à 3 AMORCENT (elles ne récoltent que le balisé) ; ce sont les passes 4 et 5, de lecture, qui CONCLUENT.

**FRONTIÈRE STRICTE MÉCANIQUE / LECTURE (invariant).** Une passe mécanique (outil) n'émet QUE : du type 1 (citation, `probable`), une PROPOSITION de type 2 (`douteux`), ou une cible attestée en `à constituer` (type provisoire = file d'attente de lecture — elle dit QUEL verset, jamais QUEL rapport). Elle n'AFFIRME JAMAIS un type 3 (commentaire) ni un type 4 (écho) : ceux-ci exigent d'avoir LU le passage (l'auteur explique-t-il le verset, ou n'y fait-il qu'écho ?), qu'aucun signal textuel ne donne. Générer un type 3 en dupliquant une citation est un BUG (récidive du 26/07/2026). `scripts/_liens-commun.mjs::verifierLienMecanique` le refuse à la source. Seule exception : la nomination précise d'une péricope (§9.3) peut ouvrir un type 3, mais posé en `à constituer`.

**La passe de LECTURE est GÉNÉRATRICE, pas un classement de l'`à constituer`.** Elle type les cibles attestées (2/3/4), MAIS surtout elle CRÉE des liens que la machine ne pouvait pas voir : les ÉCHOS (type 4) n'ont aucune trace textuelle et n'existent que par la lecture ; les COMMENTAIRES (type 3) là où l'auteur explique un verset sans le citer de façon repérable ; les citations/paraphrases (1/2) que le balisage a manquées. L'`à constituer` est un point de départ, jamais la borne du travail.

**CONTRÔLE PAR SONDAGE — obligatoire à chaque passe.** Toute passe (mécanique ou lecture) se termine par un sondage aléatoire d'une douzaine de liens (`scripts/liens-controle.mjs`) qu'on LIT : la citation du segment vise-t-elle bien ce verset, dans NOTRE ossature (les numérotations diffèrent — Vulgate/hébreu/grec — un lien attesté peut viser le mauvais verset), et le type est-il juste ? Si le taux d'erreur dépasse ~15 %, on ARRÊTE et on diagnostique AVANT de continuer, jamais après avoir écrit des milliers de liens. On contrôle la CIBLE autant que le type.

**RECONTRÔLE DES TYPES — clôture de passe (méthode actée le 27/07/2026).** Quand on repasse les types d'une œuvre déjà liée (chantier de recontrôle), le reclassement s'écrit DIRECTEMENT, sans contrôle utilisateur préalable (« contrôle avant écriture » abandonné, trop coûteux). La passe se clôt en trois temps : (1) reclasser et écrire ; (2) AUTO-ÉVALUER la qualité du reclassement — cohérence au barème des type 2 déjà validés dans la même œuvre, cas limites nommés et laissés en `douteux` ; (3) SCORER la fiabilité (`probable`/`douteux`) de chaque lien reclassé. L'utilisateur contrôle a posteriori. Quand une strate entière a été RELUE (et non échantillonnée), le contrôle est par RAPPEL exhaustif et prime alors le sondage aléatoire.
...
**Frontière lien_2 / lien_4 — ne pas confondre (amendement juillet 2026).** Le partage ne se fait pas au degré de ressemblance, mais à la **reprise verbale**. Dès que le segment reprend une **tournure distinctive du verset** — fût-elle recastée (autre personne, autre temps, fondue dans la phrase, sans guillemets ni formule d'annonce) —, c'est un **lien_2** : le verset est absorbé (§9.2), ce n'est pas un écho. Le **lien_4** est réservé à la reprise d'un **motif, épisode ou image sans aucun mot distinctif du verset**. Erreur constatée et corrigée (juillet 2026, recontrôle Du Symbole / Didachè / Job) : quantité de paraphrases fondues avaient été classées en lien_4 faute d'avoir vu les mots repris — or le lien_2 est en pratique le rapport le PLUS fréquent chez Augustin ; un corpus qui n'en compterait presque aucun est le signe de cette confusion. Exemples : « Il y a deux chemins, l'un de la vie, l'autre de la mort » reprend « le chemin de la vie et le chemin de la mort » (Jr 21, 8) = lien_2 ; « la vie et la mort placées devant l'homme » (Dt 30, 19), motif sans reprise verbale = lien_4.
...
**Ordre des passes — du plus sûr au plus interprétatif.** On ne mélange pas les régimes ; chaque passe date ses liens (`created_at`).
1. **Lecture principale (ancrage).** On lit chaque segment et l'on qualifie le rapport. Pour un commentaire verset-par-verset : le lemme cité → lien type 1 (citation directe seulement), l'annotation qui l'accompagne → lien type 3 (seulement s'il y a un commentaire). Outil réutilisable : `scripts/liens-commentaire-verset.mjs`.
2. **Autres livres cités.** Les citations d'AUTRES livres à l'intérieur des segments, au-delà du lemme principal.
3. **Échos thématiques (type 4).** Curation sur les segments doctrinalement les plus denses.
4. **Résidu et oubli.** Raccrocher les liens `à constituer` connus (versification, surnuméraires) ; repasser sur les segments porteurs de marqueurs de citation SANS aucun lien.
5. **Relecture.** Contrôle humain : `probable`/`douteux` → `vérifié`, arbitrages des couples 1+2.
6. **Consigner ce qu'on a appris.** À la fin de chaque œuvre : verser dans cette charte les enseignements généralisables (pièges de numérotation, améliorations de l'outil) et dans la mémoire de travail ce qui est propre à l'œuvre. Sans quoi la passe suivante repart de zéro.
...
*Amendement du 25 juillet 2026 (mesuré sur Job, Augustin).* Ce « non résolu » couvre aussi les **logia multi-locus** : une citation MANIFESTE mais qui se rattache à un ENSEMBLE de loci sans qu'un seul s'impose — parallèles synoptiques (« Malheur à vous, Scribes et Pharisiens » Mt 23 // Lc 11), refrains psalmiques (« pourquoi es-tu triste, ô mon âme » Ps 41/42), récits redits (conversion de Saul, Ac 9 / 22 / 26). On les traite comme le reste du non-résolu : lien `fiabilite = 'à constituer'`, sans cible, `arbitrage_requis = true`, les loci candidats listés dans le `motif`. **Et cette déclaration se fait EN BASE, jamais seulement dans une note de mémoire de travail.** La mémoire de l'assistant est invisible à l'onglet « À constituer » : un manque consigné hors base EST un manque invisible — précisément ce que cette règle interdit. Sur Job, 26 cas (7 en ch. I–XI, ~14 en ch. XII–XXXIX, 5 échos) avaient d'abord été notés en mémoire et ont dû être reversés en base.
...
- **Éditions de commentaire à numérotation propre.** Un commentaire suivi peut renuméroter, sauter des versets, ou s'arrêter avant les derniers chapitres du canon (Augustin sur Job : l'édition s'arrête à « ch. XXXIX », le matériel canonique Job 40-42 s'y repliant en surnuméraires). Le « § » indique le verset, mais on VÉRIFIE sur le texte ; les versets qui débordent l'ossature restent `à constituer`, jamais forcés sur un créneau voisin.
...
⚠️ **Corollaire de lecture — sur une œuvre-commentaire, lire le segment ENTIER.** Dans un commentaire à lemme, le **premier passage entre guillemets est le lemme commenté** (le verset de Job, du Psaume…), pas la citation que le Père produit ensuite comme preuve : ses citations d'autres livres arrivent DANS la glose (« de là cette parole… », « il est écrit… »). Juger un lien sur le seul début du segment — ou sur l'extrait qu'affiche un outil de sondage — fait prendre de vraies citations (lien_1) pour des erreurs, et l'inverse. Toujours parcourir le segment complet avant de qualifier ou d'invalider un lien.
...
## 20.05 Audit d'une œuvre — protocole « profond + sondage »

Auditer une œuvre (coquilles OCR, typographie, accentuation) ne se limite JAMAIS aux regex de surface : celles-ci ne voient que le typographique visible et **ratent les vraies coquilles** (bon mot au mauvais endroit, lettres avalées formant un vrai mot, mots manquants ou dupliqués). Standard obligatoire, quatre couches :

1. **Batterie typographique** — apostrophes droites → courbes, guillemets « », tiret demi-cadratin « – » (jamais « — »), doubles espaces, capitales non accentuées (À, É, Ê, Ç…), insécables, ligatures.
2. **Intégrité de segmentation** — segments tronqués, anormalement courts, dupliqués, débuts en minuscule inattendus (rappel : le §5 coupe en unités de 10-15 mots, un début en minuscule est normal).
3. **Passe lexicale structurelle** — extraire les tokens, isoler les **hapax**, filtrer les motifs suspects en français (4+ consonnes, `rn`, `q` sans `u`, doublements anormaux, lettres rares k/w/y, triples voyelles) pour débusquer les non-mots OCR. Lever les faux positifs (vrais mots savants, noms propres).
4. **Collation contre la source par sondage** — comparer au mot près l'incipit + quelques passages tirés au sort à l'édition d'origine (URL de `url_source`). La collation *intégrale* segment par segment est réservée aux œuvres critiques (coûteuse en contexte — le signaler).

Signaler spontanément le coût quand la profondeur demandée relit un gros corpus.

---

## 20.1 Notices bibliographiques des œuvres — complétude et vérification

Compléter au fil de l'eau les métadonnées de la table `oeuvres` (`trad_auteur`, `editeur`, `ville`, `date_publication`, `sous_titre`, `titre_original`, `genres`, `date_composition`…), mais **avec des informations sûres uniquement**. Une donnée qu'on ne peut pas établir avec certitude reste **vide** : préférer le vide à l'approximation (cohérent avec le §4). Nettoyer au passage les scories (sauts de ligne parasites dans `titre`, etc.).

**Règle de rattachement (capitale).** Une notice de catalogue (BNF, SUDOC, Wikisource, Google Books…) ne vaut que si elle correspond à la **traduction réellement reproduite** sur le site, et non à une traduction homonyme du même texte. La vérifier par **comparaison d'incipit** ou **recherche de texte exact** d'une phrase distinctive. Exemple : la traduction remacle de Cyprien, *De la vanité des idoles*, a un incipit distinct de la traduction Genoude (*Les Pères de l'Église*, t. 5 bis, Paris, Le Clère & Sapia, 1842) → attribution refusée, traducteur laissé inconnu.

**Source primaire = l'en-tête du texte.** Le traducteur ou l'édition attestés par l'en-tête de la source elle-même priment (ex. « l'abbé Auger » imprimé en tête des homélies de Basile sur remacle).

**Attribution ≠ datation.** Le traducteur est souvent certain ; l'année du tirage l'est rarement quand plusieurs éditions partagent la même traduction (ex. Auger : *Homélies et lettres choisies de S. Basile*, Paris, Crapart, 1788 vs réédition de 1827). La recherche de texte exact **ne départage pas deux tirages du même texte**. En cas de doute sur le tirage, laisser `date_publication` **vide** tout en inscrivant traducteur/éditeur/ville s'ils sont sûrs.

**Noms complets.** Prénom + nom (« Athanase Auger », pas « Abbé Auger »).
...
`catalogue_notices` est le catalogue bibliographique complet des œuvres patristiques et théologiques susceptibles d'entrer sur le site. La table `catalogue_notices` actuelle constitue la référence normative du catalogue. Elle contient 2 576 notices au 24 juillet 2026, dont la grande majorité n'est pas encore importée. Une notice peut regrouper plusieurs volumes d'une même traduction lorsqu'ils forment une série éditoriale continue : avant toute création, rechercher le titre, le traducteur, la collection et la plage d'années, car un volume récent peut déjà être inclus dans une notice agrégée. Elle est distincte de la table `oeuvres` qui ne contient que les œuvres effectivement publiées sur le site.
...
| verifie | boolean | true si la notice a été bibliographiquement traitée selon l'état actuel du catalogue ; ne suffit pas à établir une validation humaine définitive |\n| verifie_admin | boolean | true si la notice a reçu la validation humaine définitive de l'administrateur |
...
2. **États de vérification et verrouillage.** Le schéma actuel distingue `verifie` (notice bibliographiquement traitée dans le catalogue) et `verifie_admin` (validation humaine définitive). Le trigger `trg_protect_verified_notice` repose encore sur `verifie` : cette divergence entre l'état courant des données et le verrouillage technique doit être résolue avant toute modification du trigger. Ne jamais contourner ce trigger.
...
## Couverture de la collection Sources chrétiennes — état du 24 juillet 2026\n\nLa collection est dépouillée de manière exhaustive **dans le périmètre chronologique et confessionnel du catalogue**, au niveau des volumes publiés jusqu’au no 661. L’exhaustivité ne signifie donc pas que tous les numéros de la collection doivent produire une notice : sont volontairement exclus les textes juifs non chrétiens, les auteurs ou œuvres postérieurs au périmètre retenu, les anciennes éditions explicitement remplacées par une nouvelle édition SC et les numéros non encore attribués ou publiés.\n\nÉtat consolidé après contrôle général : **636 notices Sources chrétiennes rattachées à un ou plusieurs volumes précis**, représentant **543 numéros de collection**. Neuf anciennes notices génériques, négatives ou attribuées à tort à la collection ont été reclassées hors du bloc SC. Les **118 numéros non représentés** relèvent des exclusions ci-dessus et ne constituent pas, en l’état de l’audit, des lacunes bibliographiques.\n\nRègles de granularité :\n- une même traduction répartie sur plusieurs tomes reçoit une notice agrégée, avec tous les numéros SC et la plage complète de publication ;\n- un volume contenant plusieurs œuvres distinctes reçoit une notice par œuvre ;\n- une réédition sans traduction nouvelle ne crée pas une traduction distincte ;\n- un volume composite ne doit pas être artificiellement détaillé si le titre exact de ses pièces internes n’est pas établi par une source sûre.\n\nReprises encore ouvertes :\n- **SC 660, Conciles gaulois du Ve siècle** : le volume est couvert par V20-02763, mais ses seize documents issus de quatorze conciles restent à individualiser dès qu’une table des matières exacte et complète sera disponible ;\n- **SC 330** : V20-02811 est la composante verrouillée à consolider avec V20-00935 ;\n- **SC 342** : V20-02803 est la composante verrouillée à consolider avec V20-01904.\n- **Identifiants d’œuvre verrouillés** : six corrections administratives restent nécessaires — V20-00942 → A0132O0002 ; V20-02585 → A0014O0069 ; V20-02588 → A0014O0070 ; V20-02589 → A0014O0071 ; V20-02590 → A0014O0072 ; V20-02808 → A0022O0040.\n\n## Interface admin
...
#### La règle fondatrice : une ligne, un même texte (24 juillet 2026)
...
*Amendement du 25 juillet 2026 — Passe 2 relue entièrement à la main sur Job (Augustin, `A0010O0100`, 734 seg).* Le script ne récolte que le balisé : **5 liens** là où la LECTURE des candidats en a donné **106** (ch. XII–XXXIX ; 122 sur l'œuvre entière). C'est le même écart que sur l'Hexaéméron — le mécanique amorce, il ne conclut pas. Méthode qui a payé, propre aux éditions où **toute citation est entre « … »** (le lemme commenté = 1er guillemet) : ne lire que les segments à **≥ 2 guillemets ouvrants** (`length(txt) - length(replace(txt, '«', '')) >= 2`) ; en-deçà, pas de citation seconde possible. La moitié des chapitres rendent 0 (les 2ᵉˢ guillemets y continuent le lemme). Deux garde-fous constants : **(a) vérifier chaque créneau au CONTENU dans la langue du Père** — pour un Père latin, la Vulgate (TR0004) tranche ce que le français masque : « bienheureux ceux qui pleurent » = Mt 5, **5** et non 5, 4 ; « soif de **vous** » = Ps 62, 2 (« in te ») et non Ps 41, 3 ; Miserere Ps 50, 5/11 et titres de psaumes décalés. **(b) Les logia multi-locus** (parallèles synoptiques, refrains psalmiques, conversion de Saul racontée 3×, « Suis-Moi ») → ne rien poser, `arbitrage_requis`.
...
| A0010O0100 — Augustin, *Annotations sur Job* | 1 (ancrage) + 2 (autres livres) | **1 567** | Reconstruit après la remise à zéro du 24/07 (l’ancien chiffre 1 294 est caduc). Passe 1 = 1 445 (721 type 1 + 724 type 3), ancrage §=verset, 734/734 seg marqués. Passe 2 = **+122** liens « autres livres » (16 ch. I–XI + 106 ch. XII–XXXIX, 25/07), tous lus et vérifiés au contenu (Vulgate TR0004) ; nombreux logia multi-locus laissés en `arbitrage_requis`. Passe 3 (échos type 4) en cours. |
...
### 4 bis. Notes marginales sans appel : rattachement g?om?trique et s?mantique (26 juillet 2026)

Lorsqu'une ?dition porte des manchettes ou r?f?rences en marge sans appel imprim?, le rattachement suit une m?thode en deux temps : la hauteur de la note d?termine seulement une zone candidate dans le corps ; la syntaxe et le sens d?terminent ensuite le mot, la citation ou la proposition exacte. L'appel est ins?r? ? la fin de ce groupe, avant sa ponctuation. La proximit? verticale n'est jamais une preuve suffisante ? elle seule.

Avant tout rattachement, s?parer l'image du corps et celle de la marge ext?rieure, puis reconstituer les blocs marginaux. Ne pas fusionner plusieurs r?f?rences br?ves au seul motif qu'elles sont proches : si elles visent des expressions diff?rentes, elles deviennent des notes distinctes. Inversement, une manchette syntaxiquement continue compos?e sur plusieurs lignes reste une seule note. Plusieurs r?f?rences visant exactement le m?me groupe peuvent partager le m?me point d'ancrage, dans leur ordre mat?riel.

L'OCR m?lant corps et marge ne sert que de rep?re : le fac-simil? tranche toujours. Pour chaque cas ambigu, consigner au minimum la page, la note, les ancrages possibles et le choix retenu ; si le sens ne permet pas de trancher, surligner l'appel en jaune dans le DOCX pour arbitrage humain au lieu d'inventer une pr?cision. Apr?s chaque page, v?rifier qu'aucun bloc marginal n'a ?t? omis, absorb? dans le corps ou appel? deux fois.

---

## Chantier à venir — Textes bilingues latin/français

**Statut : chantier non ouvert.** Cette section fixe les décisions arrêtées ; rien n'est encore implémenté.

Objectif : proposer des œuvres **bilingues latin/français**, lisibles en regard.

- **Une seule `oeuvre`** porte les deux textes, distingués par une **colonne `segments.langue`** (`'fr'` / `'la'`) — **à créer** (défaut `'fr'` pour les lignes existantes, qui sont des traductions françaises). À ne pas confondre avec `oeuvres.langue`, qui désigne la langue **originale de composition** (latin, grec…).
- **Français** (`langue='fr'`) : segmenté selon la §5, **seul lié aux versets**, cherchable.
- **Latin** (`langue='la'`) : **cherchable mais NON segmenté** — une ligne par paragraphe (`segment_texte` = le paragraphe entier, `rang` = 1). La §5 ne s'y applique pas ; la non-transformation et le balisage d'enrichissement, si.
- **Alignement par `(oeuvre, paragraphe)`** : correspondance **implicite** quand les deux textes partagent le paragraphage (cas d'une même édition bilingue) ; **table de correspondance** seulement si les éditions divisent différemment.
- **Affichage** : mode **« en regard »** (réutilise la vue Polyglotte) et mode **« latin seul »**. Le paragraphe latin **hérite** des liens bibliques de son homologue français ; on ne lie donc jamais le latin.

À faire quand le chantier s'ouvrira : ajouter `segments.langue`, le flux d'import bilingue, les deux modes d'affichage.
