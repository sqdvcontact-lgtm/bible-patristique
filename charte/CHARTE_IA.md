...
**Un menu déroulant paraît AUSSITÔT, mais il ne se FIXE qu’au bout d’un COURT MOMENT** (décision de l’auteur, 30 août 2026 : « je ne veux pas une latence quand l’utilisateur pose son curseur sur le bouton de la navbar, je veux que le menu s’ouvre immédiatement ; je veux toutefois une latence dans la fixation du menu déroulant »). Deux gestes se ressemblent au premier instant et n’ont rien de commun : on POSE son curseur sur un onglet pour en voir le menu, ou on le PASSE dessus en allant ailleurs. Rien, à la première image, ne les distingue — d’où une règle en DEUX TEMPS, plutôt qu’un délai avant de paraître. ⛔ Faire attendre reviendrait à faire payer à la main sûre l’hésitation de l’autre : le menu s’ouvre donc SANS DÉLAI dans les deux cas. Mais il ne se FIXE qu’après un court moment de curseur posé sur l’onglet, et tant qu’il n’est pas fixé, il tombe dès que le curseur quitte l’onglet — ⛔ y compris quand celui-ci vient se poser SUR LE MENU. C’est tout le propos : un menu ouvert au passage ne doit pas pouvoir CAPTURER la main qui ne faisait que passer, ni rester en travers de la page qu’elle allait lire. ⛔ Ce seuil de fixation est la DURÉE D’UN PASSAGE, non un temps d’attente, et il se règle sur elle seule : un onglet fait une trentaine de pixels de haut, et la traversée verticale — seul trajet dont la suite entre dans le menu — se fait en quelques centièmes de seconde. ⚠️ Une demi-seconde, essayée d’abord, dépassait le temps qu’une main décidée met à descendre de l’onglet vers la première entrée : le menu tombait sous les doigts de qui venait de l’ouvrir, et il fallait apprendre à s’arrêter pour s’en servir. Deux cents millisecondes couvrent le passage plusieurs fois et restent sous le geste délibéré. Une fois fixé, il se conduit comme n’importe quel menu : le curseur y descend, y circule, et il ne se ferme qu’en quittant l’ensemble formé par l’onglet et son menu. ⚠️ La fermeture est IMMÉDIATE des deux côtés, sans le moindre sursis — elle rattrape parfois un mouvement en diagonale qui visait le menu, mais le prix en est nul puisque l’ouverture ne coûte plus rien : on revient sur l’onglet et le menu reparaît dans l’instant. ⚠️ La règle vaut pour TOUS les menus d’une même barre, sans exception ni raccourci pour l’un d’eux. ⛔ Au clavier, le menu s’ouvre sur `:focus-visible` et non sur `:focus-within`, qui le gardait ouvert après un simple clic de souris sur l’onglet, lequel laisse le lien focalisé, le curseur parti depuis longtemps.
...
**Passe 3 — Titres, casse et références de portée.** Revoir tous les headings après stabilisation de la structure : niveau, casse française, ordinaux, ponctuation, références bibliques finales, titres de section et de péricope, formes redondantes. Appliquer notamment les §§ 35.1, 35.5.1 et 35.5.2. Une référence biblique finale de heading est normalisée et placée entre parenthèses ; les chapitres bibliques sont en chiffres arabes et aucune virgule ne sépare le sigle du livre du chapitre. Exemple : `Tibériade, Jn 21, 1-25` devient dans une formulation continue `Tibériade (Jn 21, 1-25)` ; un heading dont le livre est évident peut finir par `(1, 1-25)`. La forme imprimée reste dans `facsimile_heading`, `source_markup` ou une provenance équivalente. Après **toute** modification textuelle d’un heading, contrôler immédiatement toutes les notes et ancres qui le ciblent : une ancre de titre fondée sur l’ancienne graphie doit être resynchronisée dans la même micro-passe.
