...
**Le volet de lecture : un AXE est un fil, un oui-ou-non est une ligne d’action** (décision de l’auteur, 27 août 2026). Le volet de la page Bible portait cinq cases pleine largeur pour deux décisions — trois langues et l’appareil éditorial —, soit cinq cadres, deux fonds teintés et 181 pixels en tête du volet, avant même la recherche et la liste des livres. Un axe de lecture n’est pas une collection d’objets : c’est une suite d’ÉTATS, et elle se lit comme une ligne. Les états se posent donc en fil, séparés par un blanc, et l’état retenu seul est marqué.

⛔ **Un fil se compose en SANS, et l’état retenu porte un TRAIT.** Le premier jet le donnait en sérif gris, sans autre marque que la teinte de l’actif : trois mots de prose au milieu d’un volet, qui ne s’annonçaient pas comme un réglage, et l’auteur l’a refusé à vue le jour même. Le fil porte donc les deux signaux dont le site se sert déjà pour dire ce qu’on a choisi — le SANS des repères d’interface, et le TRAIT sous l’état retenu, celui des onglets de la bibliothèque et du catalogue des péricopes. ⚠️ Le trait est posé sur les DEUX états, transparent quand le mot n’est pas retenu : sinon le fil sauterait d’un pixel à chaque changement. Le survol d’un mot inactif ESQUISSE ce trait en gris, sous le mot qu’un clic retiendrait ; ⛔ pas de soulignement en plus, deux marques au même endroit se confondraient. Le mot actif, lui, ne bouge pas au survol — c’est là qu’on est, il n’y a rien à y faire.

⛔ **Le trait prend l’ENCRE d’accent, jamais l’APLAT d’accent.** Les onglets du site tirent le leur de `--cs-vert-aplat` ; les deux valent le même vert au Clair, mais sur le Cuir l’aplat vaut un brun de `4a3c2c` posé sur un sol de `1c1813`, et le trait y disparaîtrait — c’est-à-dire précisément la marque qui dit ce qu’on lit. C’est le piège « encre contre aplat » de la § du mode sombre, rencontré ici pour la cinquième fois.

⛔ **Le point médian a été essayé, et retiré le même jour.** Un fil se coupe dès que les libellés sont longs — les trois graphies de la Bible 899 mesurent 187 pixels pour 179 de volet —, et le point restait alors SEUL en bout de ligne, ce que rien ne rattrape. Un blanc se coupe proprement, et le trait lie la série à lui seul. ⚠️ Ces largeurs se mesurent sur une planche qui rend les vrais objets de style hors session, la page Bible tombant en erreur sans compte : le fil de la Fillion tient sur une ligne, celui de la Bible 899 passe à deux, et il fallait le savoir avant de choisir.

⛔ **Un choix binaire ne se rend jamais en deux cases dont l’une est toujours éteinte.** « Avec les commentaires » et « Sans les commentaires » disaient deux fois la même décision. Une seule ligne demeure, et elle dit ce qu’un clic FERA : « Masquer les commentaires », qui devient « Afficher les commentaires » une fois masqués. Elle se passe d’étiquette de rubrique, se nommant elle-même. ⚠️ Elle porte le VERT du site, quand les mots du fil portent l’encre et le gris : dans ce volet, la teinte dit ce qu’on peut FAIRE et l’encre ce qu’on LIT. En italique grisée, elle passait pour une note de bas de page. ⚠️ Le libellé d’ÉTAT ne disparaît pas pour autant : il reste à l’infobulle et au contrôle, où l’on nomme une position, non un geste.
