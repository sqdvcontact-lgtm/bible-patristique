...
| la **mise en forme** d'un texte servi : justification, interligne, gris, blancs | **§ 3.11**, § 6, § 35.17 |
...
| **océriser une bible** | **§ 48**, dont § 48.1, le cycle canonique de travail |
...
| la **bibliographie** : notice, autorités, moteur de rendu | **§ 47**, et § 29, § 42 |
| ce qu'un **contrôle doit refuser** | § 20, § 21, § 22, § 24, § 25 |
| la **chronologie** et la frise | § 26 |
| les **gravures** d’une édition | **§ 49** |
| la **Bible polyglotte** | **§ 50** |
| l’**interface de lecture** et ses volets | § 18, § 36, § 37, § 38, § 51 |
...
| 3 | **Typographie** — signes, espaces, enrichissement et gris du texte |
...
| ~~41~~ | *vacant* — la densité des textes a rejoint le § 3.11 |
...
| 47 | **La notice bibliographique** — composition, autorités, moteur de rendu |
| 48 | **Le protocole d’océrisation d’une bible** — le cycle canonique de travail |
| 49 | **Les gravures d’une édition biblique** — régimes, détourage, part de colonne |
| 50 | **La Bible polyglotte** — réglage, composition, relevés d’ossature |
| 51 | **Les objets d’interface partagés** — cellule d’actions, marques, notification |

⚠️ **Le § 35 et le § 38 ont été cassés le 2026-09-08.** Ils pesaient 38 % de la charte sous des titres qui ne les décrivaient plus. Le protocole d’océrisation d’une bible, les gravures, la notice bibliographique, la Polyglotte et les objets d’interface partagés en sont sortis en chapitres propres, du § 47 au § 51 ; les deux retombent ensemble à 17,6 %.
...
**Ce chapitre est la loi typographique du site**, et rien de ce qu’il prescrit ne se rediscute ailleurs : les espaces et leur qualité, la ponctuation, les guillemets, les tirets, la casse, l’enrichissement, et le gris d’un texte servi — sa justification, sa césure, son interligne, sa chasse.

⛔ **La typographie se pose au RENDU, jamais dans la donnée.** Une correction portée sur le corpus fige une règle qui bougera ; une règle posée au rendu se change en un endroit et vaut aussitôt partout. La donnée porte ce que le témoin porte.

⚠️ **Trois endroits en traitent légitimement, et il faut savoir pourquoi.** Le § 6 pose la STRUCTURE — niveaux, rangs, paragraphes — dont la composition dépend. Les §§ 35.0, 35.7 et 35.10 posent l’ortho-typographie propre à une ÉDITION donnée, qui ne vaut pas hors d’elle. Le § 14.13 tient la césure entre deux unités source, qui est un fait de transcription avant d’être un fait de composition. ⛔ Tout le reste appartient ici.
...
**Double couche obligatoire : forme source et forme d’affichage.** Tout titre, sous-titre, intitulé, rubrique ou tête de niveau transcrit depuis une édition source conserve **exactement** sa forme diplomatique dans la couche source ou de provenance : casse, orthographe, ponctuation et graphies historiques. Cette fidélité ne signifie pas que la couche de lecture doive reproduire mécaniquement les capitales typographiques du témoin. Les champs réellement rendus au lecteur (`ref_niv1` à `ref_niv5`, `ref_nivN_texte`, `heading`, `display_title`, `display_subtitle` ou équivalent) portent une **forme d’affichage éditoriale**, selon les règles françaises de casse ci-dessous. Si un même champ servait jusque-là à la fois de source et d’affichage, la forme diplomatique exacte est d’abord transférée ou recopiée dans un champ de provenance (`facsimile_heading`, `source_title`, `printed_title`, métadonnée `source_*` ou équivalent), puis seulement la forme affichée est normalisée. ⛔ Il est interdit de perdre ou d’écraser la forme source pour améliorer le rendu.

**Contrôle systématique des capitales d’affichage.** À chaque reprise, audit et clôture d’une œuvre, toutes les chaînes visibles de titre, sous-titre, rubrique, chapeau et tête de niveau sont balayées. Toute chaîne entièrement ou majoritairement composée de mots lexicaux en capitales est un candidat obligatoire à normalisation dans la couche d’affichage. Les exceptions doivent être justifiées par leur fonction : sigle ou acronyme (`CSEL`, `PG`, `SC`), chiffre romain, initiale, abréviation réellement conventionnelle, ou effet de casse explicitement voulu par l’éditeur humain. Une simple composition typographique du fac-similé en grandes capitales ou petites capitales n’est **jamais** une justification suffisante pour conserver une longue période en capitales à l’écran. Exemples : forme source `ABREGÉ DE LA VIE DE S. EVCHER` → affichage `Abrégé de la vie de S. Eucher` ; `TIRÉ DV MARTYROLOGE D’ADON.` → `Tiré du Martyrologe d’Adon.` ; `AVERTISSEMENT.` → `Avertissement.` La clôture exige **0 séquence de capitales d’affichage non justifiée** sur les surfaces du périmètre. Le contrôle porte sur la surface réellement rendue, pas seulement sur `segment_texte`.

Les règles françaises de casse exposées ci-dessous s’appliquent donc aux formes d’affichage éditoriales, qu’elles dérivent ou non d’un intitulé source. La forme diplomatique du témoin reste intangible dans sa couche de provenance. Cette séparation source/affichage a priorité sur toute sous-charte de chantier.
...
#### Continuité matérielle d’une séquence poétique

⛔ **Une frontière technique ne crée jamais une strophe.** Pour toute séquence continue de segments portant `segment_metadata.forme = 'vers'`, la lineation est contrôlée contre le témoin. Entre deux vers consécutifs du même poème, `join_before` doit porter le saut de ligne attesté (`\n` dans le cas ordinaire). `NULL`, `space` ou `\n\n` au milieu d’une séquence ne sont admis que si une rupture réelle — strophe, changement de pièce ou autre articulation matérielle — est prouvée par la source. Un changement de page, d’unité source, de bloc OCR ou de paragraphe technique ne suffit jamais. Le contrôle inventorie aussi les paragraphes mêlant prose et vers : ils sont arbitrés pour que le rendu ne fasse pas tomber un poème en prose, sans scinder conjecturalement un segment dont les offsets ou appels de note exigent le maintien.
...
ce qui est le reproche déjà consigné au § 47.2.
...
⚠️ **Tout CSV reste encodé en UTF-8.** Les retours de ligne internes, guillemets et séparateurs sont échappés selon la norme CSV. ⛔ **Un import ne doit jamais perdre silencieusement une colonne inconnue : il la refuse ou la signale avant écriture.**
...
⛔ **ALIGNEMENT N’EST PAS PARAGRAPHAGE** (décision de l’auteur, 7 septembre 2026). Le groupe dit ce qui se répond d’une colonne à l’autre ; il ne dit RIEN de la découpe du texte, qui appartient à l’édition seule et se lit dans `paragraphe`. **Le PARAGRAPHE compose, le GROUPE met en regard**, et il faut les deux. La composition — filet, blanc, alinéa — appartient au paragraphe de l’édition, découpé sur la clé éditoriale entière (`id_texte`, `espace_textuel`, `ref_niv*`, `paragraphe`), ses segments rangés par `rang` et joints par `join_before`. La mise en regard appartient au groupe : lui seul tient les deux colonnes en face l’une de l’autre, et il lui faut pour cela son propre rang de grille. Chaque groupe compose son original dans le premier rang qu’il touche. Le champ `paragraphe` reste propre à chaque texte : un groupe peut traverser les divisions de la langue originale lorsque celles-ci ne coïncident pas avec le paragraphage de la traduction, mais il ne traverse jamais une frontière de paragraphe du texte traduit.
...
⛔ **Entre deux ensembles posés sur la même paire de textes, c’est le plus FIN qui porte la lecture, et la finesse se COMPTE** : une ligne de `texte_alignements` vaut un groupe, et le plus grand nombre l’emporte. `alignment_level` est l’étiquette de l’éditeur, non une mesure, et ne départage qu’à finesse inconnue ou égale. **Une division source n’impose jamais un groupe unique.** Si une section, un chapitre ou toute autre division de l’original traverse une frontière absolue du texte traduit, on scinde le **groupe d’alignement**, non le texte ni l’unité source ; l’identité de la division source demeure dans les métadonnées. Le nombre de groupes peut donc dépasser le nombre de divisions source et les deux compteurs doivent rester distincts. Après arbitrage, un ensemble concurrent conservé uniquement comme trace historique passe en `retired` et est exclu du choix et des compteurs actifs. Le choix ne s’ouvre qu’entre le texte lu et un texte en langue originale : un alignement entre deux traductions françaises n’a rien à mettre dans une colonne de latin.
...
⛔ **L’appel se place toujours AVANT la ponctuation qui clôt le passage annoté.** Cette règle vaut dans la donnée comme au rendu : `mot[[3]].`, `mot[[3]] ;`, `mot[[3]] !`, jamais `mot.[[3]]`, `mot ;[[3]]` ni `mot ![[3]]`. Lorsqu’un guillemet fermant suit le passage annoté, l’appel reste à l’intérieur du guillemet et précède aussi la ponctuation extérieure : `les sarments[[3]] » ;`. Elle vaut dans la prose, les titres, les citations sorties, les vers et les prières. L’offset d’ancre pointe donc immédiatement après le dernier caractère du passage annoté et avant tout signe de clôture.
...
⛔ **L’appel ne prend pas l’alinéa du paragraphe qui le porte.** C’est l’autre revers de l’`inline-block` : le retrait de première ligne s’hérite, et un `inline-block` est un conteneur de BLOC — il applique donc à sa propre ligne, la seule qu’il ait, le retrait destiné au paragraphe qui l’accueille. Sur une ligne de vers, dont le retrait de suite est négatif, le chiffre sortait de sa boîte par la GAUCHE et venait s’imprimer dans le mot qui le porte, tandis que la boîte, vidée de sa chasse, se refermait sur son seul rembourrage et laissait le texte suivant se recoller par-dessus. Relevé le 7 septembre 2026 sur le *Manuel* de Dhuoda : les 83 appels de la division des Prolégomènes étaient tous illisibles. Le retrait se remet donc à zéro dans la FORME de l’appel, et non dans le bloc de vers : le retrait suspendu des bibliographies et l’alinéa positif des essais l’atteignaient de la même façon.
...
⛔ **2. Les œuvres CITÉES en note entrent dans `ouvrages_bibliographiques`**, le catalogue des ouvrages, et la référence se COMPOSE depuis ses champs — jamais rédigée à la main dans un bloc. C’est le modèle du § 47.1, étendu aux notes. **Deux gardes** : un auteur ancien n’y reçoit jamais de fiche notée (§ 29), sans quoi inscrire l’Énéide dégraderait la valeur scientifique de la notice qui la cite ; et les champs d’édition (lieu, éditeur, année, ISBN) restent vides tant qu’on ne cite que l’œuvre.
...
⚠️ **Ce qui identifie la note ne disparaît pas pour autant : le NUMÉRO passe dans une gouttière**, dans la face du numéro de verset de la page Bible. Il est là quand l’intitulé n’y est pas, et il coûte une gouttière au lieu d’une ligne. ⛔ Le numéro affiché est celui du LECTEUR, jamais le numéro interne : celui-ci porte l’identité et l’ordre, non l’adresse que le lecteur vient de cliquer.

⛔ **MAIS IL SE FERRE À GAUCHE, et le fer à droite est ici une règle mal transposée** (décision de l’auteur, 2026-09-08 : « supprime l’alinéa avant le numéro de note »). Le site range à droite tout chiffre qui accompagne un TEXTE SUIVI — un numéro de verset dans une colonne de lecture —, et il a raison : cinquante repères s’y alignent les uns sous les autres, et c’est l’alignement qui les rend lisibles. Il n’y en a qu’UN dans un encart, en tête d’un objet : ferré à droite d’une gouttière fixe, un numéro d’un ou deux signes s’écartait du bord, et la note s’ouvrait sur un alinéa que rien ne justifiait. ⚠️ Règle générale : **un fer à droite ne se justifie que par une COLONNE de repères ; seul, il fait un alinéa.**

⛔ **UNE NOTE SE JUSTIFIE ET SE CÉSURE, et cela se pose sur le CORPS de l’encart, jamais sur ses paragraphes** (même décision : « j’aimerais qu’elles soient justifiées »). La page Bible justifiait les siens un par un, la lecture d’une œuvre pas du tout — le même encart rendait deux compositions selon la page qui l’ouvrait, ce que ce module existe pour empêcher. ⚠️ La césure n’est pas une option qui accompagne la justification, c’est sa condition : la piste fait une soixantaine de signes, et une justification sans coupure y creuse des lézardes. ⚠️ Et la DERNIÈRE ligne revient au fer à gauche, faute de quoi trois mots s’étirent d’un bord à l’autre — les déclarations redites sur le paragraphe écrasaient précisément cette règle-là.

⚠️ **L’APPARAT CRITIQUE SUIT LE MÊME MODÈLE, UN CRAN SOUS LA NOTE** (même décision). Sa composition vit avec celle de la note et non dans son composant, et **son corps se dit en `em` DE LA NOTE, jamais en rem** : il en descend, il ne s’en détache pas, et il suit tout resserrement futur sans qu’on y pense — c’est ce qui l’a fait maigrir avec elle le jour même. Le rapport EST le sens de ces valeurs : l’apparat se parcourt, il ne se lit pas comme de la prose. ⛔ **Sa seule dissidence est la CÉSURE, qu’il refuse** : couper un sigle ou une leçon donnerait à lire ce que l’éditeur n’a pas écrit. La justification, elle, lui va — c’est la forme d’un apparat au pied d’une page —, et le corpus le confirme : 87,5 % des 8 486 entrées tiennent sur une ligne, aucune ne porte deux blancs de suite, la justification n’a donc aucun blanc d’éditeur à étirer.

⛔ **CE QUI EST UNE MESURE SE REMESURE.** Trois constantes de ce module ne sont pas des réglages mais des relevés — les signes par ligne, et les hauteurs que trois notes réelles demandent —, et elles MENTENT dès que le corps ou le blanc bougent : une demande périmée ne rend pas la garde fausse, elle la rend MOLLE, et elle passerait sur une boîte deux fois trop haute sans rien dire. ⚠️ Corollaire, appris le même jour : une garde qui RECOPIE le corps et l’interligne au lieu de les lire au module échoue au premier changement, et c’est le défaut même que le module réunit pour empêcher.
...
⛔ **L’ENCART S’ARRÊTE AU VOLET : la marge se compte jusqu’au bord du BLOC DE LECTURE, non jusqu’au bord de la fenêtre.** Le bloc de lecture est ce qui reste entre les deux volets. Un volet est bien une navigation, mais une fenêtre posée dessus le RECOUVRE, et le lecteur qui l’a ouvert l’a ouvert pour le voir.
...
⚠️ **Un nombre élevé de lignes déclaré relu dans un temps matériellement invraisemblable constitue un signal d’alerte, non une preuve de qualité.** ⛔ **Les mentions telles que `direct_visual_review` ne sont inscrites que lorsqu’une comparaison visuelle a réellement eu lieu.**
...
✅ **TR0013 — fragments cibles intra-verset.** La source structurée peut désormais être projetée jusque dans la traduction critique sans dupliquer le texte. `internal.bible_translation_material_fragments` ne stocke que des empans Unicode dans `versets_v2.texte` : `fragment_start_offset` est inclusif, `fragment_end_offset` exclusif, tous deux à base zéro ; chaque empan est rattaché à un `verset_v2_id`, au segment source et à son `source_run_index`, avec un `join_before` de rendu. ⛔ Ne jamais copier le texte du fragment dans cette table et ne jamais utiliser les mappings AELF pour représenter cet ordre matériel : ce sont deux axes différents. `internal.v_bible899_tr0013_fragment_integrity` n’autorise la consommation d’un jeu de fragments que s’il couvre tous les runs attendus, respecte les bornes et recompose **exactement** `versets_v2.texte`. `internal.v_bible899_tr0013_material_runs` combine ensuite les runs du témoin et les fragments cibles dans un flux ordonné par `material_stream_order`. Si les fragments sont absents, incomplets ou invalides, le repli est obligatoire : le texte cible entier reste rendu une seule fois sur le premier run et les continuations ne le répètent pas ; une nouvelle recoupe source ne doit donc jamais rendre la traduction vide. Les trois cas pilotes validés sont `GEN.25.21 / GEN.EXTRA.GLOSS.25.21`, `MAT.1.25 / MAT.1.EXTRA.25A` et `ACT.4.17 / ACT.4.EXTRA.17A`. Pour les deux gloses comme pour la dittographie d’Ac 4,17, les deux fragments canoniques concaténés reconstruisent exactement le verset TR0013 et le surnuméraire validé s’intercale entre eux dans le flux matériel. ⛔ Aucun fragment cible n’est créé automatiquement à partir d’une nouvelle coupure source : il faut une revue structurelle explicite. Toutes ces structures restent dans `internal` ; aucun `GRANT` `anon` ou `authenticated` ne doit être ajouté pour les exposer.
...
`SANS_OBJET` s’applique lorsqu’aucune traduction française autonome n’est décrite. Toute décision est justifiée dans `statut_juridique_note`; ⚠️ **le champ historique `domaine_public` peut être conservé pour mémoire, mais il n’est plus normatif**.
...
Le verrou du `proxy` — redirection 307 vers `/chantier`, 403 aux robots d'IA déclarés — s'applique aux ROUTES DU SITE. L'API de données de Supabase répond sur un autre domaine (`<ref>.supabase.co/rest/v1`) et ne le traverse jamais. Or la clé publiable est servie, par construction, dans un bundle JavaScript de `/chantier` — la seule page ouverte sans session : qui la lit interroge la base directement, sans compte.

⛔ **Tant que le site est fermé, le rôle `anon` n'a AUCUN droit dans le schéma `public`** : ni `select` sur une table, une vue ou une vue matérialisée, ni `execute` sur une fonction. Les privilèges par défaut sont eux aussi révoqués pour `anon`, sans quoi la prochaine table créée rouvre la porte toute seule et en silence. La réouverture sera une migration à part, le jour du lancement, et elle seule.

⚠️ **Une politique RLS n'est pas un droit.** Elle borne les lignes qu'un rôle voit UNE FOIS qu'il a le droit de lire la table. Sans `grant`, une politique `{anon}` ne sert à rien ; avec le `grant`, elle sert pleinement. Lire les politiques seules induit en erreur dans les deux sens : `has_table_privilege` fait foi, et un `curl` avec la clé publiable tranche.

⛔ **Une vue matérialisée ignore la RLS.** Aucune politique ne s'y applique : son seul verrou possible est le `grant`. Toute vue matérialisée de `public` se tient donc hors de portée d'`anon` par révocation explicite, et sa raison d'être se vérifie. Trois d'entre elles servaient 35 480 versets en cinq traductions à qui n'avait pas de compte, sans qu'aucune ligne d'`app/` ne les emploie.
...
⛔ **Ces tables se purgent à quinze jours, et c'est la BASE qui le fait.** `internal.purger_sauvegardes(p_jours, p_max, p_essai)` ne touche qu'`internal`, qu'aux noms préfixés `backup_` ou `stage_`, et qu'à ceux qui portent une date valide — une table sans date dans le nom n'est jamais supprimée, puisqu'on ignore de quand elle date. Le travail périodique `purger_sauvegardes_internes` l'appelle chaque nuit à 3 h 25, comme `purger_sauvegardes` le fait pour `parametres`.

⚠️ **Une règle de rétention qu'il faut penser à lancer n'est pas une règle, c'est une corvée** — et une corvée s'oublie. Faute d'elle, `internal` avait atteint 10 735 tables pour 4 465 Mo, plus de la moitié d'une base de 8 094 Mo, c'est-à-dire son plafond ; et le vidage nocturne emportait tout cela chaque nuit dans son artefact. Tour à blanc par défaut ; `p_essai => false` pour supprimer. Les tables retirées restent dans les vidages conservés quatre-vingt-dix jours.
...
⛔ **Le fond d’un ENCART n’est pas un fond NU, et le jeton de l’un ne sert pas l’autre.** Un jeton `-fond` — `--cs-danger-fond` et ses pareils — est calibré pour vivre SOUS une bordure et un libellé : la teinte y accompagne ce que la forme dit déjà, et il lui suffit d’être un souffle. Posé seul, sans cadre, sans pictogramme, sans encre particulière, ce même souffle ne se distingue plus du fond de la page, et le signal cesse d’exister. C’est la loi du § 50.1 prise par l’autre bout : ce qui reste SEUL à porter un fait se hausse d’un rang, que ce soit l’encre quand on retire le fond, ou le fond quand il n’a rien pour le doubler. ⚠️ **Un fond qui porte seul le sens prend donc un jeton à lui**, dans la même famille mais à sa propre dose — ainsi `--cs-absence-fond`, la colonne d’une bible qui ne porte pas le mot cherché dans les résultats de recherche. ⛔ **Et la dose se RELIT dans chaque thème au lieu de se recopier** : un tiers d’aplat qui fonce un crème en terre cuite ne fait qu’un brun de plus sur un sol sombre, où il en faut près de la moitié pour obtenir une brique.

**Une seule apparence pour tout ce qui attend** (décision de l’auteur, 6 septembre 2026 : « uniformiser l’apparence des chargements sur le modèle de la page Bible classique »). Le site disait « Chargement… » de vingt-cinq façons : cinq corps, deux encres, trois polices, des points de suspension tantôt composés tantôt tapés, et le mot venait tantôt d’un coup, tantôt jamais. Il n’y a plus que trois pièces, toutes dans `app/lib/attenteEnCreux.tsx`, et la page Bible classique en est le modèle. Le MOT, là où un bloc vide attend son premier contenu : corps de 0,8125 rem, encre faible, italique, la police héritée du bloc ; il ne paraît qu’au bout de quatre cent cinquante millisecondes, en fondu, pour qu’une arrivée rapide ne montre rien. L’ANNEAU, là où un contenu déjà là se remplace : deux pixels de filet, le bord du site pour la piste, le vert pour la part qui tourne, sept dixièmes de seconde par tour ; lui non plus ne s’allume qu’au bout d’un instant. L’ÉCRAN, pour une page entière : le fond du site sous la barre et le mot au centre ; une page qui a des volets les dessine en creux, aux largeurs de leur premier rendu, vides. ⛔ Un « Chargement… » écrit ailleurs qu’avec ces pièces rouvre la dérive. ⚠️ Les mentions de cellule de la Polyglotte (« Chargement… » à la place d’un verset qui arrive) ne sont pas une attente mais la voix de l’éditeur, au même titre qu’« Absent de cette traduction » : elles gardent leur forme.
...
- **RÈGLE — candidats de glose : distinguer fragmentation AELF et recoupe éditoriale.** Les diagnostics `EMBEDDED_EXTRA_CANDIDATE` déjà relus et confirmés constituent la file primaire pour les gloses potentielles. Une matérialisation dans `internal.bible_canonical_alignment_spine_mappings` — fragments `verified`, notamment `source_only` — décrit la relation du segment courant à l’axe AELF ; elle **ne signifie pas** que le segment éditorial a déjà été recoupé. Elle fournit au contraire des bornes contrôlées utilisables comme preuve pour une éventuelle recoupe. Seule la présence d’un véritable `bible_editorial_segments` aligné `MANUSCRIPT_EXTRA` signifie que la matière hors canon a été séparée éditorialement ; ce cas ne doit jamais être recréé en doublon. Avant toute recoupe, contrôler donc séparément : (1) la décision philologique courante ; (2) les fragments spine et leur partition exacte ; (3) l’existence éventuelle d’un `MANUSCRIPT_EXTRA`; (4) la possibilité d’isoler la matière sans couper une balise d’incertitude ni déplacer de matière canonique d’un autre verset. Les formules telles que `ce est a dire` ne sont que des signaux de rappel et ne déclenchent jamais une séparation automatique. Le statut hors canon reste `MANUSCRIPT_EXTRA` / `manuscript_extra=true`; `metadata.phenomenon='gloss'` n’est attribué qu’après preuve philologique, avec `canonical_context` lorsqu’il est déterminable. Toute recoupe éditoriale passe sous Control V2 `bible_editorial_segment`, conserve exactement la couverture source et l’ordre matériel, crée ou ajuste la ligne surnuméraire TR0013, puis contrôle les `ordre_slot`, les fragments cibles intra-verset lorsqu’ils sont nécessaires, l’apparat et les liens.
...
- **À CLAUDE — raccordement lecteur TR0013 intra-verset.** Le backend de données est prêt ; le travail restant est un raccordement de lecteur, pas une nouvelle resegmentation. Le backend privilégié doit consommer `internal.v_bible899_tr0013_material_runs`, trier par `material_stream_order` et respecter `target_mapping_status` / `target_join_before`. `validated_fragment` se rend comme le fragment cible ; `whole_extra` comme le surnuméraire ; `whole_target` et les statuts de repli `whole_target_fragment_mapping_*` rendent le verset entier une seule fois ; les statuts `continuation_suppressed_*` ne répètent aucun texte cible. ⛔ Le client web ne doit jamais requêter `internal` directement ni obtenir `USAGE` sur ce schéma : créer ou employer une couche backend privilégiée qui ne restitue que les champs nécessaires. Ajouter trois régressions obligatoires : Mt 1,25 et Gn 25,21 doivent rendre `canon 1 → glose → canon 2` ; Ac 4,17 doit rendre `canon 1 → dittographie → canon 2`. Dans les trois cas, la concaténation des fragments canoniques doit reconstruire exactement `versets_v2.texte`, sans duplication ni omission, et le surnuméraire doit rester à sa position matérielle. Pour l’apparat, lorsqu’une ancre `validated` vise un surnuméraire, utiliser `internal.v_bible_verse_note_anchors_resolved` côté backend afin de placer la note sur le segment ; `v_bible_verse_notes` public n’est pas chargé d’exposer ces ancres structurelles.
...
**Principe de rédaction d’une note éditoriale publique.** Une note éditoriale s’adresse au lecteur de l’œuvre. Elle lui fait comprendre, en peu de phrases, une particularité intellectuelle ou documentaire que les champs structurés et la lecture du texte ne suffisent pas à expliquer : attribution discutée, constitution composite, transmission lacunaire, remaniement ou disposition inhabituelle. ⚠️ **Elle expose concrètement la cause et la portée de la difficulté** — qui a constitué le texte, quelle part revient à l’auteur, quelle forme ne vient probablement pas de lui, ou ce que la lacune change pour la lecture. ⛔ **Elle ne se borne jamais à une étiquette abstraite telle que « compilation incertaine ».**
...
6. **Stabiliser la segmentation et la provenance.** Après toute correction textuelle ou structurelle, vérifier les unités source, la recomposition, les paragraphes, rangs, `segment_numero`, `segment_key`, `join_before`, offsets, limites de source, pagination et empreintes. Une segmentation ne sacrifie ni l’unité syntaxique ni la reconstruction exacte du témoin. ⛔ **Les contrôles du § 20 sont des conditions nécessaires, jamais une preuve suffisante d’exactitude.**
   ...
   **Contrôle obligatoire de poésie et de lineation (§ 7.4).** Inventorier tous les segments `forme = vers`, les regrouper en séquences continues par pièce, confronter leurs lignes au témoin et contrôler les jointures internes. Une frontière de page, d’unité source ou d’OCR ne doit produire ni `join_before` nul, ni espace, ni blanc de paragraphe entre deux vers d’une même séquence. Toute rupture plus forte doit être attestée comme strophe ou changement réel de pièce. Relever et arbitrer aussi les blocs mêlant prose et vers.

7. **Appliquer la couche éditoriale.** Une fois texte et structure stabilisés, traiter la typographie, la ponctuation des citations, les guillemets, langues et écritures, italiques, références intégrées, lemmes et enrichissements selon le § 3 et les conventions propres au témoin. **Passage obligatoire sur les titres :** inventorier toutes les formes visibles (`ref_niv*`, `ref_niv*_texte`, headings, titres/sous-titres d’unités, rubriques, chapeaux et projections), distinguer pour chacune la forme source de la forme d’affichage, puis rechercher les chaînes entièrement ou majoritairement en capitales lexicales. Toute forme non justifiée est normalisée dans la seule couche d’affichage ; la forme diplomatique reste conservée dans la provenance. Le postcontrôle de cette étape doit établir **0 séquence de capitales d’affichage non justifiée**. Contrôler toutes les projections et surfaces rendues : ⛔ **une correction incomplète dans une projection secondaire reste une correction inachevée. Le rendu ne doit pas masquer une donnée fautive.**

8. **Reprendre notes, apparats et références bibliographiques.** Contrôler l’appartenance documentaire des apparats, puis les notes comme un système complet : identité, numérotation, blocs, ancres, cible réelle, offsets, portée, ponctuation, enrichissements, références et éventuelles ancres de titre (§§ 8, 13, 22). Dans la même étape, inventorier **toute référence bibliographique rencontrée sur les surfaces rendues**, y compris hors d’une pièce intitulée « Bibliographie ». Chaque référence identifiable passe obligatoirement par l’outil **Normaliser** : recherche de doublon par clé normalisée, réemploi ou création de `ouvrages_bibliographiques`, complétion documentée des champs disponibles, rattachement à `ouvrage_id`, autorités de contributeurs et d’éditeurs, `type_ouvrage` et état de revue si nécessaire (§§ 29.0, 47.1, 47.4). Aucune note, aucun appel ni aucun apparat n’est conservé sous une forme technique redondante lorsque le modèle normatif le représente déjà. ⛔ Cette étape n’est close que si 0 référence identifiable est rendue depuis une chaîne libre lorsqu’une représentation structurée existe, 0 ouvrage identifiable reste sans recherche de correspondance, 0 doublon de notice ou d’autorité a été créé, et toute projection matérialisée est traçable à son `ouvrage_id` ou, pour un renvoi, à ses `related_ouvrage_ids`.

9. **Contrôler les textes parallèles et alignements, s’ils existent.** Toute resegmentation ou modification de frontières oblige à revalider les alignements concernés et leurs projections. ⛔ **L’alignement est sémantique et ne force jamais du `1:1`** (§ 12). Si une division source traverse une frontière absolue du texte traduit, scinder le groupe d’alignement sans scinder artificiellement le texte ni l’unité source ; vérifier alors séparément le nombre de divisions source et le nombre de groupes. Après bascule vers l’ensemble canonique, tout ensemble concurrent conservé seulement pour mémoire est passé en `retired`, et les compteurs de clôture distinguent ensembles actifs, ensembles retirés, groupes actifs et groupes historiques. ⛔ **Les caches ou projections dérivés sont régénérés après la donnée normative, jamais l’inverse.**
...
Le rapport indique ce qui a été modifié, contrôlé et laissé en attente. Il donne les chemins des sauvegardes et artefacts utiles. ⚠️ **Les bilans propres à une œuvre restent dans `audit/` ou dans les scripts de chantier, jamais dans la charte.**
...
⚠️ **Les scripts historiques peuvent contenir des hypothèses périmées.** Avant réemploi, vérifier leurs colonnes, valeurs, filtres et garde-fous contre la présente charte et le schéma courant.
...
`Christianisme commun` et `Œcuménisme` sont des axes transversaux. `Œcuménisme` n’est attribué qu’à un dialogue, une convergence, une union, une institution commune ou une réception pluriconfessionnelle explicite. Une coexistence, une guerre ou une controverse entre confessions ne suffit pas. De même, ⚠️ **le mot générique `réforme` ne suffit jamais à rattacher un événement à la `Tradition réformée`.**
...
`portee_ecclesiale` décrit l’étendue ecclésiale selon les valeurs : `universelle`, `pluriconfessionnelle`, `propre à une tradition`, `régionale`, `locale`. ⚠️ **Cette portée ne remplace ni `portee`, ni l’importance, ni le niveau de lecture.**
...
Les titres de série sont intégrés à la recherche. ⚠️ **Une série ne doit pas devenir un fourre-tout thématique** : ses membres doivent former une chaîne identifiable.
...
Le score de recherche mesure la proximité avec la requête ; ⚠️ **il ne remplace ni l’importance historique ni le niveau de lecture.**
...
Le titre reste sombre et neutre. La couleur apparaît dans le point, le filet, la date ou le libellé ; ⚠️ **on ne colore pas toute la ligne.**
...
Les vues internes d’audit doivent rester vides d’anomalies avant la clôture d’une passe. ⚠️ **Les comptages de chantier et états provisoires appartiennent aux rapports et sauvegardes, non à la charte normative.**
...
### 27.2 Autorité et journalisation de la charte
...
Réserve (auteurs) : un auteur en réserve voit ses références écartées, afin de protéger un public fragile d'une mise en avant susceptible de heurter, par exemple celle de bourreaux. ⚠️ **La réserve ne juge pas la personne et ne préjuge pas de sa valeur académique.**
...
La règle francophone du présent système reste applicable : un ouvrage non admissible comme `citation_francophone` peut demeurer dans la documentation interne, mais n’est pas marqué `retenu_notice`. ⚠️ **Son absence de la sélection publique ne diminue pas sa valeur scientifique.**
...
L'avancement de l'océrisation se mesure sur le lot effectivement traité, distinct du document entier. Un lot est terminé lorsque toutes ses pages sont océrisées, en erreur ou volontairement exclues ; ⚠️ **les pages du document non incluses dans le lot ne sont pas comptées comme manquantes**. L'état du document et l'état du lot sont présentés séparément.
...
La recherche de formes proches, par exemple au moyen d’un algorithme de distance d’édition ou de type SymSpell, ⛔ **fournit des candidats et non des corrections.** ⚠️ **Le score, ses composantes et les candidats proposés doivent rester consultables afin qu’une décision puisse être auditée.**
...
⚠️ **L’objectif n’est pas de minimiser artificiellement le nombre de propositions, mais de concentrer la vérification humaine sans augmenter les erreurs résiduelles.** Pour chaque évolution, mesurer au minimum :
...
⚠️ **Une baisse du nombre de propositions n’est un progrès que si les sondages ne montrent pas une hausse des erreurs manquées.** Les seuils sont ajustés sur des corpus représentatifs et restent configurables par période, langue, moteur et qualité matérielle de la source.
...
### 35.19. Décisions de chantier et abréviations de renvoi (25 août 2026)
...
### 35.20. Désabréviation de *hoc loco*
...
### 35.21. Récupération d’un bloc manquant et conservation des titres source (25 août 2026)
...
### 35.22. Sections, translittérations et renvois internes (2026-08-25)
...
### 35.23. Séparation des blocs composites chapitre / péricope / commentaire
...
### 35.24. Ponctuation des lemmes et reclassification des faux appels de note
...
### 35.25. Références bibliques impossibles ou corrompues
...
### 35.26. Contrôle de clôture après resegmentation ou projection
...
⚠️ **UNE PROSE NE SE COMPOSE PAS COMME UNE ÉTIQUETTE.** « Particularités » porte quatre phrases dans une rangée dessinée pour un mot : la valeur y prend l’interligne et la césure d’un paragraphe, non celui d’une étiquette de 0,5 rem. ⛔ Sans justification : la colonne fait environ 314 px, soit quarante-cinq signes par ligne, et le justifié y creuse les blancs que le § 50.2 apprend à fermer.
...
⚠️ **Une première rédaction de cette note affirmait que la date de fin était “le plus souvent absente”.** Elle ne l’est jamais : la colonne est renseignée sur les 1 170 repères. Le raisonnement était juste — un repère sans durée n’a rien à mettre à l’échelle — et le fait invoqué était faux. C’est la règle du § 50.2 prise par un autre bout : **une règle tirée d’un raisonnement se vérifie avant d’être écrite**, et un commentaire de code n’en est pas dispensé.
...
- ⚠️ une glose qui se RÉPÈTE n’est pas une rubrique. « Prélevé dans la Bible de Sacy » paraît vingt fois sur une page ; en petites capitales elle appelait l’œil autant que la référence. Elle se compose en italique grise. *Un état qui ne varie pas n’informe pas* (§ 51.5), et il ne doit pas peser comme s’il informait.
...
⛔ **ET SUR UN TÉLÉPHONE, ON FAIT DE LA PLACE AVANT DE PLACER.** La case y vaut exactement la bande utile — 21 rem, ou l’écran moins ses deux marges : aucun côté horizontal ne peut donc la recevoir, et elle se pose forcément au-dessus ou au-dessous. Or la page amenait son sujet AU CENTRE, la seule place qui ne laisse assez de place ni d’un côté ni de l’autre. ⚠️ Mesuré le 7 septembre 2026, avec la vraie géométrie et des hauteurs de case relevées dans le navigateur : sur les cinq téléphones et les cinq tailles de sujet qu’on rencontre, la case RECOUVRAIT son sujet 210 fois sur 450, et le trait tombait avec elle — 59 % sur un iPhone SE dont le navigateur montre ses barres. La visite y expliquait, une fois sur deux, un sujet qu’elle venait de cacher.

**Le défilement décide donc où poser le sujet, et il le décide AVANT la case.** Au centre quand un côté horizontal peut la recevoir — les grands écrans ne bougent pas d’un pixel ; au centre encore quand le centre suffit déjà, pour ne pas faire remonter une page qui n’en avait pas besoin ; EN TÊTE quand la case ne peut aller que dessous, et tout ce qui reste est pour elle. ⚠️ On préfère le dessous parce qu’on lit de haut en bas : le sujet d’abord, ce qu’on en dit ensuite. Après cette règle, le recouvrement tombe à 108 sur 450, et le résidu est exactement le cas nommé ci-dessus — un volet qui occupe tout l’écran, qu’aucun défilement ne peut loger.

⛔ **LE PIED D’UNE CASE EST UN CHEMIN DE LECTURE, et ses boutons se touchent.** Mesurés à trente pixels, ils passaient le plancher de 24 que WCAG 2.2 impose et manquaient les 44 que le site vise partout où l’on lit (§ « LE DOIGT ») — or il n’y a rien d’autre à toucher dans une visite. Ils tiennent 44 px dès que le pointeur est grossier. ⚠️ L’axe est la CAPACITÉ DU POINTEUR, jamais la largeur : une fenêtre étroite sur un ordinateur garde sa souris.

⚠️ **LA BANDE UTILE SUIT LA FENÊTRE VISIBLE, non celle de la mise en page.** Safari iOS rend `window.innerHeight` comme la fenêtre LARGE, celle qu’on aurait si les barres du navigateur se rétractaient : une case posée au ras du bas se retrouvait derrière la barre d’outils, et aucun défilement ne la ramenait, puisqu’elle est fixée à la fenêtre et non au document. On prend la plus petite des deux mesures — la seule direction sûre — et le placement travaille enfin sur la même bande que le plafond de hauteur de la case.

⚠️ **CE QUI SE MESURE NE SE SUPPOSE PAS, et une case ne se mesure pas dans la page.** La composition de la case a donc quitté le composant, et son propos avec elle, exactement comme le contenu d’une fiche est séparé de sa fenêtre : sans cette coupure, aucune planche ne pouvait rendre les trente-neuf arrêts à toutes les largeurs de téléphone, et l’on en aurait été réduit à rejouer de mémoire une composition qui dérive au premier réglage.
...
**Un paragraphe par idée, deux ou trois par étape** (demande de l’auteur, 6 septembre 2026). On change de paragraphe quand on change de chose à dire : quelques petits blocs divisés se lisent d’un coup d’œil, quand un pavé de la même longueur ne se lit pas. ⛔ Une visite se lit debout, entre deux clics, et ce qui demande un développement n’est pas une explication mais un mode d’emploi, qui ne se lit pas non plus. ⚠️ QUATRE quand l’arrêt présente un axe qui a quatre états, et pas autrement : le « Modes » de la recherche nomme les trois façons de chercher, puis dit où leur explication se trouve. Ce n’est pas une idée de plus, c’est la liste que l’écran porte, et la couper serait en taire un morceau. Un seul arrêt sur les trente-neuf est dans ce cas (relevé du 7 septembre 2026).

⛔ **LE REGISTRE EST CELUI D’UN MANUEL** (reprise de l’auteur, 7 septembre 2026). Les six scénarios ont été réécrits ce jour-là, d’un trait, et la reprise fixe la manière d’écrire une visite. Le TITRE d’un arrêt est un nom, court et nominal — « Édition », « Verset », « Actions », « Recherches enregistrées » —, jamais une phrase, jamais un impératif, jamais une interrogation sans point d’interrogation : les trente-neuf en portent un, et aucun n’ouvre sur un verbe. Le TEXTE énonce ce que la chose FAIT, sans tournure engageante ni apostrophe au lecteur. L’ACCROCHE tient en une phrase qui situe la page, et le titre du message d’ouverture est le NOM de la page — « L’accueil », « La Bible classique », « Une œuvre » — non une formule d’accueil.

⛔ **Et la visite ne se présente pas elle-même.** La formule « Quelques étapes suffisent à vous montrer… » fermait les six accroches, en deux variantes ; elle est retirée. Ce que la visite va faire, ses deux boutons le disent déjà — « Commencer la visite », « Passer » — et le redire en prose coûte la seule phrase où l’on pouvait situer la page.

⚠️ **UN NOM DE COMMANDE SE COMPOSE EN GRAS**, par la syntaxe du site : « **Classique** », « **Livre entier** », « **Famille de mots** », « **Catalogue des traductions** ». C’est le seul enrichissement qu’emploient les six scénarios, et il porte un fait : ces mots-là sont écrits à l’écran, à l’endroit exact que la case désigne, et le lecteur doit pouvoir les y retrouver. ⛔ Le texte d’une visite est donc RENDU par le renderer du site — celui qui sert déjà la fiche d’auteur, la bulle d’une note et le volet d’un essai —, jamais posé tel quel : écrit sans lui, l’astérisque s’imprimerait. ⚠️ La typographie se pose au même endroit, comme partout ailleurs (§ 3.2) : le scénario s’écrit au clavier, et la fine insécable de « Jean 3, 16 » vient au rendu.
...
**L’ordre des étapes est celui de la PAGE : de haut en bas, de gauche à droite** (précisé par l’auteur le 6 septembre 2026), non celui de l’importance. Chaque colonne se descend tout entière avant qu’on passe à la suivante. ⚠️ Les colonnes se prennent l’une après l’autre, jamais par bandes horizontales : mesuré sur la page servie, la carte de l’édition, l’en-tête du texte et le volet des Pères ouvrent tous trois leur colonne à la même hauteur, et les ranger par ordonnée ferait sauter le regard d’un bord de l’écran à l’autre trois fois de suite. C’est le seul ordre qui n’oblige pas le lecteur à revenir sur ses pas.

⛔ **UNE VISITE NE S’OUVRE QUE LÀ OÙ SA PAGE PEUT LA PORTER.** Une case qui cernerait un texte à moitié rendu, ou des repères présents dans le document mais de taille nulle, ne désignent rien : la visite s’ouvrirait pour se fermer aussitôt, étape après étape. Chaque page dit donc elle-même quand elle est prête — le fondu d’ouverture achevé pour la Bible classique, dont le serveur rend le texte ; les colonnes venues et l’écran assez large pour la Polyglotte, qui charge le sien par le navigateur et refuse les écrans étroits.

**La visite de la Polyglotte** compte sept arrêts : le nombre de colonnes, la recherche d’un livre, le choix du passage, l’en-tête où l’on change de bible, la rangée et sa double numérotation, les actions d’une cellule, la colonne des notes. ⚠️ Le volet se descend dans l’ordre où il se VOIT, et c’est la règle d’ordre appliquée : mesuré sur la page servie, le bloc des traductions visibles ouvre le volet à 154 px du haut, le champ de recherche vient à 308, la liste des livres à 354. ⛔ Elle ne dit rien des versets surnuméraires, et c’est un arbitrage : ils ne paraissent que sur une minorité de chapitres, et une étape qui s’efface coûte à tout le monde l’attente qu’il faut pour constater son absence.
...
**La visite de l’accueil** compte sept arrêts, dans l’ordre où la barre se lit, de gauche à droite : elle-même, les deux bibles, la patristique, la Communauté, Aller plus loin, la recherche, et les affaires du lecteur. ⚠️ Les deux premières entrées de lecture portent DEUX flèches, l’onglet et la carte du milieu de page. ⛔ Rien sur Administration, qui ne paraît qu’à l’auteur du site ; rien sur la marque, qui ramène à l’accueil où l’on est déjà. ⚠️ Elle ne s’ouvre qu’en écran large : sous le seuil du menu déroulant la barre n’est qu’un bouton, et l’ouvrir couvrirait les cartes que la visite désigne.
...
**La visite de la Bibliothèque** — la page que la barre appelle « Patristique » — compte cinq arrêts : les trois sections, ce qui restreint la liste, une carte d’auteur, ce qu’elle déplie, et ce qu’une ligne d’édition offre. Une seule colonne, donc pas de question d’ordre entre volets : tout se descend. ⛔ Elle dit ce qu’aucune page voisine ne dit — que la liste ne porte QUE les auteurs dont une œuvre est en ligne, que le catalogue en recense bien d’autres qui ne le sont pas, et que l’étoile d’une ligne d’édition remplit l’onglet Favoris. Le reste se voit.
...
Ce chapitre tenait sous le chantier Fillion, où il n’était qu’une sous-partie. La notice bibliographique se compose pourtant partout sur le site — apparat d’une œuvre, ligne de péricope, fiche d’administration, presse-papiers — et elle a un moteur unique.
...
Une liste d’ouvrages lue SEULE, en pièce liminaire, prend la composition du § 47.0 : un seul titre en tête, une œuvre par ligne, aucune puce ni tiret, aucun cadre, aucun fond, aucune bordure, alignement à gauche, retrait suspendu, blanc léger entre deux références, et un corps légèrement inférieur au texte de l’apparat. ⚠️ Le retrait suspendu remplace le retrait de première ligne prescrit jusque-là (décision de l’auteur du 28 août 2026).
...
**Rappel de l’auteur du 5 septembre 2026 : « c’est ville, éditeur, année ; il faut uniformiser. »** L’adresse d’une édition — « Bar-le-Duc, Louis Guérin, 1866 » — se lit dans cet ordre, qui est celui de la notice bibliographique (§ 47.1) et du libellé court d’édition (§ 5). Il n’y a aucune raison qu’un libellé de navigation dise les mêmes trois choses dans un autre ordre.
...
Avant toute écriture, identifier toutes les surfaces susceptibles d’être rendues : `heading`, corps de lecture principal, `editorial_normalization.reading_text`, sous-blocs `editorial_normalization.blocks[].reading_text`, rendu des notes, titres et chapeaux. ⚠️ **Une correction n’est complète que si toutes les projections qui exposent le même contenu sont cohérentes.**
...
### 48.6. Sauvegardes et discipline d’écriture
...
Tout ce qui touche aux gravures d’une édition : les trois régimes, le détourage, le rattrapage, la part de colonne, l’export et la provenance.

### 49.0. Les gravures de Fillion — trois régimes, et ce que chacun fait du papier
...
### 49.1. La rampe alpha se mesure aux deux bouts, et sur cette gravure-ci
...
### 49.2. Une photogravure ne se détoure pas : elle se cadre
...
### 49.3. Une vignette se compose dans le commentaire qui couvre son verset
...
### 49.4. Une vignette franchit la manchette, et à gauche elle prend sa colonne
...
### 49.5. Le flou d'une gravure réduite est sa hachure, et aucun traitement ne la rend
...
### 49.6. Un fac-similé ne se complète pas : ce que deux modèles d'IA font vraiment d'une gravure
...
Une gravure se sert au double de sa taille d'affichage (§ 49.5). Sur un écran
...
contraste local (§ 49.5), rencontré par une autre porte. Les planches cadrées
...
⛔ **Ce n'est pas une exception au § 49.6, c'est son revers.** Deux choses très
...
45 %. La règle du § 49.5 n'est pas défaite, elle est encadrée.
...
Le § 49.7 réservait le rattrapage large au TRAIT et laissait le ton continu à
...
⛔ **Et l'on creuse les tons EN DERNIER** (§ 49.11). La puissance passée AVANT
...
sert un master, et c'est pourquoi il reste neutre (§ 49.11).
...
du § 49.11 y serait posée sans avoir regardé chacune. **Question ouverte.**
...
EMPILE. C'est la règle qui a présidé à la reprise des 32 planches (§ 49.14).
...
le double de la taille affichée (§ 49.5) : toucher aux bornes du § 49.12 sans
...
§ 49.21 dit comment on l'a su** : `is_public` est nécessaire et ne suffit pas,
...
alpha (§ 49.1), la demi-largeur du pic prise de son côté CLAIR — nettoie
...
qu'au § 49.13. Une planche parfaitement propre dont le trait a fondu n'est pas un
...
noircir serait une falsification (§ 49.6).

⚠️ **Le blanc de papier est une remise à l'ÉCHELLE, non un parti** : c'est ce qui
l'autorise là où la puissance de ton du § 49.11 reste, elle, en attente. La
distinction est celle du master (§ 49.15) : un master porte le recadrage et
...
vérification du § 49.15 a signalé une discordance sur une planche — 282 410
...
première.** Le § 49.13 a retiré la part fixe des vignettes le 30 août, parce
...
rien. Le régime ne décide plus que du DÉTOURAGE, ce que le § 49.5 énonçait déjà
...
Le § 49.4 annonçait que les gravures sans mention de procédé « retombent au
...
apparaît. C’est l’inverse du pari du § 49.4, et il faut le corriger dans l’autre
...
Le § 49.4 dit qu’aucune mesure de pixel ne sépare la demi-teinte du bois gravé,
...
REÇOIT.** Le § 49.15 pose déjà la règle pour le RENDU ; elle vaut un cran plus
...
bonne, et c’est celle du § 49.21. Mais elle exigeait **en outre**
...
La page de comparaison des traductions : son réglage, sa composition, et les relevés d’ossature qui la teignent.
...
⛔ **Le manque se dit ENTRE CROCHETS**, dans les DEUX membres de l’édition. C’est le signe que la philologie donne depuis toujours à ce qu’un témoin a perdu, et c’est celui que la donnée écrit déjà ; les chevrons « ⟨ Lacune ⟩ » de la colonne du manuscrit y reviennent, de sorte que le même fait se dise du même signe des deux côtés d’une seule édition.

⛔ **ET CE QUI S’IMPRIME ENTRE CES CROCHETS EST LA CAUSE, jamais des points de suspension** (décision de l’auteur, 2026-09-08 : « plutôt que des “…”, indiquer la nature de la lacune ou du problème » — elle renverse la règle du 5 septembre, qui gardait le motif à l’infobulle). « [déchirure] », « [fin du manuscrit] ». La cause n’est pas une donnée d’appoint : `tei.ts` REFUSE tout `<gap>` privé de `reason` et de `cause`, et l’écrit dans le marqueur. La masquer revenait à jeter à l’écran le seul renseignement que l’édition ait pris la peine d’établir, pour lui substituer trois points qui ne disent rien. ⚠️ Une lacune SANS cause porte le mot « lacune », qui dit au moins de quoi il s’agit ; « non précisée », que l’import écrit faute de mieux, est un aveu d’ignorance et non une cause : il retombe sur le mot nu. ⚠️ Un marqueur COUPÉ entre deux versets retombe lui aussi sur le mot nu — la cause qu’on y lirait serait tronquée, et l’on n’invente pas ce qu’on ne peut pas lire.

⛔ **LA MARQUE PREND ALORS L’ITALIQUE, et ce n’est pas un ornement.** Le jour où elle porte un MOT et non trois points, « [déchirure] » et « [m’exauça] » ont exactement la même forme — et le second est une RESTITUTION, du texte rendu au témoin par l’éditeur. L’italique est la voix éditoriale du site, celle de toute mention : elle sépare ici ce qui parle DU manuscrit de ce qui parle POUR lui. Règle générale : **une marque d’absence qui cesse d’être un signe pour devenir un mot doit prendre la voix éditoriale**, faute de quoi elle se lit comme du texte.
...
⛔ **UNE TRADUCTION NON RECOMPOSÉE NE PASSE PAS PAR LE TOKENISEUR DU TÉMOIN.** Celui-ci consomme tout crochet fermant, parce que la recomposition par créneau canonique coupe un marqueur en deux ; la traduction moderne, elle, porte QUATRE-VINGT-CINQ RESTITUTIONS entre crochets (« il [m’exauça] »), qui sont l’usage philologique et doivent s’imprimer telles quelles. Le tokeniseur y mangerait leur crochet fermant.

⛔ **LA RÈGLE QUI LES PROTÈGE SE DÉMONTRE, elle ne se soupèse pas : ON NE CONSOMME JAMAIS UN « ] » QUI SUIT UN « [ » DANS LA MÊME PORTION.** Ne sont donc reconnues, dans un texte non recomposé, que des formes NOMMÉES — un marqueur du vocabulaire déclaré, complet ou ouvert jusqu’au bout du verset — et une FERMETURE ORPHELINE, définie comme un « ] » qui précède TOUT crochet ouvrant : aucune restitution ne peut occuper cette place, puisqu’il lui faudrait un « [ » avant elle. ⚠️ Et cette fermeture ne se lit que dans la portion qui OUVRE le texte : ailleurs, le crochet ouvrant qui l’appareille peut vivre dans une portion précédente. La règle dégrade du bon côté — un marqueur non reconnu s’imprime tel quel, aucune restitution n’est mangée.
...
⛔ **UN FAIT DE L’ÉDITION NE SE DIT PAS DE DEUX FAÇONS SELON LA COLONNE OÙ ON LE LIT** (2026-09-08). La traduction moderne imprimait BRUTS ses 652 marqueurs « [lecture incertaine : … ] » : le lecteur trouvait « et [lecture incertaine : preig] » au milieu d’une phrase française — un terme d’atelier posé dans le texte — quand la colonne du manuscrit rendait le même fait d’une simple teinte. Ils prennent désormais la forme du témoin : teinte effacée, infobulle, sans crochets. Relevé en base avant de coder : 554 paires complètes, 43 ouvertures coupées et 43 fermetures orphelines, exactement appareillées, zéro lacune coupée.

⚠️ **RESTE UNE DOUZAINE D’ÉTIQUETTES QUE LE VOCABULAIRE NE CONNAÎT PAS**, et elles s’impriment brutes à dessein : « lecture difficile » (2), « Fragment » (2), « Suite incertaine » (2), « Suite corrompue » (2), « Restitution », « Restitution incertaine », « Passage altéré », « reprise ». ⛔ Le rendu ne DOIT PAS inférer une sémantique d’un libellé quelconque placé avant un deux-points entre crochets (§ registre GPT ↔ Claude) : « Restitution » n’est pas « lecture incertaine », et la distinguer d’une restitution philologique ne se devine pas. Elles attendent un vocabulaire contrôlé, c’est-à-dire un arbitrage de l’auteur — non une règle de code.
...
### 51.x Jamais de composant déclaré au rendu

Un composant défini DANS le corps d'un autre composant change d'identité à chaque rendu du parent. React ne le reconnaît pas d'un rendu à l'autre : il démonte l'ancien et en monte un neuf, avec son état et ses nœuds du DOM.
