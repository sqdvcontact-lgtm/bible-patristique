...
- **CONFIRMÉ — notes d’apparat rendues obsolètes par une correction V2.** Une mutation contrôlée de `versets_v2.texte` peut invalider le commentaire d’un `bible_verse_note_block` sans que Control V2 autorise la correction de ce bloc. Cas constatés après contrôle lexical de TR0013 : la note `573d25eb-734e-4edb-8608-592745edcbea` d’`ACT.4.4` décrit encore « mille » comme rétabli alors que le témoin et la cible corrigée portent seulement « cinq » ; la note `aa52a08b-1a36-405a-a762-421bc9e80d32` de `NUM.29.22` décrit encore l’offrande et la libation rétablies d’après les contrôles alors que la cible corrigée conserve désormais « le sacrifice tué ». ⛔ Ne pas réparer ces blocs hors protocole. Claude doit étendre le chemin de mutation gardé à `bible_verse_note_block`, puis traiter ces dettes et ajouter un contrôle de cohérence entre une note de restitution et le texte courant qu’elle décrit.
...
- **CONFIRMÉ — gloses manuscrites encore absorbées dans le canon TR0009/TR0013.** L’audit reproductible `TR0009-gloss-absorption-qc-20260905-v1` montre une dette structurelle réelle. TR0009 contient actuellement 164 segments `manuscript_extra=true`, dont seulement 39 portent `phenomenon="gloss"`. Parmi 44 cas uniques lus dans un échantillon volontairement enrichi en expansions explicatives ou anormalement longues, 42 sont des développements exégétiques, lexicaux ou doctrinaux distincts du noyau biblique ; `JDT.5.13` et `TOB.13.1` relèvent plutôt de différences de recension/versification. Cas représentatifs : `MAT.1.25` (glose insérée entre deux membres du même verset), `JHN.2.4`, `LUK.2.7`, `MAT.6.13`, `JHN.9.4`, `JHN.16.23`, `JHN.19.14`, `JHN.19.27`, ainsi que des cas très longs sans formule déclencheuse comme `MRK.5.43`, `JHN.18.9`, `JHN.19.15` et `MAT.26.19`. Le modèle cible existe déjà : le statut hors canon est `manuscript_extra=true` / `material_extra=true`, tandis que la nature du phénomène est `metadata.phenomenon="gloss"`, avec `canonical_context` renseigné ; ne pas créer une nature `glose` concurrente de `surnuméraire`. ⛔ Ne jamais transformer automatiquement les 739 occurrences canoniques de `ce est a dire` en extras : cette formule n’est qu’un signal de candidature, et des gloses existent aussi sans elle. Toute séparation doit être revue philologiquement, préserver exactement la matière et l’ordre matériel, les bornes d’incertitude et les références de lignes, et distinguer les gloses des expansions de recension. Le code et le modèle d’alignement doivent supporter un surnuméraire intercalé **à l’intérieur d’un même verset canonique**, entre deux fragments canoniques, sans perte ni duplication à la recomposition. Après recoupe : contrôler exhaustivement recomposition expanded/diplomatic, ordre matériel, couverture des offsets, cible TR0013, extras et dépendances. `bible_editorial_segment` restant hors Control V2, ne pas effectuer ces recoupes par écriture directe : étendre d’abord le chemin contrôlé ou fournir une mutation gardée équivalente. Ajouter des tests de régression sur `MAT.1.25` (positif interne), une glose post-verset et un long passage de recension négatif (`JDT.5.13`).

- **CONFIRMÉ — notes d’apparat obsolètes supplémentaires après correction V2.** Aux dettes déjà recensées pour ACT.4.4 et NUM.29.22 s’ajoutent `2PE.1.16`, note `ecd7fc25-7430-49df-a76a-33f19d1b2640`, qui décrit encore le verbe suppléé « vu » désormais retiré de la cible ; `NUM.36.11`, note `b4156a41-1b95-4604-9fa2-f6c542561ecd`, dont le commentaire accompagne encore l’ancienne formulation grammaticalement fautive ; `JDT.4.10`, note `46c0c507-5adc-497c-840a-3b8a066584a1`, qui affirme encore qu’une négation perdue a été restituée alors que la cible conserve désormais la contradiction positive du témoin ; et `NUM.34.22`, note `a63ae1bf-9714-46ab-960f-5aa3588efd92`, qui décrit encore « fils de Jogli » comme filiation rétablie alors que la cible conserve désormais `Bocchu Leogli` sans ce lien suppléé. À ces six dettes s’ajoutent après la passe lexicale v4 : `1SA.30.10`, note `4fc04a8f-6f42-4a19-a71c-b775389f28ce`, qui décrit encore la reconstruction des quatre cents poursuivants et des deux cents hommes restés au Besor désormais retirée ; `DEU.10.10`, note `f7b2e6e1-09d5-4f9c-be71-2e92518d332f`, qui présente encore « le Seigneur m’exauça » comme restitution retenue alors que ce verbe a été supprimé de la cible ; `JOB.15.14`, note `694813a7-1426-450b-a69d-ab095f5669fc`, qui décrit encore « sans tache et juste » comme restitution minimale alors que cette importation latine a été retirée. À ces neuf dettes s’ajoute `NUM.26.51`, note `35f0ad35-8e23-4744-b4e2-d1c10e0aa78a`, qui affirme encore que le total 601 730 est rétabli d’après le latin et la traduction moderne alors que la cible conserve désormais la formulation numérique effectivement transmise par le témoin (« six mille, cent, sept et trente »). À ces dix dettes s’ajoutent après l’audit nombres/noms v1 : `1KI.5.30`, note `a34315a6-2b87-4e76-a3dd-ae8948845def`, qui décrit encore 3 300 surveillants rétablis d’après les contrôles alors que la cible conserve désormais « trois mille et trois » ; `NUM.26.37`, note `5c2f8827-f3aa-4b02-9e38-3434bb24d20c`, qui décrit encore 32 500 comme complément contrôlé alors que la cible conserve « trente-deux mille et cinq » ; `NUM.28.11`, note `010b79b0-4c71-403d-80ce-4d3f72fe3e0d`, qui explique encore la substitution de deux veaux alors que la cible conserve maintenant la leçon propre « deux agneaux du gros bétail ». À ces treize dettes s’ajoutent après l’audit numérique v2 : `NUM.26.25`, note `5630b88c-9145-4780-ab23-57398bed86c7`, qui décrit encore 64 300 comme complément contrôlé alors que la cible conserve « soixante-quatre mille et trois » ; `NUM.31.28`, note `2b16a324-8033-4e43-bf98-b153cfc33a58`, qui justifie encore le taux d’un sur cinq cents alors que le témoin et la cible portent un sur cinquante ; `NUM.31.34`, note `95a23ef5-869b-4ec0-b7e9-77ff3a351039`, qui affirme encore le rétablissement de 61 000 au lieu des 60 000 transmis ; `NUM.31.37`, note `1dd0dfd8-a154-48da-9b81-260d2e08e941`, qui justifie encore 675 alors que la cible conserve « six et soixante-quinze » ; `NUM.31.39`, note `dd41926d-ff42-4905-a204-453781862756`, qui explique encore 30 500/61 alors que la cible conserve désormais 32 500/60. À ces dix-huit dettes s’ajoutent `2SA.24.15`, note `fa0bb41a-e02b-483f-b284-09c39a326704`, et `2SA.24.17`, note `51dc455a-59ea-4143-bf5e-3d66c9c29a6a` : toutes deux décrivent encore une restauration canonique désormais supprimée au profit de `[…]` et du seul fragment transmis. Comme `bible_verse_note_block` reste hors Control V2, ne pas réparer ces blocs par écriture directe ; étendre le chemin contrôlé et traiter ensemble les vingt dettes connues.\n\n- **CONFIRMÉ — omission canonique encadrée par des fragments partiels.** Cas `2SA.24.15–17` : la couche source vérifiée ligne par ligne passe directement de `f161r_b_l20` (« del matin tres qua tens ») à `f161r_b_l21` (« que sont cornes contre moi ») ; `2SA.24.16` est non portant. Aucun texte n’était masqué par les offsets. TR0013 avait néanmoins complété silencieusement la fin de 24,15 et le début de 24,17 d’après le latin/AELF. Règle : lorsqu’un slot non portant est encadré par des fragments tronqués, contrôler les unités source complètes et leurs offsets avant toute restitution ; si la matière n’est réellement pas transmise, ne pas remplir depuis le canon, employer la convention existante `[…]` aux frontières et conserver seulement les petites restitutions locales explicitement crochetées. Ajouter un test de régression 2SA.24.15–17 et vérifier qu’une vue/recomposition ne transforme jamais automatiquement un slot non portant en texte canonique restauré.
