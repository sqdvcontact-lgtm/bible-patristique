...
### 35.14.5. Listes d’abréviations — références complètes et normalisées

Une liste d’abréviations est une table de correspondance structurée. Chaque entrée conserve séparément le sigle imprimé, son expansion source et, lorsque le sigle désigne un ouvrage, la relation vers `ouvrages_bibliographiques` et ses autorités normalisées. ⛔ Une référence bibliographique ne se recompose jamais par analyse d’une chaîne précomposée : on passe par le normalisateur bibliographique et ses champs structurés ; les compléments propres à l’édition citée sont ajoutés seulement lorsqu’ils sont attestés par le témoin ou une autorité externe sûre.

Dans la couche de lecture, chaque entrée occupe une ligne autonome sous la forme `sigle — référence`. Toutes les lignes prennent un point final. Lorsqu’il s’agit d’un ouvrage, la référence est aussi complète que le permettent les données établies : auteurs sous leurs formes d’autorité, titre et sous-titre en italique, mention d’édition, lieu, éditeur, date, nombre de volumes, format, pagination, planches, figures, cartes ou autre description matérielle pertinente. Contrairement aux listes générales d’ouvrages du § 35.6.1, ces informations matérielles sont ici conservées à l’affichage lorsque la table d’abréviations les fournit ou lorsqu’elles précisent sans ambiguïté l’édition citée.

Le normalisateur distingue l’identité de l’œuvre de l’état ou de l’édition effectivement cités. Si le sigle ne permet pas d’identifier une édition unique, les champs certains sont affichés et l’incertitude reste documentée ; aucune année ni mention d’édition n’est inventée pour compléter artificiellement la référence. Les formes sources demeurent intégralement conservées pour contrôle. Un sigle conventionnel non bibliographique, tel `LXX`, reste une simple expansion, mais sa ligne reçoit elle aussi son point final.
