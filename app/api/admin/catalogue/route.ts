...
export async function GET(req: Request) {
  ...
  let q = supabaseAdmin
    ...
    .select(
      ...
      'verifie, genre, langue_originale, date_oeuvre, authenticite, created_at',
      { count: 'exact' }
    )
...
export async function PATCH(req: Request) {
  if (!(await estAdminUtilisateur(req))) return NextResponse.json({ error: 'Non autorisé.' }, { status: 403 })
  const { id, verifie } = await req.json()
  if (!id) return NextResponse.json({ error: 'id manquant' }, { status: 400 })
  const { error } = await supabaseAdmin.from('catalogue_notices').update({ verifie }).eq('id', id)
