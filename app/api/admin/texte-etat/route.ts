...
//
// L'état de validation d'un texte, et le motif qui le retient (charte § 52).
//
...
const COLONNES_TEXTE =
  'id_texte, id_oeuvre, titre_version, langue, edition_label, statut, is_public, is_default, nb_signes, motif_non_publication'
...
export async function POST(req: NextRequest) {
  ...
  const { id_texte, statut, motif } = await req.json()
  ...
  const { error } = await supabaseAdmin
    .from('oeuvre_textes')
    .update({ statut, motif_non_publication: motifPropre })
