...
export default function EncartTraduction({ trad }: { trad: TraductionEncart }) {
  ...
  return (
    ...
    <div className="cs-volet-carte" style={{ flexShrink: 0, boxSizing: 'border-box', overflow: 'hidden', padding: '10px 10px 9px', borderBottom: '1px solid var(--cs-bord)', background: 'var(--cs-fond)', display: 'flex', flexDirection: 'column', gap: '3px' }}>
      ...
      {/* ⚠️ Le libellé se raccourcit avec le volet : « En savoir plus sur cette
          traduction » tient sur trois lignes à 120 pixels. Les deux formes sont
          écrites, et la requête de conteneur n'en montre qu'une — un libellé ne se
          coupe pas en JavaScript, sinon il faudrait mesurer à chaque rendu. */}
      <button onClick={() => setModaleOuverte(true)} style={{ marginTop: 'auto', paddingTop: '4px', fontSize: '0.59375rem', color: 'var(--cs-texte-faible)', textDecoration: 'underline', textUnderlineOffset: '2px', width: 'fit-content', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
