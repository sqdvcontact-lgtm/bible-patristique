  ...
  return createPortal(
    <div onClick={onClose}
      ...
      <div onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="cs-compte-titre"
        style={{ background: '#fffdfb', borderRadius: '12px', border: '1px solid var(--cs-bord)', width: '100%', maxWidth: '25rem', boxShadow: '0 18px 50px rgba(60,40,20,0.26)', overflow: 'hidden' }}>

        {/* Bandeau — teinte vert d'encre, emblème discret (plume). */}
        <div style={{ position: 'relative', padding: '22px 24px 18px', background: 'linear-gradient(180deg, var(--cs-vert-pale) 0%, #fbf8f3 100%)', borderBottom: '1px solid var(--cs-bord-clair)' }}>
          ...
          <div aria-hidden="true"
            style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#fffdfb', border: '1px solid rgba(var(--cs-vert-rgb),0.30)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '11px' }}>
        ...
        <div style={{ padding: '17px 24px 22px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '9px' }}>
            <Link href={ROUTE_INSCRIPTION} onClick={onClose}
              style={{ display: 'block', textAlign: 'center', fontSize: '0.8125rem', fontWeight: 600, padding: '10px 16px', borderRadius: '7px', background: 'var(--cs-vert)', color: '#fff', textDecoration: 'none' }}>
              Créer un compte
            </Link>
            <Link href={ROUTE_CONNEXION} onClick={onClose}
              style={{ display: 'block', textAlign: 'center', fontSize: '0.75rem', padding: '8px 16px', borderRadius: '7px', border: '1px solid var(--cs-bord)', background: 'var(--cs-surface)', color: 'var(--cs-texte-second)', textDecoration: 'none' }}>
              J’ai déjà un compte
            </Link>
          </div>
        </div>
