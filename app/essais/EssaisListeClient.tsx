    ...
    <main style={{ background: '#f7f4ef', minHeight: '100vh', paddingTop: '8px' }}>
      <div style={{ maxWidth: '71rem', margin: '0 auto', padding: '8px 30px 70px' }}>
        ...
        <div style={{ position: 'relative', textAlign: 'center', marginBottom: '4px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-source-serif), Georgia, serif', fontSize: '1.6875rem', fontWeight: 'normal', color: '#1e2e24', margin: 0, letterSpacing: '0.02em' }}>
            Publications
          </h1>
          <div aria-hidden="true" style={{ color: '#9a7a40', fontSize: '0.4375rem', letterSpacing: '0.4em', margin: '6px 0 8px' }}>◆</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '2px', borderBottom: '1px solid #ddd8cf' }}>
            {([
              ...
              <button key={o.key} onClick={() => setOnglet(o.key)}
                style={{ padding: '6px 16px', fontSize: '0.71875rem', fontWeight: onglet === o.key ? 600 : 400, color: onglet === o.key ? 'var(--cs-vert)' : '#9a958d', background: 'transparent', border: 'none', borderBottom: onglet === o.key ? '2px solid var(--cs-vert)' : '2px solid transparent', cursor: 'pointer', whiteSpace: 'nowrap', letterSpacing: '0.01em' }}>
                {o.label}
              </button>
            ))}
          </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', margin: '14px 0 18px' }}>
              {([
                ...
                <button key={s.key} onClick={() => setSousEcrire(s.key)}
                  style={{ fontSize: '0.6875rem', padding: '5px 14px', borderRadius: '999px', border: `1px solid ${sousEcrire === s.key ? 'var(--cs-vert)' : '#d6d0c4'}`, background: sousEcrire === s.key ? 'rgba(var(--cs-vert-rgb),0.09)' : 'rgba(255,255,255,0.6)', color: sousEcrire === s.key ? 'var(--cs-vert)' : '#8a8278', fontWeight: sousEcrire === s.key ? 600 : 400, cursor: 'pointer' }}>
                  {s.label}
                </button>
              ))}
            </div>
    ...
    <>
      ...
      <style>{`
        ...
        .publications-sommaire-tete::after {
          ...
          height: 1px;
          flex: 1;
          background: rgba(214,208,196,0.55);
        }
        ...
        .publication-entree {
          ...
          border-top: 1px solid rgba(214,208,196,0.55);
        ...
        @media (max-width: 900px) {
          ...
          .publication-entree.premier { border-top: 1px solid rgba(214,208,196,0.55); }
        }
      `}</style>
        ...
        {groupes.map(g => {
          ...
          return (
            <button key={g.key} onClick={() => setFiltre(g.key)}
              style={{ fontSize: '0.625rem', padding: '3px 10px', borderRadius: '999px', border: `1px solid ${actif ? 'var(--cs-vert)' : '#e0dacf'}`, background: actif ? 'rgba(var(--cs-vert-rgb),0.09)' : 'transparent', color: actif ? 'var(--cs-vert)' : '#9a958d', cursor: 'pointer', fontWeight: actif ? 700 : 500, letterSpacing: '0.01em' }}>
              ...
              <div key={e.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '10px', alignItems: 'center', background: statutStyle.fond, border: `1px solid ${statutStyle.bordure}`, borderLeft: `3px solid ${statutStyle.accent}`, borderRadius: '7px', padding: '8px 12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
                  <button onClick={() => basculerPublication(e)} disabled={!peutBasculer || verrouille}
                    ...
                    style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '0.625rem', color: e.statut === 'publie' ? 'var(--cs-vert)' : '#9a958d', background: 'transparent', border: 'none', padding: 0, cursor: !peutBasculer || verrouille ? 'default' : 'pointer', opacity: !peutBasculer ? 0.4 : 1, fontWeight: 600 }}>
                    {timer && <span style={{ fontSize: '0.5625rem', color: '#9a958d', fontWeight: 600 }}>{timer}</span>}
                    <span style={{ width: '26px', height: '14px', borderRadius: '999px', background: e.statut === 'publie' ? 'var(--cs-vert)' : '#d6d0c4', position: 'relative', display: 'inline-block', transition: 'background 0.15s' }}>
        ...
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            <button
              ...
              style={{ fontSize: '0.71875rem', color: peutRelancer ? 'var(--cs-vert)' : '#b0a89e', background: 'none', border: 'none', cursor: peutRelancer ? 'pointer' : 'default', padding: 0, textDecoration: peutRelancer && !chargement ? 'underline' : 'none', fontStyle: 'italic' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', marginTop: '14px' }}>
              {versets.map((_, i) => (
                <button key={i} onClick={() => setIndex(i)}
                  style={{ width: '6px', height: '6px', borderRadius: '50%', background: i === index ? 'var(--cs-vert)' : '#d6d0c4', border: 'none', cursor: 'pointer', padding: 0, transition: 'background 0.13s' }} />
              ))}
            </div>
          )}
        </>
