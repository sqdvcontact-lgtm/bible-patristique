...
function couleurDecision(d: string | null): { color: string; bg: string; border: string } {
  ...
  return { color: 'var(--cs-texte-second)', bg: 'var(--cs-fond)', border: 'var(--cs-bord-clair)' }
}
...
function ChampNotice({ label, valeur, accent = false, transform }: {
  ...
  return (
    <div style={{
      ...
      <span style={{ display: 'block', marginBottom: '2px', fontSize: '0.625rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: critique ? '#b14b38' : 'var(--cs-texte-doux)' }}>
        {label}
      </span>
      <span style={{ display: 'block', fontSize: '0.8125rem', lineHeight: 1.3, color: critique ? 'var(--cs-danger)' : 'var(--cs-encre)', fontWeight: accent ? 700 : 400, wordBreak: 'break-word' }}>
        {contenu}
      </span>
    </div>
  )
...
function GroupeNotice({ titre, children }: { titre: string; children: React.ReactNode }) {
  return (
    <section style={{ border: '1px solid var(--cs-fond-doux)', borderRadius: '8px', background: 'var(--cs-fond-clair)', padding: '10px' }}>
      <h3 style={{ margin: '0 0 8px', fontSize: '0.65625rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--cs-texte-gris)' }}>{titre}</h3>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '9px' }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '7px', flexWrap: 'wrap' }}>
        {[
          ...
          <span key={label} style={{ fontSize: '0.65625rem', color: 'var(--cs-texte-gris)', background: 'var(--cs-fond-doux)', border: '1px solid var(--cs-bord-clair)', borderRadius: '4px', padding: '2px 6px' }}>
    ...
    <div style={{ maxWidth: '60rem' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <input
          ...
          style={{ fontSize: '0.8125rem', padding: '5px 10px', border: '1px solid var(--cs-vert-fonce)', borderRadius: '4px', background: 'var(--cs-encre-fonce)', color: '#c8d8cc', outline: 'none', width: '12.5rem' }}
        />
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {regrouperNotices(notices).map(groupe => (
            <div key={groupe.cle} style={{ border: '1px solid var(--cs-bord-clair)', borderRadius: '8px', overflow: 'hidden', background: 'var(--cs-surface)' }}>
              <div style={{ padding: '9px 14px', background: 'var(--cs-fond)', borderBottom: '1px solid var(--cs-bord-clair)', display: 'flex', alignItems: 'baseline', gap: '9px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '0.9375rem', fontWeight: 700, color: 'var(--cs-encre)', fontFamily: 'var(--font-source-serif), Georgia, serif' }}>{groupe.titre}</span>
                <span style={{ fontSize: '0.78125rem', color: 'var(--cs-texte-second)', fontStyle: 'italic' }}>{groupe.auteur}</span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {groupe.notices.map(n => {
                  ...
                  return (
                    <div key={n.id} style={{
                      background: n.verifie ? 'var(--cs-fond)' : 'var(--cs-surface)',
                ...
                <div style={{ padding: '9px 14px', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => setOuverte(ouv ? null : n.id)}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--cs-texte)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '22.5rem', fontStyle: n.titre_edition || n.titre_original ? 'italic' : 'normal' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', flexShrink: 0 }}>
                    <span style={{ fontSize: '0.6875rem', fontWeight: 700, padding: '2px 6px', borderRadius: '4px', background: 'var(--cs-fond)', color: couleurScore(n.score_fiabilite), minWidth: '24px', textAlign: 'center' }}>
                      {n.score_fiabilite ?? 'Score ?'}
                    </span>
                    <span style={{ fontSize: '0.65625rem', padding: '2px 7px', borderRadius: '4px', background: dec.bg, color: dec.color, border: `1px solid ${dec.border}`, whiteSpace: 'nowrap' }}>
                      {labelDecision(n.decision_import)}
                    </span>
                    {n.verifie ? (
                      <span style={{ fontSize: '0.65625rem', padding: '2px 6px', borderRadius: '4px', background: 'var(--cs-vert-pale)', color: 'var(--cs-vert)', fontWeight: 600 }}>Validee</span>
                    ) : (
                      <span style={{ fontSize: '0.65625rem', padding: '2px 6px', borderRadius: '4px', background: 'var(--cs-danger-fond)', color: 'var(--cs-danger)' }}>A verifier</span>
                    )}
                    {n.url_source && (
                      <a
                        href={n.url_source}
                        ...
                        title={n.url_source}
                        style={{ fontSize: '0.6875rem', padding: '3px 8px', borderRadius: '4px', border: '1px solid #b8cddd', background: 'var(--cs-fond)', color: '#2f5d7a', cursor: 'pointer', fontWeight: 600, whiteSpace: 'nowrap', textDecoration: 'none' }}
                      >
                        Source&nbsp;↗
                      </a>
                    )}
                    <button
                      ...
                      style={{ fontSize: '0.6875rem', padding: '3px 8px', borderRadius: '4px', border: '1px solid var(--cs-bord)', background: ouv ? 'var(--cs-vert)' : 'var(--cs-surface)', color: ouv ? 'var(--cs-surface)' : 'var(--cs-vert)', cursor: 'pointer', fontWeight: 600, whiteSpace: 'nowrap' }}
                    >
                ...
                {ouv && (
                  <div style={{ borderTop: `1px solid ${n.verifie ? '#b8d4c4' : 'var(--cs-fond-doux)'}`, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '10px', background: n.verifie ? 'var(--cs-fond-doux)' : 'var(--cs-fond-clair)' }}>
                    ...
                    {n.url_source && (
                      <a href={n.url_source} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.78125rem', color: '#3d6b87', wordBreak: 'break-all', textDecoration: 'underline' }}>
                        {n.url_source}
                      </a>
                    )}
                    ...
                    {n.verifie ? (
                      ...
                      <div style={{ display: 'flex', gap: '8px', paddingTop: '6px', borderTop: '1px solid var(--cs-fond-doux)', flexWrap: 'wrap' }}>
                        <button onClick={() => valider(n.id)} className="btn-vert" style={{ fontSize: '0.78125rem', padding: '5px 14px', borderRadius: '4px', cursor: 'pointer' }}>
                          Valider définitivement
                        </button>
                        <button onClick={() => supprimer(n.id)} className="btn-rouge" style={{ fontSize: '0.78125rem', padding: '5px 12px', borderRadius: '4px', cursor: 'pointer', marginLeft: 'auto' }}>
                          Supprimer
                        </button>
                      </div>
                    )}
