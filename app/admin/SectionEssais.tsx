...
export default function SectionEssais({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {liste.length === 0 ? (
        <Carte><p style={{ fontSize: '0.93437rem', color: 'var(--cs-texte-doux)', fontStyle: 'italic', margin: 0 }}>Aucun essai en attente de validation.</p></Carte>
      ) : liste.map(e => {
        ...
        return (
          <Carte key={e.id}>
            ...
            <Link href={`/essais/${e.id}`} target="_blank" rel="noopener noreferrer"
              style={{ fontFamily: "var(--font-source-serif), Georgia, serif", fontSize: '1.07813rem', color: 'var(--cs-encre-fonce)', textDecoration: 'none', display: 'block', marginBottom: '2px' }}>
              {e.titre} ↗
            </Link>
            {e.sous_titre && <p style={{ fontSize: '0.8625rem', color: '#8a8278', fontStyle: 'italic', margin: '0 0 6px' }}>{e.sous_titre}</p>}
            {e.resume && <p style={{ fontSize: '0.89844rem', color: 'var(--cs-texte)', lineHeight: 1.55, margin: '0 0 8px' }}>{e.resume}</p>}

            <div style={{ display: 'flex', gap: '14px', marginBottom: '10px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '0.79062rem', color: 'var(--cs-texte-second)', fontWeight: 500 }}>{e.auteur_pseudo ?? 'Auteur inconnu'}</span>
              <span style={{ fontSize: '0.79062rem', color: 'var(--cs-texte-faible)', marginLeft: 'auto' }}>{dateFormat(e.created_at)}</span>
            </div>

            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              {statut === 'loading' ? (
                <span style={{ fontSize: '0.82656rem', color: 'var(--cs-texte-doux)' }}>…</span>
              ) : statut === 'publie' ? (
                <span style={{ fontSize: '0.82656rem', color: 'var(--cs-vert)', fontWeight: 600 }}>✓ Publié</span>
              ) : statut === 'renvoye' ? (
                <span style={{ fontSize: '0.82656rem', color: 'var(--cs-danger)' }}>Renvoyé en brouillon</span>
              ) : (
                <>
                  <button onClick={() => renvoyer(e.id)} className="btn-gris" style={{ fontSize: '0.82656rem', padding: '5px 14px', borderRadius: '5px', cursor: 'pointer' }}>
                    Renvoyer en brouillon
                  </button>
                  <button onClick={() => renvoyer(e.id, true)} className="btn-rouge" style={{ fontSize: '0.82656rem', padding: '5px 14px', borderRadius: '5px', cursor: 'pointer' }}>
                    Refus
                  </button>
                  <button onClick={() => publier(e.id)} className="btn-vert" style={{ fontSize: '0.82656rem', padding: '5px 14px', borderRadius: '5px', cursor: 'pointer' }}>
                    Publier ✓
                  </button>
                </>
              )}
