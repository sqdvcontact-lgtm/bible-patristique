...
export default function SectionEssais({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {liste.length === 0 ? (
        <Carte><p style={{ fontSize: '0.93437rem', color: '#9a958d', fontStyle: 'italic', margin: 0 }}>Aucun essai en attente de validation.</p></Carte>
      ) : liste.map(e => {
        ...
        return (
          <Carte key={e.id}>
            ...
            <Link href={`/essais/${e.id}`} target="_blank" rel="noopener noreferrer"
              style={{ fontFamily: "var(--font-source-serif), Georgia, serif", fontSize: '1.07813rem', color: '#1e2e24', textDecoration: 'none', display: 'block', marginBottom: '2px' }}>
              {e.titre} ↗
            </Link>
            {e.sous_titre && <p style={{ fontSize: '0.8625rem', color: '#8a8278', fontStyle: 'italic', margin: '0 0 6px' }}>{e.sous_titre}</p>}
            {e.resume && <p style={{ fontSize: '0.89844rem', color: '#5a5450', lineHeight: 1.55, margin: '0 0 8px' }}>{e.resume}</p>}

            <div style={{ display: 'flex', gap: '14px', marginBottom: '10px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '0.79062rem', color: '#6b6560', fontWeight: 500 }}>{e.auteur_pseudo ?? 'Auteur inconnu'}</span>
              <span style={{ fontSize: '0.79062rem', color: '#b0a89e', marginLeft: 'auto' }}>{dateFormat(e.created_at)}</span>
            </div>
