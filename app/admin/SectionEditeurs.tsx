...
export default function SectionEditeurs() {
  ...
  const champ: React.CSSProperties = { width: '100%', fontSize: '12px', padding: '6px 9px', border: '1px solid #d6d0c4', borderRadius: '5px', background: '#fff', color: '#2a2520', outline: 'none', boxSizing: 'border-box' }
  const label: React.CSSProperties = { display: 'block', fontSize: '10px', fontWeight: 700, letterSpacing: '0.04em', color: '#8a8278', textTransform: 'uppercase', margin: '0 0 3px' }

  return (
    <div style={{ maxWidth: '760px', margin: '0 auto' }}>
      <p style={{ fontSize: '13px', color: '#6b6560', lineHeight: 1.55, margin: '0 0 20px' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #e4dfd8', borderRadius: '9px', padding: '16px 18px', marginBottom: '22px' }}>
        <p style={{ fontSize: '12px', fontWeight: 700, color: '#3d6b4f', margin: '0 0 12px' }}>{brouillon.id ? 'Modifier un éditeur' : 'Ajouter un éditeur'}</p>
        ...
        {statut === 'err' && <p style={{ fontSize: '11px', color: '#c0562a', margin: '0 0 8px' }}>{erreur}</p>}
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
          {brouillon.id && <button onClick={() => { setBrouillon(VIDE); setStatut('idle') }} style={{ fontSize: '11.5px', padding: '6px 13px', borderRadius: '5px', border: '1px solid #d6d0c4', background: '#fff', color: '#6b6560', cursor: 'pointer' }}>Annuler</button>}
          <button onClick={enregistrer} disabled={statut === 'envoi'} style={{ fontSize: '11.5px', padding: '6px 15px', borderRadius: '5px', border: 'none', background: '#3d6b4f', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>
      ...
      {aNormaliser.length > 0 && (
        <div style={{ marginBottom: '22px' }}>
          <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: '#9a5a2a', margin: '0 0 8px' }}>À normaliser ({aNormaliser.length})</p>
          <p style={{ fontSize: '11.5px', color: '#8a8278', margin: '0 0 10px' }}>Formes rencontrées dans le catalogue, pas encore répertoriées. Cliquez pour préremplir le formulaire.</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {aNormaliser.map(nom => (
              <button key={nom} onClick={() => { setBrouillon({ ...VIDE, nom_complet: nom }); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
                style={{ fontSize: '11.5px', padding: '4px 11px', borderRadius: '999px', border: '1px solid #e3cdb0', background: '#fdf6ef', color: '#9a5a2a', cursor: 'pointer' }}>
                {nom}
              </button>
            ))}
          </div>
      ...
      <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: '#8a8278', margin: '0 0 8px' }}>Répertoriés ({editeurs?.length ?? 0})</p>
      {editeurs === null ? (
        <p style={{ fontSize: '12px', color: '#9a958d', fontStyle: 'italic' }}>Chargement…</p>
      ) : editeurs.length === 0 ? (
        <p style={{ fontSize: '12px', color: '#9a958d', fontStyle: 'italic' }}>Aucun éditeur répertorié pour l'instant.</p>
      ) : (
          ...
          {editeurs.map(e => (
            <div key={e.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '10px', alignItems: 'center', background: '#fff', border: '1px solid #e4dfd8', borderRadius: '7px', padding: '9px 13px' }}>
              <div style={{ minWidth: 0 }}>
                <span style={{ fontFamily: 'var(--font-source-serif), Georgia, serif', fontSize: '13.5px', color: '#1e2e24' }}>{e.nom_complet}</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', fontSize: '10px', color: '#b0a89e', marginTop: '2px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
                <button onClick={() => editer(e)} style={{ fontSize: '11px', color: '#3d6b4f', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: 0 }}>Modifier</button>
                <button onClick={() => supprimer(e.id)} style={{ fontSize: '11px', color: '#c0562a', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: 0 }}>Supprimer</button>
              </div>
            </div>
          ))}
        </div>
