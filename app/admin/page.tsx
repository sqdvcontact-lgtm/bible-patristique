...
async function actionConnexion(formData: FormData) {
  ...
  if (mdp === MOT_DE_PASSE) {
    ...
    cookieStore.set(COOKIE_NOM, COOKIE_VALEUR, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 60 * 60 * 8, path: '/' })
  }
  redirect('/admin')
}
async function actionDeconnexion() { 'use server'; const c = await cookies(); c.delete(COOKIE_NOM); redirect('/admin') }
async function actionValiderCommentaire(id: number) { 'use server'; await supabaseAdmin.from('commentaires').update({ valide: true }).eq('id', id); redirect('/admin') }
async function actionSupprimerCommentaire(id: number) { 'use server'; await supabaseAdmin.from('commentaires').delete().eq('id', id); redirect('/admin') }
async function actionMarquerTraite(id: number) { 'use server'; await supabaseAdmin.from('signalements').update({ traite: true }).eq('id', id); redirect('/admin') }
async function actionSupprimerSignalement(id: number) { 'use server'; await supabaseAdmin.from('signalements').delete().eq('id', id); redirect('/admin') }
...
export default async function AdminPage() {
        ...
        <div style={{ background: '#fff', border: '1px solid #d6d0c4', borderRadius: '10px', padding: '36px 40px', width: '320px' }}>
          ...
          <form action={actionConnexion} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <input type="password" name="mdp" placeholder="Mot de passe" required autoFocus
              style={{ padding: '9px 12px', fontSize: '13px', border: '1px solid #d6d0c4', borderRadius: '6px', outline: 'none', color: '#2a2520', background: '#faf8f4' }} />
            <button type="submit" style={{ padding: '9px', fontSize: '13px', fontWeight: 500, background: '#3d6b4f', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Accéder</button>
          </form>
        </div>
      </main>
    )
  ...
  // Bibliothèque
  const { data: auteursData } = await supabaseAdmin.from('auteurs').select('id_auteur, nom, dates, siecle, oeuvres(id_oeuvre, titre, sous_titre, titre_original, trad_auteur, trad_date, editeur, collection, ville, date_publication, url_source, genre, langue)').order('siecle', { ascending: true, nullsFirst: false })
  const auteurs = (auteursData ?? []).filter((a: any) => a.oeuvres?.length > 0)
