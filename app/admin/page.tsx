...
export default async function AdminPage() {
  ...
  const vague1 = await Promise.all([
    ...
    supabaseAdmin.from('auteurs').select('id_auteur, nom, nom_original, titre, dates, date_naissance, date_mort, siecle, traditions, note_biographique, note_theologique, langue_principale, chronologie, anecdotes, influence, photo_position, photo_version, oeuvres!oeuvres_id_auteur_fkey(id_oeuvre, titre, titre_affichage, auteur_affichage, sous_titre, sous_titre_affichage, titre_original, titre_original_affichage, trad_auteur, trad_auteur_affichage, provenance_affichage, editeur, collection, ville, date_publication, date_composition, url_source, langue_originale, genre, genres, profondeur_sommaire, nb_signes, niveaux_sommaire, niveaux_corps, texte_sommaire, texte_corps, afficher_numeros, acces_public, motif_non_publication, commentaire_traduction, note_editoriale_complete, note_editoriale_complement, note_editoriale_titre)').order('siecle', { ascending: true, nullsFirst: false }),
  ...
  const commentairePriveParOeuvre = new Map(
    (commentairesPrivesOeuvres ?? []).map(c => [c.id_oeuvre, c])
  )
  const auteurs = (auteursData ?? []).map(a => ({
    ...a,
    oeuvres: (a.oeuvres ?? []).map(o => ({
      ...o,
      commentaire_prive: commentairePriveParOeuvre.get(o.id_oeuvre)?.commentaire ?? null,
      note_acces_public: commentairePriveParOeuvre.get(o.id_oeuvre)?.note_acces_public ?? null,
    })),
  }))
