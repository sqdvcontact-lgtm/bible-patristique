    ...
    return (
      <main style={{ minHeight: 'calc(100vh - 3.5rem)', background: 'var(--cs-fond)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ background: 'var(--cs-surface)', border: '1px solid var(--cs-bord)', borderRadius: '8px', padding: '36px 40px', width: '21.25rem', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '0.875rem', color: 'var(--cs-texte-second)', lineHeight: 1.6, marginBottom: '22px' }}>Cette page est réservée au compte administrateur.</p>
    ...
    return (
      <main style={{ minHeight: 'calc(100vh - 3.5rem)', background: 'var(--cs-fond)', padding: '48px 24px' }}>
        <div style={{ maxWidth: '36rem', margin: '0 auto', background: 'var(--cs-danger-fond)', border: '1px solid var(--cs-danger-bord)', borderRadius: '8px', padding: '20px 24px' }}>
          ...
          <p style={{ fontSize: '0.875rem', lineHeight: 1.6, color: 'var(--cs-texte)', margin: '0 0 6px' }}>Vos directives n’ont pas pu être lues, la page ne s’ouvre donc pas : écrire maintenant recouvrirait des décisions déjà prises.</p>
          <p style={{ fontFamily: 'ui-monospace, Consolas, monospace', fontSize: '0.71875rem', color: 'var(--cs-texte-second)', margin: 0 }}>{error.code ? `${error.code} — ` : ''}{error.message}</p>
        </div>
      </main>
    )
  }

  const directives = data ? lireDirectives(data.valeur) : DIRECTIVES_VIDES

  return (
    <RegistreNotes
      initialInstructions={directives.instructionsGenerales}
      initialReponses={directives.reponsesGenerales}
      initialMajLe={directives.majLe}
    />
  )
}
