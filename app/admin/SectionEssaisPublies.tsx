...
export default function SectionEssaisPublies({ essais: init, variante = 'publies' }: { essais: EssaiPublie[]; variante?: 'publies' | 'brouillons' }) {
  ...
  if (essais.length === 0) {
    return <p style={{ fontSize: '0.9375rem', color: 'var(--cs-texte-doux)', fontStyle: 'italic' }}>{estBrouillons ? "Aucun brouillon pour l'instant." : "Aucun essai publié pour l'instant."}</p>
  }
        ...
        <tbody>
          {essais.map(e => (
            <tr key={e.id} style={{ borderBottom: '1px solid var(--cs-fond-doux)' }}>
              <Td>
                <Link href={estBrouillons ? `/essais/${e.id}/modifier` : `/essais/${e.id}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.875rem', color: 'var(--cs-encre-fonce)', textDecoration: 'none', fontWeight: 600 }}>{e.titre} ↗</Link>
                {e.sous_titre && <p style={{ fontSize: '0.75rem', color: 'var(--cs-texte-doux)', fontStyle: 'italic', margin: '2px 0 0' }}>{e.sous_titre}</p>}
              ...
              <Td><span style={{ fontSize: '0.75rem', color: estBrouillons ? 'var(--cs-attente)' : 'var(--cs-vert)', fontWeight: 700 }}>{estBrouillons ? 'Brouillon' : 'Publié'}</span></Td>
              <Td align="right">
                {action[e.id] === 'ok' ? (
                  <span style={{ fontSize: '0.78125rem', color: 'var(--cs-vert)', fontWeight: 600 }}>✓ Renvoyé</span>
                ) : (
...
function Th({ children, align = 'left' }: { children: React.ReactNode; align?: 'left' | 'right' }) {
  return <th style={{ padding: '8px 10px', fontSize: '0.6875rem', color: 'var(--cs-texte-gris)', letterSpacing: '0.06em', textTransform: 'uppercase', textAlign: align, fontWeight: 700 }}>{children}</th>
}

function Td({ children, align = 'left' }: { children: React.ReactNode; align?: 'left' | 'right' }) {
  return <td style={{ padding: '9px 10px', verticalAlign: 'top', fontSize: '0.8125rem', color: '#4a4540', textAlign: align }}>{children}</td>
}

function DateInfo({ label, valeur, discret = false }: { label: string; valeur: string | null; discret?: boolean }) {
  if (!valeur) return <p style={{ fontSize: '0.75rem', color: 'var(--cs-texte-faible)', margin: '0 0 2px' }}>{label} : —</p>
  return (
    <p style={{ fontSize: '0.75rem', color: discret ? 'var(--cs-texte-faible)' : 'var(--cs-texte-second)', margin: '0 0 2px', whiteSpace: 'nowrap' }}>
...
function Info({ label, valeur, alerte = false }: { label: string; valeur: number; alerte?: boolean }) {
  return (
    <span style={{ display: 'block', fontSize: '0.75rem', color: alerte ? 'var(--cs-danger)' : 'var(--cs-texte-doux)', whiteSpace: 'nowrap', marginBottom: '2px' }}>
...
function petitBouton(couleur: string, bordure: string): React.CSSProperties {
  ...
  return { display: 'inline-block', boxSizing: 'border-box', minWidth: '82px', textAlign: 'center', fontSize: '0.75rem', padding: '4px 8px', borderRadius: '4px', border: `1px solid ${bordure}`, background: 'var(--cs-surface)', color: couleur, cursor: 'pointer', textDecoration: 'none', fontWeight: 600 }
}
