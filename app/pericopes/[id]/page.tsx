            ...
            <div key={o.id} lang={langAttr}>
              {occurrences.length > 1 && (
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '9px', marginBottom: '7px', paddingBottom: '4px', borderBottom: `1px solid ${SEP}` }}>
                  ...
                  {o.est_principale && <span style={{ fontFamily: SANS, fontSize: '0.625rem', letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--cs-vert)' }}>principale</span>}
                  {o.fiabilite && <span style={{ fontFamily: SANS, fontSize: '0.6875rem', color: 'var(--cs-texte-gris)', fontStyle: 'italic' }}>{o.fiabilite}</span>}
                </div>
              )}
              {texteLoading ? (
            ...
            <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '2px' }}>
              {occurrences.map(o => (
                <li key={o.id} style={{ fontFamily: SANS, fontSize: '0.75rem', color: 'var(--cs-texte)', display: 'flex', alignItems: 'baseline', gap: '7px' }}>
                  ...
                  {o.est_principale && <span style={{ fontFamily: SANS, fontSize: '0.625rem', letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--cs-vert)' }}>princ.</span>}
                </li>
              ))}
            </ul>
          </div>
                ...
                <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {n.liens.map((l, i) => (
                    <li key={`${l.source.code}-${i}`} style={{ borderLeft: `2px solid ${SEP}`, paddingLeft: '8px' }}>
                      ...
                      {l.referenceInterne && (
                        <p style={{ fontFamily: SANS, fontSize: '0.6875rem', color: 'var(--cs-texte-gris)', margin: '3px 0 0', lineHeight: 1.35 }}>
                      ...
                      {l.note && (
                        <p style={{ fontFamily: SANS, fontSize: '0.6875rem', color: 'var(--cs-mention)', margin: '3px 0 0', lineHeight: 1.4, textAlign: 'justify', hyphens: 'auto' } as React.CSSProperties}>
...
function LigneInfo({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '5.5rem 1fr', columnGap: '10px', alignItems: 'baseline' }}>
      <dt style={{ fontFamily: SANS, fontSize: '0.6875rem', fontWeight: 600, letterSpacing: '0.03em', color: 'var(--cs-texte-gris)' }}>{label}</dt>
