...
// par livre biblique (ordre canonique). Lecture avec la session normale de l'utilisateur.
...
export default function CataloguePericopesPage() {
  const mobile = useEstMobile(900)
  const [items, setItems] = useState<PericopeCatalogueItem[]>([])
  ...
  useEffect(() => {
    ...
    chargerCataloguePericopes(ctrl.signal)
      .then(data => { if (!annule) { setItems(data); setEtat('ok') } })
      .catch(err => { if (annule || err?.name === 'AbortError') return; setEtat('erreur') })
        ...
        <section style={{ flex: 1, minWidth: 0, padding: mobile ? '16px 14px 56px' : '16px 32px 64px' }}>
          <div style={{ maxWidth: '48rem', margin: '0 auto' }}>
            ...
            {etat === 'chargement' ? (
              ...
              <p style={{ fontSize: '0.85rem', color: '#a2564a', textAlign: 'center', paddingTop: '20px' }}>Les péricopes n’ont pas pu être chargées.</p>
            ) : groupes.length === 0 ? (
