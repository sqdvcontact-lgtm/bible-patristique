...
describe('la taille du texte biblique, en trois crans', () => {
  ...
  it('la feuille déclare les trois crans : 15/1,50, 16/1,55, 18/1,60', () => {
    const feuille = readFileSync(join(process.cwd(), 'app/globals.css'), 'utf8')
    expect(feuille).toContain('--cs-lecture-corps: 1rem;')
    expect(feuille).toContain('--cs-lecture-interligne: 1.55;')
    expect(feuille).toMatch(/:root\[data-corps="petit"\] \{\s*--cs-lecture-corps: 0\.9375rem;\s*--cs-lecture-interligne: 1\.5;/)
    expect(feuille).toMatch(/:root\[data-corps="grand"\] \{\s*--cs-lecture-corps: 1\.125rem;\s*--cs-lecture-interligne: 1\.6;/)
  })
})
