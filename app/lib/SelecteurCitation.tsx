...
// `complet` : citation « pleine » — le texte cité (guillemets) va dans un bloc Citation,
// et la référence complète dans une note. `texte`/`ref` portent ces deux morceaux.
type Choix = { label: string; type: 'verset' | 'segment'; id: string; href?: string; complet?: boolean; texte?: string; ref?: string }
...
// Le seul texte cité, entre guillemets, SANS la référence (elle ira dans la note).
function citationTexteSeul(texte: string): string {
  return `« ${texteSansPointFinal(texteSansEnrichissement(texte))} »`
}
          ...
          {versets.map(v => {
            ...
            <div key={v.id_verset}
              ...
              <BoutonsChoix
                ...
                onComplet={() => onChoisir({ label: citationBibliqueComplete(v.texte, ref), type: 'verset', id: v.id_verset, href: hrefVerset(livre, chapitre, v.verset), complet: true, texte: citationTexteSeul(v.texte), ref })}
              />
            </div>
          )})}
        </div>
...
// Référence combinée d'une SÉLECTION de segments : « Auteur, Œuvre, [niveaux], §a–§b ».
function refSelection(auteurNom: string, oeuvreTitre: string, segs: SegPatr[]): string {
  const nums = segs.map(s => s.segment_numero).sort((a, b) => a - b)
  const contigu = nums.length > 1 && nums.every((n, i) => i === 0 || n === nums[i - 1] + 1)
  const labelNums = nums.length === 1 ? `§${nums[0]}` : contigu ? `§${nums[0]}–§${nums[nums.length - 1]}` : `§${nums.join(', ')}`
  const niv = [segs[0]?.ref_niv1, segs[0]?.ref_niv2, segs[0]?.ref_niv3, segs[0]?.ref_niv4, segs[0]?.ref_niv5].filter(Boolean).join(', ')
  return [auteurNom, oeuvreTitre, niv, labelNums].filter(Boolean).join(', ')
}
// Texte cité d'une sélection : les segments réunis entre guillemets, sans référence.
function texteSelection(segs: SegPatr[]): string {
  const inner = segs.map(s => convertirGuillemetsInternes(texteSansEnrichissement(s.segment_texte).trim())).join(' ')
  return `« ${inner} »`
}
...
function ParcourirPatristique({ onChoisir }: { onChoisir: (c: Choix) => void }) {
  const [oeuvres, setOeuvres] = useState<{ id_oeuvre: string; titre: string; auteurNom: string }[] | null>(null)
  const [oeuvre, setOeuvre] = useState<{ id_oeuvre: string; titre: string; auteurNom: string } | null>(null)
  ...
  // Toutes les œuvres publiées, avec le nom de l'auteur, pour la recherche par nom.
  useEffect(() => {
    supabase.from('oeuvres').select('id_oeuvre, titre, note, auteurs(nom)').order('titre').then(({ data }) => {
      const liste = ((data ?? []) as any[]).filter(estOeuvrePubliee)
        .map(o => ({ id_oeuvre: o.id_oeuvre, titre: o.titre, auteurNom: o.auteurs?.nom ?? '' }))
      setOeuvres(liste)
    })
  }, [])

  const choisirOeuvre = async (o: { id_oeuvre: string; titre: string; auteurNom: string }) => {
    ...
    return (
      <div>
        <input value={rechercheOeuvre} onChange={e => setRechercheOeuvre(e.target.value)} autoFocus
          ...
          style={{ width: '100%', boxSizing: 'border-box', fontSize: '13px', padding: '9px 12px', borderRadius: '6px', border: '1px solid #d6d0c4', background: '#faf8f4', color: '#2a2520', marginBottom: '12px', outline: 'none' }} />
        {oeuvres === null ? (
          <p style={{ fontSize: '12px', color: '#9a958d', fontStyle: 'italic' }}>Chargement du catalogue…</p>
        ) : resultats.length === 0 ? (
          <p style={{ fontSize: '12px', color: '#9a958d', fontStyle: 'italic' }}>{q ? 'Aucune œuvre ne correspond.' : 'Aucune œuvre disponible.'}</p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {resultats.slice(0, 60).map(o => (
              <button key={o.id_oeuvre} onClick={() => choisirOeuvre(o)}
                style={{ textAlign: 'left', fontSize: '13px', padding: '8px 11px', borderRadius: '5px', border: '1px solid #ede9e2', background: '#fff', color: '#2a3d30', cursor: 'pointer' }}>
                {o.titre}
  ...
  const insererSelection = (complet: boolean) => {
    if (segsSelectionnes.length === 0) return
    const ref = refSelection(oeuvre.auteurNom, oeuvre.titre, segsSelectionnes)
    const premier = segsSelectionnes[0]
    const href = hrefSegment(oeuvre.id_oeuvre, premier.id)
    if (complet) {
      const texte = texteSelection(segsSelectionnes)
      onChoisir({ label: texte, type: 'segment', id: String(premier.id), href, complet: true, texte, ref })
    } else {
      onChoisir({ label: ref, type: 'segment', id: String(premier.id), href })
    }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '8px' }}>
        <button onClick={() => { setOeuvre(null); setSelection(new Set()) }} style={{ fontSize: '11px', color: '#3d6b4f', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>← Œuvres</button>
        <span style={{ fontSize: '12px', color: '#2a3d30', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{oeuvre.titre}</span>
        {oeuvre.auteurNom && <span style={{ fontSize: '11px', color: '#9a958d', fontStyle: 'italic' }}>{oeuvre.auteurNom}</span>}
      </div>
      <input value={rechercheSeg} onChange={e => setRechercheSeg(e.target.value)}
        placeholder="Chercher un passage dans cette œuvre…"
        style={{ width: '100%', boxSizing: 'border-box', fontSize: '12px', padding: '7px 10px', borderRadius: '5px', border: '1px solid #d6d0c4', background: '#faf8f4', color: '#2a2520', marginBottom: '10px', outline: 'none', flexShrink: 0 }} />
            ...
            {segmentsFiltres.map(s => {
              ...
              return (
                <button key={s.id} type="button" onClick={() => toggle(s.id)}
                  ...
                  <span style={{ flexShrink: 0, width: '14px', height: '14px', marginTop: '1px', borderRadius: '3px', border: `1px solid ${sel ? '#3d6b4f' : '#c8c0b4'}`, background: sel ? '#3d6b4f' : '#fff', color: '#fff', fontSize: '10px', lineHeight: '13px', textAlign: 'center' }}>{sel ? '✓' : ''}</span>
                  <span style={{ fontSize: '11px', fontWeight: 700, color: '#3d6b4f', flexShrink: 0 }}>§{s.segment_numero}</span>
                  <span style={{ fontSize: '12.5px', color: '#2a2520', lineHeight: 1.5, flex: 1 }}>{t.slice(0, 200) + (t.length > 200 ? '…' : '')}</span>
                </button>
              )
            })}
          </div>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: '10px', paddingTop: '10px', marginTop: '4px', borderTop: '1px solid #ede9e2' }}>
        <span style={{ fontSize: '11.5px', color: segsSelectionnes.length ? '#3d6b4f' : '#9a958d' }}>
        ...
        <span style={{ marginLeft: 'auto', display: 'flex', gap: '6px' }}>
          <button type="button" disabled={!segsSelectionnes.length} onClick={() => insererSelection(false)}
            style={{ ...petitChoixStyle, opacity: segsSelectionnes.length ? 1 : 0.45, cursor: segsSelectionnes.length ? 'pointer' : 'default' }}>Référence abrégée</button>
          <button type="button" disabled={!segsSelectionnes.length} onClick={() => insererSelection(true)}
            style={{ ...petitChoixStyle, background: segsSelectionnes.length ? '#3d6b4f' : '#e4dfd8', color: segsSelectionnes.length ? '#fff' : '#9a958d', border: 'none', opacity: 1, cursor: segsSelectionnes.length ? 'pointer' : 'default' }}>Citation complète</button>
        </span>
      </div>
    </div>
  )
...
function MesCitations({ source, onChoisir }: { source: 'bible' | 'patristique'; onChoisir: (c: Choix) => void }) {
  ...
  const choisir = async (it: any, complet = false) => {
    if (source === 'bible') {
      ...
      if (data) onChoisir({ label: complet ? citationBibliqueComplete(texte, ref) : ref, type: 'verset', id: data.id_verset, href: hrefVerset(livreCode, it.ref_chapitre, it.ref_verset), complet, texte: complet ? citationTexteSeul(texte) : undefined, ref })
    } else {
      ...
      if (data) {
        ...
        onChoisir({ label: complet ? citationPatristiqueComplete(it.texte ?? '') : ref, type: 'segment', id: String(data.id), href: hrefSegment(it.id_oeuvre, data.id), complet, texte: complet ? citationPatristiqueComplete(it.texte ?? '') : undefined, ref })
      }
    }
  }
