...
export function fournisseurClaudeLocal(env = {}) {
  ...
  // (triage — classement de planches pour le tri IA — retiré le 2026-08-10 avec la fonction de tri.)

  // RELECTURE d'une PAGE océrisée (contrôle) : le CLI lit l'image de la page (image_path) et renvoie les
  // corrections ligne à ligne. La consigne (OCR ligne par ligne) est fournie par l'appelant (serveur).
  // Modèle de CONTRÔLE (Sonnet) par défaut ; jamais le modèle léger (diagnostic/Haiku). Ne lève jamais.
  async function relecturePage(charge = {}, { timeoutMs } = {}) {
    const { image_path, consigne = '', cwd = undefined } = charge
    const ab = (erreur) => ({ type: 'relecture_page', statut: 'candidat', abstention: true, corrections: [], erreur, fournisseur: 'claude-local' })
    if (!image_path) return ab('image de page absente')
    if (!consigne) return ab('consigne absente')
    const modeleCorr = modele.controle || modele.vision || null
    const invite = SYSTEME + '\n\n' + consigne + '\n\nL’image de la page est le fichier :\n' + String(image_path) +
      '\nUtilise l’outil Read sur ce fichier pour l’examiner, puis réponds UNIQUEMENT par l’objet JSON demandé, sans aucun texte autour.'
    const r = await lancer(bin, argvClaude({ modele: modeleCorr, addDir: cwd }), invite, { cwd, timeoutMs })
    const env0 = extraireJson(r.out)
    if (env0 && env0.is_error) return ab(String(env0.result || 'erreur CLI'))
    if (!env0) return ab(r.ok ? 'sortie CLI illisible' : (r.erreur || ('CLI code ' + r.code + (r.err ? ' — ' + String(r.err).slice(0, 200) : ''))))
    const data = extraireJson(env0.result)
    if (!data || !Array.isArray(data.corrections)) return ab('JSON de relecture absent de la réponse')
    return { ...data, type: 'relecture_page', statut: 'candidat', abstention: !!data.abstention, corrections: data.corrections, fournisseur: 'claude-local', modele: modeleCorr || 'défaut (abonnement)' }
  }

  // ANCRAGE DES NOTES (§13) : le CLI lit l'image de la page et dit, pour chaque glose de marge, quelle
  // ligne du corps elle annote. La consigne est fournie par l'appelant (serveur). Modèle de CONTRÔLE.
  // Ne renvoie JAMAIS de numérotation : elle est posée par l'outil (§13.2). Ne lève jamais.
  async function ancrageNotes(charge = {}, { timeoutMs } = {}) {
    const { image_path, consigne = '', cwd = undefined } = charge
    const ab = (erreur) => ({ type: 'ancrage_notes', statut: 'candidat', abstention: true, ancrages: [], erreur, fournisseur: 'claude-local' })
    if (!image_path) return ab('image de page absente')
    if (!consigne) return ab('consigne absente')
    const modeleCorr = modele.controle || modele.vision || null
    const invite = SYSTEME + '\n\n' + consigne + '\n\nL’image de la page est le fichier :\n' + String(image_path) +
    ...
    const data = extraireJson(env0.result)
    if (!data || !Array.isArray(data.ancrages)) return ab('JSON d’ancrage absent de la réponse')
