...
const clean = (s) =>
  ...
  overlap = (s, w) => {
    const a = words(s),
      ed = [w?.TR0001, w?.TR0003, w?.TR0004].filter(Boolean).map(words);
    return Math.max(
      0,
      ...ed.map(
        (e) => [...e].filter((x) => a.has(x)).length / Math.max(1, e.size),
      ),
    );
  };
const rows = [
...
writeFileSync(`${R}/ps-q41-80-existing-review.tsv`, rows.join("\n") + "\n");
...
writeFileSync(
  `${R}/ps-q41-80-omission-candidates.json`,
