...
const PREMIER = 2561
const DERNIER = 2635
const SORTIE = 'scripts/heptateuque/audit-reprise/liens-deuteronome-q41-q50-lecture.json'
const CIBLES = [
  'DEU.24.10', 'DEU.24.11', 'DEU.24.12', 'DEU.24.13', 'DEU.24.16', 'DEU.24.17', 'DEU.24.19', 'DEU.24.20', 'DEU.24.21',
  'DEU.25.1', 'DEU.25.2', 'DEU.25.3', 'DEU.25.5', 'DEU.25.6', 'DEU.26.13', 'DEU.26.14', 'DEU.28.14', 'DEU.28.69',
  'DEU.29.1', 'DEU.29.2', 'DEU.29.3', 'DEU.29.4',
  'EZK.18.17', 'EXO.20.5', 'ROM.5.12', 'AMO.1.3', '1TI.5.4', '1TI.5.5', '1TI.5.6',
  'RUT.4.5', 'RUT.4.10', 'RUT.4.17', 'MAT.1.6', 'MAT.1.12', 'MAT.1.13', 'MAT.1.15', 'MAT.1.16',
  'LUK.3.23', 'LUK.3.24', 'LUK.3.27', 'LUK.3.31', 'PRO.4.27', 'PSA.1.6',
  'PSA.143.8', 'PSA.143.9', 'PSA.143.10', 'PSA.143.11', 'PSA.143.12', 'PSA.143.13', 'PSA.143.14', 'PSA.143.15',
  'GEN.17.4', 'GEN.9.9',
]
...
writeFileSync(SORTIE, `${JSON.stringify({ genere_le: new Date().toISOString(), oeuvre: OEUVRE, ref_niv1: REF_NIV1, lot: 'Deutéronome XLI-L', bornes: [PREMIER, DERNIER], relus_globaux: relusGlobaux, empreinte, liens_existants: liens, voisins, segments, temoins, feedback_liens_protocole: memoire.valeur }, null, 2)}\n`, 'utf8')
console.log(JSON.stringify({ sortie: SORTIE, segments: segments.length, bornes: [PREMIER, DERNIER], questions: questionsLive, liens_existants: liens.length, deja_relus: segments.filter((s) => s.liens_revus_le || s.liens_revus_par).length, relus_globaux: relusGlobaux, empreinte, pages: [...new Set(segments.map((s) => s.page))], voisins: voisins.filter((s) => s.segment_numero < PREMIER || s.segment_numero > DERNIER).map(({ segment_numero, ref_niv2, page, liens_revus_le }) => ({ segment_numero, ref_niv2, page, liens_revus_le })) }, null, 2))
