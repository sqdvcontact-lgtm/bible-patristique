...
const plan = JSON.parse(readFileSync(`${ROOT}/PRIMA-SECUNDAE-Q1-40-DOSSIER-STRICT.json`, 'utf8'));
const APPLY = process.argv.includes('--apply');
const questions = Array.from({ length: 40 }, (_, index) => `Question ${index + 1}`);
...
const snapshot = (label, payload) => {
  ...
  const name = `PRIMA-SECUNDAE-Q1-40-${label}-${stamp}.json`;
...
async function live() {
  ...
  for (const question of questions) for (let from = 0; ; from += 100) {
    const page = await must(sb.from('segments').select('*').eq('id_oeuvre', 'A0013O0002')
      .eq('ref_niv1', 'Prima Secundae').eq('ref_niv2', question).order('segment_numero').range(from, from + 99), `${question}:${from}`);
    segments.push(...page);
...
const isDesired = (state) => state.segments.length === 2143 && state.links.length === 242
  && stable(state.links.map(tuple).sort()) === stable(desired.map(tuple).sort());

if (raw.segments.length !== 2143 || raw.links.length !== 219 || plan.decisions.length !== 248
  || plan.summary.liens_finaux_proposes !== 242 || plan.summary.suppressions !== 6
  || plan.summary.ajouts !== 29 || plan.summary.controle_stratifie < 30
...
const finalJson = literal(JSON.stringify(desired));
...
if n<>2143 then raise exception 'préétat : nombre de segments %, attendu 2143',n; end if;
perform 1 from liens_bibliques where segment_id=any(scope_ids) for update;
get diagnostics n = row_count;
if n<>219 then raise exception 'préétat verrouillé : nombre de liens %, attendu 219',n; end if;
with expected as (
...
if n<>219 then raise exception 'préétat contenu liens %, attendu 219',n; end if;
delete from liens_bibliques where segment_id=any(scope_ids);
get diagnostics n = row_count;
if n<>219 then raise exception 'suppression de périmètre %, attendu 219',n; end if;
...
get diagnostics n = row_count;
if n<>242 then raise exception 'insertion finale %, attendu 242',n; end if;
select count(*) into n from segments where id=any(scope_ids) and liens_revus_le is not null and liens_revus_par='IA-lecture';
if n<>2143 then raise exception 'marqueurs segments %/2143',n; end if;
select count(*) into n from liens_bibliques where segment_id=any(scope_ids); if n<>242 then raise exception 'liens finaux %',n; end if;
...
if (!APPLY) {
  console.log(JSON.stringify({
    ...
    before, prestate_exact: exact, segments: 2143,
    bulk_strategy: ['pré-check live exact hors transaction', 'verrouillage relationnel du scope et garde des 219 liens',
      'DELETE ensembliste du périmètre', 'INSERT JSON recordset de 242 lignes',
      'assertion sans UPDATE des 2 143 segments déjà marqués IA-lecture'],
    deletes_scope: 219, inserts: 242, final: 242, transaction_bytes: Buffer.byteLength(transaction, 'utf8'),
    checks: ['préétat exact en JavaScript et dans la transaction', 'pagination complète', 'cibles exclusives', 'doublons',
      'cibles canoniques, chapitre et versets_v2 vivants', 'métadonnées vérifiées', '40 contrôles dont 20 difficiles', 'snapshots SHA-256'],
  }, null, 2));
  process.exit(0);
}
if (done) { console.log(JSON.stringify({ applied: false, noop: true, before, segments: 2143, links: 242 }, null, 2)); process.exit(0); }
...
console.log(JSON.stringify({ applied: true, before, after, segments: 2143, links: 242 }, null, 2));
