// Enrichissement du livre VI de la Cité de Dieu par lecture (Corpus Scriptura).
// Livre entièrement consacré à réfuter Varron et Sénèque : aucune citation biblique
// nouvelle (le seul verset réel, Ps 39,5, était déjà posé). Trois faux amis à retirer,
// tous des phrases classiques prises pour l'Écriture.
...
const MOTIF = 'Lecture Corpus Scriptura — Cité de Dieu, livre VI';

const AJOUTS = [];
const RETRAITS = [
  [1971, 'PRO.13.8'],   // mot d'esprit prêté aux nymphes (jeu vita/vitis), pas Proverbes
  [1990, '1TI.6.7'],    // segment sur Cicéron louant Varron, aucun rapport avec 1 Tm 6,7
  [2190, 'LEV.19.37'],  // « le sage observera toutes ces pratiques » = Sénèque, pas Lévitique
];
...
async function main() {
  const segs = (await pageAll('id, segment_numero, ref_niv1', 'segments', { id_oeuvre: OEUVRE })).filter((s) => s.ref_niv1 === 'Livre VI');
