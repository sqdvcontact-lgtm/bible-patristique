...
const plan = JSON.parse(readFileSync(`${ROOT}/SS-Q88-93-DOSSIER-STRICT.json`, 'utf8'));
...
function snapshot(label, payload) {
  ...
  const name = `Q88-93-${label}-${stamp}.json`;
...
if (raw.segments.length !== 315 || raw.links.length !== 91 || plan.decisions.length !== 91 || plan.insertions.length !== 22 || plan.summary.liens_finaux_proposes !== 112) throw new Error('Dossier incomplet.');
...
for (const item of [...plan.decisions, ...plan.insertions]) {
  if (!item.ancre_locale_exacte || !item.temoins_versets_lecture?.length) throw new Error('Preuve locale ou témoin absent.');
}

const beforeLive = await readLive();
const beforePath = snapshot('live-before', beforeLive);
if (stable(beforeLive.segments) !== stable(raw.segments) || stable(beforeLive.links) !== stable(raw.links)) throw new Error(`Préétat différent du snapshot audité : ${beforePath}`);
if (beforeLive.segments.some((segment) => segment.liens_revus_le || segment.liens_revus_par)) throw new Error('Au moins un segment est déjà marqué comme relu.');
if (!APPLY) {
  console.log(JSON.stringify({
    ready: true,
    applied: false,
    database_mode: 'lecture seule',
    guard: '--apply requis ; option volontairement non exécutée',
    before: beforePath,
    segments: 315,
    updates: 90,
    deletes: 1,
...
for (const decision of plan.decisions) {
  ...
  if (decision.decision === 'supprimer') {
    statements.push(`delete from liens_bibliques where ${exactPredicate(old)};if not found then raise exception 'delete ${old.id}: prestate mismatch';end if;n_del:=n_del+1;`);
    continue;
  }
...
if n_up<>90 or n_del<>1 or n_ins<>22 or n_mark<>315 then raise exception 'counts %,%,%,%',n_up,n_del,n_ins,n_mark;end if;
select count(*) into n from liens_bibliques where segment_id in(${segmentIds});if n<>112 then raise exception 'final count %',n;end if;
...
if (afterLive.segments.length !== 315 || afterLive.links.length !== 112) throw new Error(`Postétat inattendu : ${afterPath}`);
console.log(JSON.stringify({ applied: true, before: beforePath, after: afterPath, segments: 315, links: 112 }, null, 2));
