...
const questions = Array.from({ length: 6 }, (_, index) => `Question ${94 + index}`);
const segments = [];
const pagination = [];
for (let from = 0; ; from += 100) {
  ...
  pagination.push({ objet: 'segments', range: [from, from + 99], lignes: data.length, segment_numero: data.length ? [data[0].segment_numero, data.at(-1).segment_numero] : null });
  segments.push(...data);
...
const targetIds = [...new Set(links.map((link) => link.canon_id).filter(Boolean))];
const witnesses = [];
...
writeFileSync('tmp/somme-liens-audit-2026-07-29/ss-q94-99-raw.json', `${JSON.stringify(output, null, 2)}\n`);
console.log(JSON.stringify({ segments: segments.length, range: [segments[0]?.segment_numero, segments.at(-1)?.segment_numero], links: links.length,
  types: Object.fromEntries([1, 2, 3, 4].map((type) => [type, links.filter((link) => link.type === type).length])), pagination }, null, 2));
