// Enrichissement du livre IX de la Cité de Dieu par lecture (Corpus Scriptura).
// Livre sur les démons médiateurs et les passions : très philosophique, mais culmine
// sur le vrai Médiateur (1 Tm 2,5) et les « dieux des dieux ». Un faux ami : une phrase
// de Salluste (âme/corps) prise pour Luc 8,55.
...
const MOTIF = 'Lecture Corpus Scriptura — Cité de Dieu, livre IX';

const AJOUTS = [
  [3242, '1CO.8.1', 1],    // « la science enfle, mais la charité édifie »
  [3255, 'MAT.4.11', 2],   // après la tentation, les anges se mirent à le servir
  [3267, 'PSA.49.1', 1],   // « le Dieu des dieux, le Seigneur a parlé »
  [3267, 'PSA.135.2', 1],  // « Rendez gloire au Dieu des dieux »
];
const RETRAITS = [
  [3122, 'LUK.8.55'],      // « à l'âme le commandement, au corps l'obéissance » = Salluste, pas Luc
];
...
async function main() {
  const segs = (await pageAll('id, segment_numero, ref_niv1', 'segments', { id_oeuvre: OEUVRE })).filter((s) => s.ref_niv1 === 'Livre IX');
