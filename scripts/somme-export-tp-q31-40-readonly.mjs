...
const questions = Array.from({ length: 10 }, (_, index) => `Question ${index + 31}`);
const segments = [], pagination = [];
for (let from = 0; ; from += 100) {
  const { data, error } = await sb.from('segments').select('*').eq('id_oeuvre', 'A0013O0002')
    .eq('ref_niv1', 'Tertia Pars').in('ref_niv2', questions).order('segment_numero').range(from, from + 99);
...
writeFileSync('tmp/somme-liens-audit-2026-07-29/tp-q31-40-raw.json', `${JSON.stringify(output, null, 2)}\n`);
console.log(JSON.stringify({
  segments: segments.length, range: [segments[0]?.segment_numero, segments.at(-1)?.segment_numero], links: links.length,
