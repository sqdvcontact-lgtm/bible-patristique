...
for (const arg of process.argv.slice(2)){
  ...
  console.log(`\n${'═'.repeat(LARG)}\nSIRACIDE ${c}   —   Sacy : ${sac.filter(v=>v.v>0).length} versets` +
              `${sac.some(v=>v.v===0)?' + suscription':''}   |   référent : ${ref.length} créneaux`)
  console.log('─'.repeat(LARG))
  const wrap = (t, tag, num) => {
    const w = LARG - 12, mots = t.split(/\s+/); let l = '', out = []
    for (const m of mots){ if ((l+' '+m).trim().length > w){ out.push(l); l = m } else l = (l?l+' ':'')+m }
    if (l) out.push(l)
    return out.map((x,j) => `  ${j?' ':tag} ${j?'   ':String(num).padStart(3)} │ ${x}`).join('\n')
  }
  const n = Math.max(sac.length, ref.length)
  for (let i = 0; i < n; i++){
    const a = sac[i], b = ref[i]
    console.log(a ? wrap(a.texte.replace(/<\/?i>/g,''), 'S', a.v) : '  S     │')
    console.log(b ? wrap(b.texte||'—', 'R', b.canon_id.split('.')[2]) : '  R     │')
    console.log('')
  }
}
