// Enrichissement du livre VII de la Cité de Dieu par lecture (Corpus Scriptura).
// Livre consacré aux « dieux choisis » de Varron : très païen, peu d'Écriture. Les faux
// amis abondent sur les vers de Valérius Soranus (« Jupiter père et mère ») et les phrases
// de Varron, mal appariés par le matcheur.
...
const MOTIF = 'Lecture Corpus Scriptura — Cité de Dieu, livre VII';

const AJOUTS = [
  [2320, 'JHN.10.9', 1],  // « la vérité qui a dit : Je suis la porte » (face aux portes de Janus)
];
const RETRAITS = [
  [2338, 'TOB.10.12'],    // vers de Valérius Soranus « Jupiter père et mère », pas Tobie
  [2352, 'LUK.2.33'],     // idem, apparié sur « père et mère »
  [2374, 'LUK.2.33'],     // idem « Jupiter père et mère de toutes choses »
  [2500, 'ACT.17.20'],    // phrase de Varron « une chose peut être une et multiple », pas Actes
];
...
async function main() {
  const segs = (await pageAll('id, segment_numero, ref_niv1', 'segments', { id_oeuvre: OEUVRE })).filter((s) => s.ref_niv1 === 'Livre VII');
