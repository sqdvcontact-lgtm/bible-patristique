// Contrôle sémantique des liens des Confessions d'Augustin (A0010O0001).
// Compare le texte du segment au texte biblique français réel (versets_v2, toutes
// traductions, meilleur recouvrement des mots pleins). Une citation (type 1) doit
// fortement recouvrir le verset ; un recouvrement quasi nul sur un type 1 = faux ami
// probable. LECTURE SEULE : distribution + fichier de scores pour tri.
...
async function main() {
  ...
  console.log(`segments Confessions : ${segs.length}`);
  ...
  console.log(`liens résolus au canon : ${liens.length}`);
  ...
  console.log(`\nsans texte biblique comparable : ${rows.length - avec.length}`);
  ...
  writeFileSync('scripts/_confessions-scores.json', JSON.stringify(rows), 'utf8');
  console.log('scores → scripts/_confessions-scores.json');
}
