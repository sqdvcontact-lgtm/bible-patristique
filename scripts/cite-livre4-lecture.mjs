// Enrichissement du livre IV de la Cité de Dieu par lecture (Corpus Scriptura).
// Livre plus doctrinal (critique du polythéisme) : citations pauliniennes et geste
// de l'Exode. Un faux ami : un vers de Virgile (Géorgiques) pris pour Ps 68,35.
...
const MOTIF = 'Lecture Corpus Scriptura — Cité de Dieu, livre IV';

const AJOUTS = [
  [1192, '2PE.2.19', 1],  // « chacun est esclave de celui qui l'a vaincu »
  [1325, '1TI.6.15', 2],  // « le Roi des rois et le Seigneur des seigneurs »
  [1331, '1TI.1.17', 2],  // « le véritable Roi des siècles »
  [1357, 'ROM.1.17', 1],  // « le juste vit de la foi »
  [1482, 'ROM.1.25', 1],  // « adorateurs de la créature plutôt que du Créateur »
  [1523, 'EXO.1.7', 2],   // le peuple se multiplie en Égypte
  [1526, 'EXO.14.28', 2], // la mer ramène ses flots sur les Égyptiens
  [1527, 'EXO.16.14', 2], // la manne du ciel
  [1527, 'EXO.17.6', 2],  // le rocher frappé par Moïse
];
const RETRAITS = [
  [1274, 'PSA.68.35'],    // « Dieu circule à travers toutes les terres… » = Virgile (Géorgiques)
];
...
async function main() {
  const segs = (await pageAll('id, segment_numero, ref_niv1', 'segments', { id_oeuvre: OEUVRE })).filter((s) => s.ref_niv1 === 'Livre IV');
