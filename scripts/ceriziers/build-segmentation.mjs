...
async function main() {
  ...
  for (const pending of anchorsPending.sort((a, b) => a.note_number - b.note_number)) {
    ...
    notes.push({
      ...
      metadata: {
        ...
        semantic_reading: sourceNote.kind === 'anomalie_titre_imprime' ? pending.segment.ref_niv2 : null,
  ...
  if (verseSegments.length !== 1214) throw new Error(`1 214 vers attendus, ${verseSegments.length} obtenus`);
