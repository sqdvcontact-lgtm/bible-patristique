// Enrichissement du livre III de la Cité de Dieu par lecture (Corpus Scriptura).
// Livre d'histoire romaine (Troie, les rois, guerres puniques et civiles) — presque
// aucune Écriture ; l'apport est surtout un faux ami à retirer.
...
const MOTIF = 'Lecture Corpus Scriptura — Cité de Dieu, livre III';

const AJOUTS = [
  [809, 'GEN.6.4', 2],    // « les anges se sont unis aux filles des hommes » et les géants
  [940, 'MAT.27.45', 2],  // les ténèbres sur le soleil à la crucifixion
];
const RETRAITS = [
  [947, 'JOB.3.5'],       // « ténèbres qui couvrirent la mort de Romulus » = Cicéron (Hortensius), PAS Job
];
...
async function main() {
  const segs = (await pageAll('id, segment_numero, ref_niv1', 'segments', { id_oeuvre: OEUVRE })).filter((s) => s.ref_niv1 === 'Livre III');
