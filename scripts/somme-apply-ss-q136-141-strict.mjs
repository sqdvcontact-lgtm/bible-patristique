/** Application atomique de l'audit IIa-IIae Q136-141. Garde active sans --apply. */
...
const plan = JSON.parse(readFileSync(`${ROOT}/Q136-141-AUDIT-EXHAUSTIF.json`, 'utf8'));
const APPLY = process.argv.includes('--apply');
const OEUVRE = 'A0013O0002', PARTIE = 'Secunda Secundae';
const QUESTIONS = Array.from({ length: 6 }, (_, index) => `Question ${136 + index}`);
const EXPECTED = { segments: 211, existing: 32, updates: 28, deletes: 4, inserts: 12, final: 40 };
const EXPECTED_TYPES = new Map([[1, 28], [2, 0], [3, 12], [4, 0]]);
const EXPECTED_SEGMENTS_HASH = '472662dc2f2c2bb782bcf142f15f30b7ade42736242ccc71d164dd37bba2dc87';
const EXPECTED_LINKS_HASH = 'bf4625552597038c8dbf6d9dfc690713c632e976a087688f3bfb870b22526261';
...
const lit = (value) => value == null ? 'null' : typeof value === 'number' ? String(value)
...
const snapshot = (label, segments, links) => {
  ...
  const name = `Q136-141-${label}-${stamp}.json`;
...
const fetchLive = async () => {
  ...
  for (let from = 0; ; from += 100) {
    const page = await must(sb.from('segments').select('*').eq('id_oeuvre', OEUVRE).eq('ref_niv1', PARTIE)
  ...
  for (let offset = 0; offset < segments.length; offset += 100) {
    ...
    for (let from = 0; ; from += 100) {
      const page = await must(sb.from('liens_bibliques').select('*').in('segment_id', ids).order('id').range(from, from + 99),
        `liens lot ${offset / 100 + 1} page ${from / 100 + 1}`);
...
if (raw.segments.length !== EXPECTED.segments || raw.links.length !== EXPECTED.existing ||
    raw.segments[0]?.segment_numero !== 18900 || raw.segments.at(-1)?.segment_numero !== 19110) throw new Error('Baseline brute inattendue.');
...
if (plan.decisions?.length !== EXPECTED.existing || plan.insertions?.length !== EXPECTED.inserts ||
    plan.segments_audites?.length !== EXPECTED.segments) throw new Error('Dossier incomplet.');
const kept = plan.decisions.filter((decision) => decision.final), deleted = plan.decisions.filter((decision) => !decision.final);
if (kept.length !== EXPECTED.updates || deleted.length !== EXPECTED.deletes ||
    plan.summary?.liens_finaux_proposes !== EXPECTED.final) throw new Error('Répartition du plan inattendue.');
if (plan.controle_stratifie?.length < 24 || plan.controle_stratifie.filter((item) => item.type === 3 || item.type === 4).length < 12 ||
    plan.controle_stratifie.some((item) => !item.verdict.startsWith('juste'))) throw new Error('Contrôle stratifié insuffisant.');
for (const item of [...kept, ...plan.insertions]) {
  ...
  if (final.fiabilite !== 'vérifié' || final.provenance !== 'lecture' || final.arbitrage_requis !== false || !final.motif || final.motif.length < 50) throw new Error(`Métadonnées finales invalides ${item.link_id ?? item.id_proposition}`);
}
...
const finalKeys = finalItems.map(finalKey);
...
const auditIds = new Set(plan.segments_audites.map((item) => item.id));
if (auditIds.size !== EXPECTED.segments || raw.segments.some((segment) => !auditIds.has(segment.id))) throw new Error('Couverture des segments incomplète.');
const decisionIds = new Set(plan.decisions.map((item) => item.link_id));
if (decisionIds.size !== EXPECTED.existing || raw.links.some((link) => !decisionIds.has(link.id))) throw new Error('Couverture des liens incomplète.');
const canonicalIds = [...new Set(finalItems.map((item) => item.canon_id))];
const canonRows = [];
for (let index = 0; index < canonicalIds.length; index += 100) canonRows.push(...await must(sb.from('versets_canon').select('id').in('id', canonicalIds.slice(index, index + 100)).range(0, 99), 'cibles canon'));
const liveCanon = new Set(canonRows.map((row) => row.id));
if (canonicalIds.some((id) => !liveCanon.has(id))) throw new Error(`Cible morte dans le plan : ${canonicalIds.filter((id) => !liveCanon.has(id)).join(', ')}`);
...
if (!APPLY) {
  console.log(JSON.stringify({ ready: true, applied: false,
    reason: 'Garde active : aucune écriture sans --apply explicite après validation humaine.', snapshot: before,
    pagination_segments: [100, 100, 11], pagination_liens_par_lot: [18, 14, 0], segments: EXPECTED.segments,
    liens_avant: EXPECTED.existing, mises_a_jour: EXPECTED.updates, suppressions: EXPECTED.deletes,
    insertions: EXPECTED.inserts, liens_apres_attendus: EXPECTED.final }, null, 2));
  process.exit(0);
}

const oldById = new Map(raw.links.map((link) => [link.id, link]));
const oldPredicate = (link) => [`id=${lit(link.id)}`, `segment_id=${lit(link.segment_id)}`,
  `canon_id is not distinct from ${lit(link.canon_id)}`, `verset_v2_id is not distinct from ${lit(link.verset_v2_id)}`,
  `livre is not distinct from ${lit(link.livre)}`, `chapitre is not distinct from ${lit(link.chapitre)}`, `type=${lit(link.type)}`,
  `fiabilite=${lit(link.fiabilite)}`, `motif is not distinct from ${lit(link.motif)}`, `provenance=${lit(link.provenance)}`,
  `arbitrage_requis=${lit(link.arbitrage_requis)}`, `created_at is not distinct from ${lit(link.created_at)}::timestamptz`,
  `updated_at is not distinct from ${lit(link.updated_at)}::timestamptz`].join(' and ');
const statements = [];
for (const decision of plan.decisions) {
  const old = oldById.get(decision.link_id);
  if (!old || stable(old) !== stable(decision.avant)) throw new Error(`Préétat décision invalide ${decision.link_id}`);
  if (!decision.final) {
...
const sql = `do $strict_q136_141$ declare n_up int:=0; n_del int:=0; n_ins int:=0; n_mark int:=0; n int; begin
...
select count(*) into n from liens_bibliques where segment_id=any(array[${idsSql}]::bigint[]) and (canon_id is null or verset_v2_id is not null or livre is not null or chapitre is not null); if n<>0 then raise exception 'cible unique %',n; end if;
