/** Application atomique de l'audit IIa-IIae Q153-158. Garde active sans --apply. */
...
const plan = JSON.parse(readFileSync(`${ROOT}/Q153-158-AUDIT-EXHAUSTIF.json`, 'utf8'));
const APPLY = process.argv.includes('--apply');
const OEUVRE = 'A0013O0002', PARTIE = 'Secunda Secundae';
const QUESTIONS = Array.from({ length: 6 }, (_, index) => `Question ${153 + index}`);
const EXPECTED = { segments: 397, existing: 73, updates: 72, deletes: 1, inserts: 50, final: 122 };
const EXPECTED_TYPES = new Map([[1, 76], [2, 7], [3, 39], [4, 0]]);
const EXPECTED_SEGMENTS_HASH = 'ef501d83d662a81d2684b2109cce6161edaf0ddd3319006faa505937386ae3ce';
const EXPECTED_LINKS_HASH = '3db6d296874fcf36c5dbd205b5f43accaba7573982265e24d0d08a120ec23987';
...
const snapshot = (label, segments, links) => {
  ...
  const name = `Q153-158-${label}-${stamp}.json`;
...
if (raw.segments.length !== EXPECTED.segments || raw.links.length !== EXPECTED.existing ||
    raw.segments[0]?.segment_numero !== 19569 || raw.segments.at(-1)?.segment_numero !== 19965) throw new Error('Baseline brute inattendue.');
...
const canonicalIds = [...new Set(finalItems.map((item) => item.canon_id))], canonRows = [];
...
if (!APPLY) {
  console.log(JSON.stringify({ ready: true, applied: false,
    ...
    pagination_segments: [100, 100, 100, 97], pagination_liens_par_lot: [25, 14, 15, 19],
...
const sql = `do $strict_q153_158$ declare n_up int:=0; n_del int:=0; n_ins int:=0; n_mark int:=0; n int; begin
