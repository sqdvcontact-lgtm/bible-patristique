...
⛔ **Le volet ne porte plus de CASES** (2026-08-27, charte § 18). Un axe se rend en FIL — les états en ligne, séparés par un blanc (`MOT_DU_FIL`, `BLANC_DU_FIL` dans `stylesVoletLecture.ts`) — et un groupe déclaré `bascule` se rend en UNE ligne d'action portant son `labelAction` (`LIGNE_ACTION_VOLET`), sans étiquette de rubrique. Cinq cases pour deux décisions pesaient 181 px en tête du volet ; il en reste 83. ⚠️ Un libellé ne redit pas le nom de son axe — les graphies sont « Modernisée », « Développées », « Diplomatique », et « Abréviations développées » débordait à lui seul la largeur du volet.

⛔ **Le fil se compose en SANS, et l'état retenu porte un TRAIT** (reprise du 2026-08-27, le premier jet ayant été jugé illisible par l'auteur). En sérif gris, sans autre marque que la teinte de l'actif, c'étaient trois mots de prose au milieu d'un volet, qui ne s'annonçaient pas comme un réglage. Il porte donc les deux signaux dont le site se sert déjà : le sans des repères d'interface, et le trait sous l'état retenu, celui des onglets de la bibliothèque. ⚠️ Le trait est posé sur les DEUX états, transparent quand le mot n'est pas retenu, sinon le fil sauterait d'un pixel à chaque changement ; et il prend `--cs-vert`, **jamais `--cs-vert-aplat`** dont les onglets du site se servent — c'est un aplat, il vaut `#4a3c2c` sur le sol du Cuir et le trait y disparaîtrait (les deux valent le même vert au Clair). Le survol d'un mot inactif ESQUISSE le trait en gris ; ⛔ pas de soulignement, deux marques au même endroit se confondraient. Règles dans `globals.css` (`.cs-mot-fil`, `.cs-ligne-action`).

⛔ **Le point médian a été essayé et retiré le même jour** : le fil se coupe quand les libellés sont longs — les trois graphies de la Bible 899 font 187 px pour 179 de volet — et le point restait alors SEUL en bout de ligne. Un blanc de 14 px se coupe proprement, et le trait lie la série à lui seul. ⚠️ Ces largeurs se mesurent sur `tmp/planche-volet-lecture.mjs`, qui rend les VRAIS objets de style hors session (la page Bible tombe en 500 sans session) : Fillion tient sur une ligne, la Bible 899 passe à deux.
