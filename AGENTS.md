...
- ⛔ **le filet du DESSUS tombe** (`barreVolet` dans `NavLivres`) : la barre était enfermée entre deux filets à trente pixels l'un de l'autre et se lisait comme une **bande posée en travers du volet**. Un seul filet ferme la tête, et c'est le sien, qui la sépare de ce qu'elle commande. ⚠️ Le bloc qui la précède n'est pas toujours le même — les axes quand l'édition en offre, la carte de traduction sinon — d'où le drapeau lu aux deux endroits et la propriété `filetBas` d'`EncartTraduction`.

**Barre : 35 → 27 px ; tête : 243 → 234 px.** Quatre partis ont été rendus à la largeur réelle et dans les deux thèmes avant de trancher (`tmp/planche-onglets-volet.tsx`) : le modèle tel quel, le modèle resserré, le couple sur la pastille du volet, et deux mots au fer sans barre. Le resserrement l'emporte, étant à la fois **le plus léger des quatre** (27 px contre 33 et 31) et le seul qui garde le modèle. ⛔ La pastille a été écartée bien qu'élégante : elle recréait un contrôle propre au volet, c'est-à-dire la divergence même que le modèle commun a fermée.
