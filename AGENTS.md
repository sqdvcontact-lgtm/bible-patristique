...
⛔ **Le volet ne porte plus de CASES** (2026-08-27, charte § 18). Un axe se rend en FIL — les états en ligne, séparés par des points médians, l'actif seul en encre du site (`MOT_DU_FIL`, `SEPARATEUR_FIL` dans `stylesVoletLecture.ts`) — et un groupe déclaré `bascule` se rend en UNE ligne d'action portant son `labelAction` (`LIGNE_ACTION_VOLET`), sans étiquette de rubrique. Cinq cases pour deux décisions pesaient 181 px en tête du volet ; il en reste 83. ⚠️ Le cadre parti, c'est le SURVOL qui dit l'action : les classes `.cs-mot-fil` et `.cs-ligne-action` de `globals.css` portent ce soulignement, et le mot actif en est exclu. ⚠️ Un libellé ne redit pas le nom de son axe — les graphies sont « Modernisée · Développées · Diplomatique », et « Abréviations développées » débordait à lui seul la largeur du volet.
