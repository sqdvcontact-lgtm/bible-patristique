...
- **36 tests** (`specimens.test.tsx`) : chaque unité est RENDUE et doit porter du texte ; trois tests vérifient en outre que la planche tire bien ses valeurs des modules partagés, et non de copies. `titre_chapitre_livre` fait exception — son VIDE est la bonne réponse (charte § 35.1), ce que le test exige explicitement.
