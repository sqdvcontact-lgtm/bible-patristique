...
- ⛔ **LA COUTURE : les rangs d'un même paragraphe se TOUCHENT** (`.para-bilingue--couture`) — ni filet, ni blanc, ni retrait ; seul le dernier rang le ferme (`BlocEnRegard.clot`). ⚠️ Deux classes au sélecteur, sinon `.para-bilingue > p` l'emporterait. ⚠️ Il reste une coupure de LIGNE à chaque empan, et elle est irréductible : aucune écriture CSS ne fait couler un texte d'un rang de grille au suivant en gardant deux colonnes accordées.
