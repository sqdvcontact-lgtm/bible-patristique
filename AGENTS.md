...
- ⛔ **La légende SORT DU FLUX** (`position: absolute` dans la marge). Une grille à deux colonnes aurait rompu la fusion des marges verticales et les règles de voisinage — `.verset-row + .cs-bible-axe > .cs-bible-bloc` —, c'est-à-dire précisément ce que la planche sert à juger. Sous 1 200 px la marge repasse sous son unité.
