...
🎨 **La PLANCHE DES STYLES — `/admin/styles`** (2026-08-28). Un exemple de chaque style, à la mesure du bloc de lecture, sur fond Papier ou Cuir. **Quatre onglets, parce qu'il y a quatre vocabulaires et qu'on les confond** : le texte biblique, le corps d'une œuvre, l'apparat d'une œuvre, l'apparat d'une bible. Code dans `app/admin/styles/` — la page pose le cadre, `specimens.tsx` porte les exemples et leur légende.
- ⚠️ **Chaque spécimen dit sa FIDÉLITÉ.** `composant réel` : il passe par le composant du site — c'est le cas de TOUT l'onglet « Apparat des bibles », rendu par `BlocEditorialBible`, qui ne peut donc pas dériver. `reproduction` : la composition est enfermée dans `OeuvreClient` ou `TexteBible` et ne s'appelle pas d'ailleurs ; le spécimen la rejoue aux valeurs exactes et nomme sa source.
- ⛔ **Une reproduction qui dérive est pire que pas de spécimen** : elle ferait autorité contre le site. En touchant à la composition d'une œuvre ou d'une rangée de verset, on passe par `specimens.tsx`.
- **38 tests** (`specimens.test.tsx`) : chaque spécimen est RENDU et doit porter du texte — sauf `titre_chapitre_livre`, dont le vide est la bonne réponse (charte § 35.1), ce que le test exige explicitement.
