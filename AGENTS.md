...
- ⛔ **ALIGNEMENT N'EST PAS PARAGRAPHAGE, et le contraire a valu jusqu'au 2026-09-07.** Un groupe dit ce qui se répond d'une colonne à l'autre ; il ne dit RIEN de la découpe du texte, qui appartient à l'édition seule et se lit dans `paragraphe`. Le bloc de lecture est donc le PARAGRAPHE, découpé sur la clé éditoriale entière — `espace_textuel`, `ref_niv*`, `paragraphe` —, ses segments rangés par `rang` et joints par `join_before` ; les groupes se RÉPARTISSENT ensuite dessus (`repartirGroupes`), chacun composant son original dans le premier bloc qu'il touche.
- ⚠️ **Le Discours 38 l'a démontré** : son alignement, posé au SEGMENT, compte 76 groupes sur un corps de deux paragraphes, et le lecteur en tirait 76 blocs, chacun sous son filet et son blanc — soixante-seize paragraphes ouverts en minuscule (« ces choses là… », « aussi ont faict les Juifs… ») là où Morel n'en a écrit qu'un. ⛔ Et l'on ne « corrige » pas une minuscule en base pour masquer un défaut d'affichage : cette minuscule est la trace d'une phrase qui continue.
- ⚠️ **Contrôle sur les données réelles** (`tmp/controle-blocs-bilingues.mts`, qui rejoue les DEUX découpes avec les vraies fonctions) : Discours 38, 77 blocs → 3 ; Cité de Dieu, 2 391 → 1 064 ; Boèce, 1 142 → 475 ; Didachè, 106 → 57 ; Confessions inchangées, 933 → 943. Là où l'alignement est plus GROSSIER que le paragraphage, la découpe rend au contraire les coupures que l'édition demande : Heptateuque, 786 blocs dont 133 à colonne vide.
- ⚠️ **Le tri par `rang` est celui de `paragraphesDe`, et il ne peut plus mêler deux moitiés de groupe** : le bloc étant le paragraphe, `rang` y est bien l'ordre de lecture. Côté original, c'est toujours `member_order` qui fait foi. ⛔ `repartirGroupes` juge l'appartenance d'une borne par un `Set`, jamais par la position `ids[0]` : un bloc trié par `rang` et des bornes prises dans l'ordre de lecture ne se comparent pas par index.
- ⚠️ **Un paragraphe qui réunit plusieurs groupes fond leurs originaux en un seul texte** (`fondreOriginaux`) : la PROSE se joint par `join_before` — d'où `BlocOriginal.joinBefore`, le liant du premier segment du groupe — et les VERS par un saut de ligne, la jonction se jugeant bloc par bloc sur `toutVers`.
