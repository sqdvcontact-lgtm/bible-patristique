...
- **Une seule définition de la préparation : `app/lib/preparerPortrait.ts`** (module partagé, partie géométrique testée). Réduction à l'intérieur de 600 × 750 sans rognage, proportions conservées, orientation EXIF respectée, conversion en JPEG qualité 90. Les TROIS écrans qui déposent un portrait l'emploient : bibliothèque, auteurs, traductions. Ne pas recomposer un redimensionnement ailleurs — c'est ainsi que les trois avaient divergé.
- **La route refuse ce qui n'est pas du JPEG** plutôt que de le ranger sous une extension qui ment. Le seau portait des objets `.jpg` qui étaient des PNG de 3 Mo (`A0015` l'est encore, versé avant le correctif).
- ⚠️ **Le seau `traductions` reste à reprendre** : sept portraits de 1,5 à 6,5 Mo, versés bruts avant le correctif, pour un affichage de quelques centaines de pixels.
