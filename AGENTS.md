...
## Les quatre menus déroulants paraissent aussitôt, mais ne se FIXENT qu'au bout d'un court moment (2026-08-30)

Doctrine à la charte, § 18 (« Un menu déroulant paraît aussitôt, mais il ne se FIXE qu'au bout d'un court moment »). Ce qui suit en est l'implémentation, dans `app/components/Navbar.tsx`.
...
- **`useMenuSurvol`** tient l'état, en DEUX TEMPS. L'entrée sur l'onglet ouvre le menu **sans délai** ; une minuterie de `DELAI_FIXATION_MS` le déclare ensuite *fixé*. Le drapeau `fixe` est une **référence**, non un état : la fixation ne change rien à ce qui est dessiné, en faire un état ferait re-rendre la barre après chaque survol, pour rien.
- ⛔ **`DELAI_FIXATION_MS` est la durée d'un PASSAGE, pas un temps d'attente**, et se règle sur elle seule : un onglet fait 31 px de haut, la traversée verticale — le seul trajet qui se poursuive dans le menu — prend 15 à 40 ms à l'allure ordinaire, 100 en flânant. ⚠️ **500 ms, essayés d'abord, dépassaient le geste délibéré** (2 à 4 dixièmes pour descendre de l'onglet vers la première entrée) : le menu tombait sous les doigts de qui venait de l'ouvrir. Réglé à **200 ms**. C'est le seul chiffre à toucher si le réglage doit encore bouger.
- ⛔ **Deux jeux de gestionnaires, non interchangeables.** `onglet` (posé sur le `<Link>`) : `onPointerEnter` ouvre et lance le compte, `onPointerLeave` ferme **tant que le menu n'est pas fixé** — le curseur fût-il en train d'entrer dans le menu. `groupe` (posé sur le `<span class="cs-plus">`, qui englobe le menu) : `onPointerLeave` ferme un menu fixé. C'est l'écart entre les deux boîtes qui porte toute la règle ; les intervertir la détruit.
- ⚠️ **Rentrer sur l'onglet quand `ouvert` est déjà vrai ne relance RIEN** (`if (ouvert) return`) : on remonte du menu vers l'onglet, et reprendre le compte à zéro déferait une fixation acquise — le menu tomberait au premier aller-retour.
- **Fermeture immédiate des deux côtés, sans sursis.** Elle rattrape parfois un mouvement en diagonale qui visait le menu et sort par le coin de l'onglet ; le prix en est nul, l'ouverture ne coûtant plus rien.
