...
- **Lib partagée `app/lib/bible899.ts`** (client + serveur, testée `bible899.test.ts`) : `chargerVersets899(client, {livre, chapitre?})`, `livresDisponibles899`, couche (`diplomatic`/`expanded`, défaut `expanded`), prédicats `rendu899` / `aRevoir899`. La capacité de mode `verse` (source `editorial-segments`) est ajoutée au catalogue par `withEditorialVerseCapability` (`bibleMultimode.ts`), interrogée dans `loadBibleReadingCatalog` — chemin **privé**, distinct des vues publiques `v_bible_reading_capabilities` / `v_bible_canonical_lookup` (qui, elles, excluent le non-public et donc Ruth/1 Samuel).
- **Règles d'affichage des statuts** (mêmes règles Bible et Polyglotte) : `MATCH`/`OFFSET` = texte normal ; `verification_status='review'` ou `alignment_status='UNCERTAIN'` = mention discrète « à revoir » ; `CANONICAL_GAP` = « **[lacune du manuscrit]** » (le manuscrit ne porte pas ce verset) ; `MANUSCRIPT_EXTRA` (`canon_id` NULL, rubriques incipit/explicit/argument) = **jamais** transformé en faux verset canonique (écarté de la grille).
- **Bible classique** (`app/page.tsx` → `BibleLayout`/`TexteBible`) : en mode `verse`, si `estVerseEditorial(trad)` on charge via la lib au lieu de `versets_lecture`. `TexteBible` porte le sélecteur de mode (`BibleReadingModeSelector`) et le choix de couche (`?couche=diplomatic|expanded`), rend les lacunes/relectures, et **masque les actions d'écriture** (prélèvement/signalement/édition ciblent un id de verset canonique inexistant pour le manuscrit) + ne compte pas la lecture. `BibleLayout` lit les livres disponibles via `livresDisponibles899` (TR0009 absent de `livres_par_traduction`). ⚠️ **L'effet de préférence de `BibleLayout` (traduction par défaut enregistrée) ne s'applique PLUS quand l'URL nomme `?trad=`** (`tradExplicite`) : le lien profond fait foi, sinon la préférence écrasait le choix et, pour TR0009, lisait une colonne absente des versets chargés → texte vide. Un garde complémentaire interdit tout échange **en mémoire** vers/depuis une traduction éditoriale (elle exige un rechargement serveur, via `handleSetTraductionIndex` / l'URL).
- **Polyglotte** (`app/polyglotte/page.tsx`) : TR0009 est une **colonne synthétique** ajoutée à `trads` hors `versets_v2` ; dans `charger`, son texte est recomposé en `V2Row` clé `canon_id` (mêmes `chScope` que le reste), rendu générique. Choix de couche « BIBLE 899 · TEXTE » affiché quand la colonne est présente.
- **Garde-fous** : ne pas écrire dans `versets_canon` / TR0001–TR0005 ; ne pas copier TR0009 dans `versets_v2` ; ne créer aucune graphie **modernisée** (seules `diplomatic` et `expanded` existent) ; réutiliser les composants existants (pas de duplication de la grille ni du lecteur).
