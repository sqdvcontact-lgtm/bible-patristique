...
⛔ **ET LE JOURNAL DE CHANTIER NE VA PAS DANS LA CHARTE, mais dans `parametres.carnet_ia`** (ouvert le 2026-09-08). Journaux de séance, bilans chiffrés, listes d'œuvres traitées, mesures datées, relevés d'audit, récit d'une correction, décisions périmées : au carnet, qui n'est pas normatif. ⚠️ La charte l'interdisait dès son préambule depuis l'origine et n'a jamais été suivie, faute d'un lieu où déverser — deux chapitres de journal y pesaient un tiers du texte. **Une interdiction sans destination ne s'applique pas.** Miroir : `charte/CARNET_IA.md`, tiré et poussé par `node scripts/synchroniser-charte-supabase.mjs --carnet --pull` (ou `--push`), qui ne lui applique que le contrôle des caractères de remplacement — un carnet n'a ni numérotation à tenir ni invariant de note positionnelle.

⚠️ **LE NOYAU est la charte réduite à sa loi, et il est DÉRIVÉ** : `charte/NOYAU.md`, régénéré par `node scripts/charte-noyau.mjs`, extrait l’impératif de chaque énoncé marqué ⛔ ou ⚠️ et le range sous le numéro de sa section. 164 000 signes contre 938 000, soit un sixième — de quoi savoir qu’une règle EXISTE, puis ouvrir la charte pour ce qu’elle dit exactement. ⛔ On ne l’édite jamais : une correction portée là se perd à la régénération. ⛔ Et il ne va PAS en base : `charte_ia` en est la source, une troisième copie rouvrirait le cimetière que le § 27.1 vient de fermer.

⚠️ **Le noyau MESURE la discipline d’écriture, et son relevé de couverture le dit** : une section écrite règle-d’abord, l’impératif en gras et seul, y rend des entrées nettes ; une section qui noie sa règle dans sa démonstration y rend des phrases molles. Un chapitre qui prescrit SANS employer ⛔ ni ⚠️ — le § 48, le § 23, le § 26 — y est sous-représenté, et cela ne veut pas dire qu’il prescrit peu.

⚠️ **Le partage se juge sur la PORTÉE, non sur le sujet.** Une RÈGLE vaut pour la prochaine séance : elle prescrit, elle interdit, elle nomme un invariant, elle dit ce qu'une garde doit refuser — charte. Un CONSTAT vaut pour un jour donné : une mesure, un compte, ce qu'on a trouvé, ce qu'on a essayé puis écarté — carnet. Une trouvaille de carnet FONDE souvent une règle : on écrit la règle dans la charte, courte, et l'on garde la mesure au carnet.
...
### ⛔ LE DOIGT — ce que la transposition mobile n'avait pas transposé (audit du 2026-09-06)

**La phrase qui résume l'audit, et la seule chose à en retenir : la mise en page bascule à 900px, la GÉOMÉTRIE des contrôles ne bascule pas.** Le prop `mobile` a servi aux largeurs et aux empilements ; il n'a jamais servi à la taille des boutons. C'est la cause unique de la moitié des défauts relevés — boutons, onglets, croix, appels et champs ont gardé le gabarit du bureau, où l'on vise à la souris.

- ⛔ **UN CONTRÔLE TACTILE NE DESCEND PAS SOUS 24px, et vise 44 sur le chemin de lecture.** WCAG 2.2 § 2.5.8 (niveau AA) fixe le plancher à 24 × 24 px CSS ; Apple et Google visent 44. Mesuré avant correction : flèches de chapitre **5,4 × 20**, appel de note **6 × 8**, croix de fermeture **8 × 16**, boutons d'action **16 et 18**, options de la recherche **18,9**, pagination **27**, hamburger **32**.
- **Trois formes, et l'on choisit selon le VOISINAGE.** Un contrôle ISOLÉ reçoit une zone de frappe débordante, `.cs-cible-fine` (`::after { inset: -12px }`), qui agrandit la CIBLE sans toucher au dessin : c'est le patron de `.etoile-favori`, porté à la bonne mesure. Un contrôle en GRAPPE — quatre boutons dans le pavé d'un verset, cinq dans une carte patristique — grandit sa BOÎTE (`.bouton-action-verset`, `.cs-bouton-fin` : 2,25rem), un débord s'y avalant l'un l'autre. Un contrôle qui porte déjà une classe se hausse par son PLANCHER (`min-height: 2.75rem` sur `.cs-onglet`, `.cs-option-volet`).
- ⚠️ **L'appel de note a sa propre mesure**, `.cs-appel-cible` (`inset: -8px -9px`) : deux appels qui se suivent (« 2 & 3 ») ne sont séparés que par leur esperluette, et le débord ordinaire ferait que l'un avale les taps de l'autre. ⛔ Et l'appel est du CONTENU : on ne le grossit pas, on ne fait que l'atteindre.
- ⚠️ **Le débord de `.etoile-favori` (`inset: -7px -6px`) ne vaut PAS 44px** : il porte un glyphe de 12 à 26 sur 24, tout juste le plancher AA. Le geste était bon, la valeur trop courte ; ne pas le recopier tel quel.
- ⛔ **Un débord ne vit pas dans un `overflow: hidden` plus étroit que lui**, et l'hôte doit être positionné (la classe pose `position: relative`, qu'un `position: absolute` en ligne bat sans dommage).

⛔ **LA GARDE iOS EST GLOBALE, JAMAIS PAR PAGE.** Elle n'existait que dans `app/chantier/page.tsx` — les seuls champs de `.cs-ouverture`, sous 640px — alors que ce fichier l'annonçait partout. Une trentaine de champs étaient servis entre 11 et 13,5px : la recherche du site à 13,5, celle de la bibliothèque à 12,5, le filtre de la Communauté à 11, les quatre champs de `/contact`, qui est PUBLIQUE. Safari iOS agrandit la page à chaque prise de foyer sur un champ sous 16px et ne la rend jamais ; dans un tiroir `position: fixed` — le composeur de commentaires — ce zoom pousse le bouton d'envoi hors de l'écran. La règle vit désormais une seule fois, dans `globals.css`, § « LE DOIGT ». ⛔ Le point d'exclamation y est nécessaire : la quasi-totalité de ces champs porte sa taille en style EN LIGNE.

⛔ **L'AXE EST LA CAPACITÉ DU POINTEUR, JAMAIS LA LARGEUR.** Une fenêtre étroite sur un ordinateur garde sa souris, une tablette de 1024px n'en a pas. C'était déjà la règle de `useSansSurvol` ; elle vaut pour tout ce qui se replie au doigt. Trois surfaces s'y trompaient et laissaient leurs actions à `opacity: 0` sur une tablette en paysage, où le prop `mobile` est faux : ni pavé flottant, ni survol possible, donc des boutons présents et invisibles — les actions du verset, celles de la Polyglotte (qui s'affiche dès 821px) et celles d'un segment d'œuvre, ces dernières interceptant en outre des taps sans se montrer.

⚠️ **`viewport-fit=cover` n'est PAS posé, et il ne faut pas le poser sans raison.** `app/layout.tsx` s'en remet au viewport par défaut de Next : la zone de contenu exclut donc d'elle-même les encoches et l'indicateur d'accueil, un `position: fixed; bottom: 0` se posant au bas de la zone SÛRE. Aucun `env(safe-area-inset-*)` n'est nécessaire, et il n'en existe aucun dans `app/`. Le poser rendrait au contraire tous ces décalages obligatoires d'un coup.

⛔ **LA COQUILLE SERVIE PAR LE SERVEUR SE REPLIE EN CSS**, faute de quoi elle n'existe que pour le bureau. `BibleLayout` rend toujours la mise en page de bureau — le prop `mobile` part à faux pour ne pas désaccorder l'hydratation — et rien ne la masquait sous 900px : deux volets de 460px écrasaient la colonne de texte, en `flex: 1; min-width: 0`. Le lecteur voyait cette frame le temps du montage, un navigateur sans JavaScript ne voyait qu'elle. `.cs-bible-coquille` (posée quand le prop est faux) empile les trois blocs en flux sous 900. ⚠️ **On EMPILE plutôt qu'on ne masque** : c'est le patron mobile de la charte, et rien n'y disparaît de ce que le serveur a rendu.

⚠️ **UN SEUIL DE PLACE N'EST PAS LE SEUIL DU HOOK.** Le 900 dit « téléphone » ; la lecture d'un essai ferme son volet sous **1100**, la lecture en regard de la Bible empile ses colonnes sous **980**, parce que ces deux-là disent « la place manque pour lire à côté ». Mesuré dans la bande tablette : 249px de mesure pour un essai à 901px, 140 et 180px par colonne biblique. ⛔ Ce n'est pas une raison d'inventer des seuils : 980 est dans la liste admise, et 1100 s'écrit avec sa raison.

⚠️ **Un composant PARTAGÉ ne réserve pas les barres d'une page qu'il ne sert pas.** `PanneauPatristique` réservait la barre d'onglets et le bandeau de la page Bible dans son mode empilé ; la page d'une péricope, qui le reprend, y gagnait 46px de blanc en tête et 40 en pied. D'où le prop `sousBarres`. Règle générale : une réservation se passe en prop, elle ne se suppose pas.

**Ce qui reste ouvert après cette passe**, et qui demande une décision ou un chantier : le composeur d'un tiroir sous le clavier virtuel d'iOS (il faudrait mesurer `visualViewport`) ; sept informations portées par le seul `title`, invisibles au doigt ; l'administration, qui n'a ni garde d'écran étroit ni message ; et l'empilement, dont la charte note depuis le 2026-08-23 qu'il n'a jamais eu sa passe — quarante valeurs de `z-index` pour un besoin qui en demande cinq, dont quatre ont été corrigées ici sans qu'une échelle soit posée.
...
# Recherche — le texte normalisé, les trois RPC v2 et le module pur (2026-09-06)

Doctrine : charte § 43. Audit et correction du 6 septembre 2026 (« se pencher sur le système de recherche »). Règles de code :

- ⛔ **La page des résultats appelle QUATRE RPC publiques, et rien d'autre** (2026-09-06, « optimise la page ») : `recherche_versets_v2_repartition(p_termes, p_mode, p_scope) → (livre, n)` et `recherche_segments_v2_repartition(p_termes, p_mode) → (id_oeuvre, auteur_nom, oeuvre_titre, n)`, les COMPTES par livre et par œuvre dont les totaux se déduisent ; `recherche_versets_v2(p_termes, p_mode, p_scope, p_livre, p_decalage, p_taille)` et `recherche_segments_v2(p_termes, p_mode, p_id_oeuvre, p_decalage, p_taille)`, UNE PAGE de vingt lignes, rangée et filtrée par la base (le canon ; l'auteur puis l'œuvre, clé de tri `norm_fr` calculée une fois par œuvre dans une CTE matérialisée) ; plus `lexemes_recherche(p_termes)` en mode famille. Le texte original (latin, grec) est cherché dans le MÊME appel que le français : chaque passage dit `match_fr` / `match_orig`, et `langue` est celle de l'ORIGINAL (`oeuvres.langue_originale`, jamais celle de la traduction lue). Les deux fonctions internes `recherche_segments_v2_corresp` et `recherche_versets_v2_ids` portent la condition, écrite une seule fois. ⛔ La page ne rapatrie plus de liste entière : plus de `pagine`, plus de plafond, plus de tri ni de répartition dans le navigateur, et elle ne REJETTE plus rien de ce que la base rend (elle marque ce qu'elle reconnaît). Un mot ou plusieurs passent par la même voie ; ⛔ plus de `ilike` du navigateur sur `segment_texte` ou `versets_lecture`, plus de boucle de graphies u/v i/j côté client. Modes : `prefixe` | `exact` | `famille`. Migration `20260906030000_recherche_paginee.sql`, contrôles dans `supabase/controles/`.
- ⛔ **Les motifs entrent dans la requête en CONSTANTES** (`return query execute format(… %L …)`), jamais en variables plpgsql. Mesuré sur « dieu » (18 072 passages) : 110 ms en constantes, de 230 à 4 700 ms en paramètres — le planificateur ne connaît pas la valeur d'un paramètre, estime au hasard la sélectivité d'une expression rationnelle et bascule sur un parcours complet dès que le plan générique lui paraît bon marché ; le banc d'essai l'a vu à 2 708 ms sur la répartition. Après : répartition « dieu » 321 ms, première page 553 ms, répartition biblique 62 ms, « fils dieu » 280 ms.
- ⛔ **Les trois fonctions des passages sont `security definer`, la garde de publication écrite en clair** (`acces_public`, `is_default`, `is_public` ; migration `recherche_paginee_definer`, le soir même). Mesuré EN LIGNE sous `authenticated` : 5 371 ms sur la répartition de « dieu », contre 310 sous `postgres` — sous la politique de lecture de `segments` (`is_admin()` OU un EXISTS à trois tables), le planificateur abandonne l'index trigramme, relit les 77 507 segments des 35 textes publics par l'index (id_texte, id_oeuvre) et évalue la politique ligne à ligne (505 000 tampons contre 14 000). Après : 274 à 306 ms sous le lecteur, et 16 887 passages pour « dieu » au lieu de 18 072 sous `postgres` — les textes par défaut NON publics n'y entrent plus, à l'administrateur compris : la recherche ne montre que le corpus public. ⛔ Toute fonction de recherche qui lit `segments` s'ÉPROUVE sous le rôle du lecteur (`set local role authenticated` + `request.jwt.claims`), jamais sous `postgres` seul, dont le canal d'administration contourne la RLS : c'est le contrôle 9 du fichier de contrôles.
- **Côté page (`RechercheClient`)** : une `Requete` active (clé numérotée, termes, mode, périmètre), les deux répartitions, et une page par corpus portée avec la CLÉ de la demande qu'elle satisfait (`cleDePage`). Une page dont la clé n'est pas celle demandée est EN ATTENTE, et l'effet la redemande : l'attente se DÉDUIT, rien ne s'allume dans un effet (patron de la Polyglotte). Les lignes d'avant restent sous un voile (`styleAttente`) ; une page qui arrive remonte le défileur, ou lui rend la position d'une recherche reprise (`scrollCibleRef`, consommée par la page de l'onglet affiché). Le filtre d'œuvre est l'IDENTIFIANT (`id_oeuvre`), celui de livre le code. `lancer` part avec six requêtes ensemble : les deux répartitions, la première page de chaque corpus, les publications, les racines. Une erreur se DIT, avec « Réessayer ».
- **La Polyglotte des résultats reproduit le tableau de la page Polyglotte** (demande de l'auteur, 2026-09-06 : « il y a une nouvelle version du tableau dans la page Polyglotte, il faut la reproduire ») : l'en-tête est la barre de la page de lecture (nom en sérif, millésime par `millesimeEdition` dessous en capitales espacées, chevron, fond de survol DANS LA FEUILLE), le menu de colonne un `<select>` natif invisible posé sur le titre, groupé par langue (`<optgroup>`) ; le livre qui change est le `<h2>` collant de la page de lecture (sérif vert centré, filets pâles, rembourrage de gauche égal à la marge de référence). Les classes de colonne restent celles de `globals.css`.
- **`versets_recherche`** est une vue matérialisée (id_verset, trad_id, livre, chapitre, verset, ordre, texte_norm = `norm_fr`) sur les cinq colonnes de `versets_lecture`, avec un index unique (id_verset, trad_id), un GIN trigramme et un GIN plein texte français. ⛔ Elle se rafraîchit UNIQUEMENT dans `rafraichir_versets_lecture`, après `versets_lecture`, concurremment (≈ 20 s de plus par rafraîchissement réel ; le cron d'une minute ne rafraîchit que sur écriture). Ne jamais la peupler ni la corriger à la main : elle dérive.
- **`app/lib/rechercheRequete.ts` est pur et testé** : termes, mode (`modeDepuisParametre`), référence biblique tapée (`referenceBiblique`, par la grammaire des péricopes), la MARQUE (`marqueDe` → termes, ou racines en famille), la relecture (`contientMarque`, `compterMarque`, `regexMarque`) et les graphies latines pour relire un original. ⛔ La frontière de mot y est `[^\p{L}\p{N}]`, celle de la base — l'ancienne liste de séparateurs ignorait l'apostrophe et le tiret, et rejetait « l’espérance ». ⛔ **Et `normaliser` reprend les règles de graphie ancienne de `norm_fr`** (oit → ait, oient → aient, connoître → connaître, foible → faible) : relevé sur le site le jour même, la base rendait 3 249 versets pour « était » et la page n'en gardait que 2 153 — les 1 096 où seule Sacy (« étoit ») porte le mot étaient rejetés par une relecture qui ne connaissait pas la graphie. Seules les règles qui GARDENT LA LONGUEUR sont reprises (le marquage retrouve ses positions par l'index) ; « tems », « enfans », « sçav » restent à la base seule. **Toute règle ajoutée à `norm_fr` se reporte ici**, sous cette contrainte, sinon la page rejette ce que la base rend.
- ⚠️ **En mode famille, la page ne rejette rien de ce que la base rend** : elle marque ce qu'elle reconnaît par les racines (`lexemes_recherche`, prefix des mots fléchis), le mot fléchi ENTIER (`jusquAuBout`), et le texte original se relit sous ses graphies latines en début de mot ou entier.
- ⛔ **Une référence chiffrée ne lance AUCUNE recherche** : `lancer` pose la carte et s'arrête — « jean », « 3 », « 16 » sont partout, et la page attendait des milliers de lignes inutiles avant de montrer où aller.
- **La barre rapide** offre en premier rang « Passage biblique » quand `referenceBiblique(requete)` n'est pas nul, et Entrée y mène ; sinon Entrée mène à `/recherche`.
- **Mesures** (2026-09-06, rôle postgres) : « miséricorde » toutes bibles 890 ms → 9 ms ; « était » dans Sacy 0 → 2 267 ; « fils dieu » segments 342 ms (ilike brut) → 36 ms (regex normalisée) ; « aimer » famille 541 versets, 1 692 segments. Contrôles rejoués après application : `supabase/controles/20260905233021_recherche_normalisee_controles.sql` (migration `20260905233021_recherche_normalisee_versets_segments.sql`, version choisie par le journal à l'application, le fichier renommé d'après lui).
- ⚠️ **Le banc d'essai HTTP (`dry-run-migration.mjs`) plafonne à quelques secondes par instruction** ; une migration qui bâtit des index se joue par le canal d'administration (MCP `execute_sql`, `begin … rollback`), qui a tenu vingt secondes ici.
- Ce qui reste : les essais se cherchent en `ilike` brut (25 textes, chargés entiers et filtrés dans la page) ; le grec n'est pas désaccentué ; `recherche_segments`, `recherche_versets(p_terme, p_scope)` et `recherche_segments_original_v2` restent en place sans appelant dans le site — la dernière gardée le temps du déploiement, la base étant PARTAGÉE avec le site en ligne (une RPC retirée avant que le correctif soit déployé met la recherche en erreur pendant ces minutes) ; un identifiant de segment au-delà de 2^53 se perd dans le JSON, comme partout sur le site.
...
- **Affichage** : titre principal / référence biblique / catégorie. La référence se construit sur la PREMIÈRE occurrence principale. Ligne « Correspond à : X » quand l'appellation qui a RÉPONDU n'est pas le titre, alias masqué compris (`correspondanceVisible`, module pur `pericopesRecherche`, testé ; décision du 2026-09-06 : « Homme » rendait « Je ne fais pas le bien que je veux » sans un mot pour dire pourquoi, par la paraphrase masquée « Homme malheureux que je suis » — un alias masqué ne paraît pas parmi les NOMS d'une péricope, mais quand c'est lui qui répond, le taire rend le résultat incohérent). ⛔ Jamais un alias INEXACT (`populaire_inexact`, ex. « baleine » → « Jonas et le grand poisson », jamais « Jonas dans la baleine »), visible ou non : c'est l'USAGE qui décide, non le drapeau `correspondance_visible`, qui ne sert plus qu'à la page de la péricope.
...
## ⛔ UNE SEULE MARQUE, ET C'EST LE CHIFFRE (2026-09-06)

Demande de l'auteur : « remplace le logo de la navbar par le logo qu'on trouve en bas de la page d'accueil ». Le site en avait DEUX — la lettrine gothique en tête de chaque page, le chiffre didone au pied de l'accueil — et deux marques ne font pas une identité : aucune des deux ne pouvait devenir celle qu'on reconnaît. ⚠️ Le dépôt le disait sans le voir : le commentaire de `.colophon-marque` prévenait « ce chiffre n'est PAS le monogramme de la barre », ce qui est la description exacte du défaut.

- **La planche est `public/ornements/chiffre-cs.png`** (535 × 512), posée **deux fois, en MASQUE** : `.colophon-marque` (`app/accueil/page.tsx`), 1,75 rem, peinte `--cs-or` sur le papier ; `.cs-marque` (`app/components/Navbar.tsx`), 1,625 rem, peinte par `currentColor`, c'est-à-dire l'encre du nom. ⛔ Une seule planche, deux encres — la charte le dit depuis le monogramme : « le détourage ne sert que l'ALPHA : la couleur, on la repose ».
- ⛔ **Dans la barre, la LARGEUR est écrite (`calc(1.625rem * 535 / 512)`), elle ne se déduit pas du rapport.** Un `aspect-ratio` suffit au colophon, qui vit dans un bloc ordinaire ; la marque de la barre est un ENFANT DE FLEX, et la largeur qu'un moteur transfère depuis le rapport y est le point faible des implémentations. Chrome la résout — mesuré en ligne, 37,3 px pour 35,8 de haut à la racine 22 — mais une marque effondrée à zéro ne se verrait sur AUCUNE page, et c'est la première chose que le lecteur regarde.
- ⚠️ **1,625 rem et non 1,875, et ce n'est pas un goût** : le chiffre est LARGE (535 sur 512) quand la lettrine était haute et étroite (464 sur 671). À hauteur égale il aurait pesé un tiers de plus dans la barre la plus disputée du site. Jugé sur planche (le panneau navigateur, la vraie cascade, les deux sols), à cette taille sa hauteur de capitale répond à celle de « Corpus Scriptura » posé contre elle. Sa largeur passe tout de même de 21 à 27 px : la mesure de repli s'en accommode seule, elle lit `scrollWidth`.
- ⛔ **`currentColor`, jamais une teinte écrite.** La marque suit ainsi le Cuir — dont la barre est maroquin — sans être déclinée deux fois, et elle appartient à la LIGNE du nom au lieu d'y trancher : c'est le raisonnement déjà consigné pour le monogramme, qui prenait la teinte du titre.
...
- ⚠️ **La glose mobile rend 4,8 de contraste** (blanc à 68 % sur `--cs-barre-fond-profond`), pour les 4,5 qu'un texte de 13 px réclame. Plus effacée, elle passerait sous le seuil.
- ⛔ **LA BORNE BASSE D'UN MENU GOUVERNE SA LARGEUR, JAMAIS LA HAUTE.** Un menu est en `position: absolute` et son bloc conteneur est l'ONGLET, large d'une centaine de pixels : la largeur idéale d'un bloc ajusté à son contenu se borne à la place DISPONIBLE, si bien que la boîte retombe toujours sur son `min-width`. Relevé sur le site : à 21 rem de minimum elle mesurait 21 rem, à 24 elle mesure 24, sa borne haute valant 27. ⚠️ Une glose qui s'enroule ne se corrige donc pas en relevant le maximum — je l'ai cru le 2026-09-06, et la boîte n'a pas bougé d'un pixel.
- **La mesure se prend en REM, sur la chasse réelle, et le chrome se compte à part.** Les six gloses demandent de 15,7 à 18,5 rem ; le chrome — bords, les deux rembourrages, l'écart et l'emblème — vaut 3,9 rem à la racine 16 et 3,1 à la racine 22, ses pixels ne suivant pas la police racine. D'où **24 rem** de borne basse : la plus longue tient sur une ligne aux deux bouts de l'échelle, avec un rem et demi de reste. ⛔ Au-delà de 20 rem une glose s'enroule et sa rangée prend une hauteur à elle : c'est la GLOSE qu'on raccourcit, pas la boîte qu'on élargit. Celle de la Bibliographie en demandait 22, elle est reprise.
- ⛔ **L'emblème se MESURE EN REM, jamais par l'attribut `taille` du composant** (`.cs-plus-riche-emb svg`, 1,1875 rem). La police racine du site est fluide : un dessin posé en pixels rapetissait à mesure que le nom grandissait. ⚠️ À cette mesure il vaut la LIGNE du nom (0,875 × 1,3 = 1,1375 rem) au dixième de pixel près, et le décalage de deux pixels qui le calait à la main n'a plus lieu d'être.
- **Le nom monte d'un rang** (13 → 14 px) : c'est LUI qu'on vient chercher, et deux pixels et un gris le séparaient seuls de la glose qui l'explique. ⛔ Pas la graisse : `.cs-plus-riche-nom--fort` la réserve à l'entrée par laquelle on entre presque toujours, dans le menu voisin.
- ⚠️ **Contrôle sur planche, en avant/après** : le site étant fermé et l'onglet du navigateur de l'auteur caché — donc sans capture possible —, la comparaison s'est faite dans le panneau navigateur, sur une réplique du menu portant les valeurs RÉELLES des deux états.

## Les quatre menus déroulants ne paraissent que si la main SE POSE (2026-08-30)

Doctrine à la charte, § 18 (« Un menu déroulant ne paraît que si la main se POSE »). Ce qui suit en est l'implémentation, dans `app/components/Navbar.tsx`.
...
- ⛔ **L'intention se lit dans la VITESSE, jamais dans la durée**, et deux réglages par la durée ont été essayés puis abandonnés avant d'en venir là : *attendre avant de montrer* (500 ms, puis 200) fait payer à la main sûre l'hésitation de l'autre ; *montrer puis retirer* fait clignoter la barre à chaque traversée. La vitesse, elle, se connaît **dès l'entrée** et ne fait rien attendre. ⚠️ Aucun délai ne subsiste dans le dispositif, hormis le filet ci-dessous.
- **`VITESSE_POSE_PX_MS = 0.35`** (≈ 350 px/s) : au-dessus, la main file et rien ne paraît ; en dessous, elle se pose et le menu s'ouvre à l'instant. Repères : un balayage de barre court entre 800 et 3 000 px/s ; une main qui vise un onglet franchit ce seuil quelques centièmes avant de s'arrêter. C'est le seul chiffre à toucher si le réglage doit bouger.
- **`DELAI_REPOS_MS = 90`** n'est **pas** un délai d'ouverture mais un filet : une souris qui arrive vite et s'**arrête net** n'émet plus de mouvement, si bien que le ralentissement ne peut plus se lire. ⚠️ Chaque mouvement le réarme — une traversée continue, si longue que soit la cellule, ne le laisse jamais échoir.
- ⛔ **Le pouls du curseur est un écouteur UNIQUE, en capture sur `window`**, partagé par tous les onglets par compteur d'abonnés (`suivreLeCurseur`). Il **range la vitesse**, non le dernier point : React écoute au conteneur racine, donc après la capture au niveau fenêtre, et un point rafraîchi juste avant d'être lu donnerait toujours une distance nulle. ⚠️ Le navigateur émet `pointerover` **avant** le `pointermove` de la même position : à l'entrée, la vitesse lue est bien celle du mouvement qui PRÉCÈDE — exactement ce qu'on veut.
- ⛔ **Deux jeux de gestionnaires, non interchangeables.** `onglet` (sur le `<Link>`) : `onPointerEnter` et `onPointerMove`, où se juge la pose. `groupe` (sur le `<span class="cs-plus">`, qui englobe le menu) : `onPointerLeave`, qui ferme. Tant que le menu est caché, la boîte du groupe est celle de l'onglet — sortir de l'un c'est sortir de l'autre, et le guet en cours s'arrête avec lui : c'est ce qui garantit qu'une traversée ne laisse rien derrière elle.
- ⚠️ **Plus de règle de fixation** : elle n'a plus d'objet, un menu ouvert au passage n'existant plus. Un menu ne reste ouvert que si le curseur s'immobilise dedans, ce qui est précisément le cas où il doit rester.
- ⛔ **Souris seulement** : `e.pointerType !== 'mouse'` sort des trois gestionnaires. Au doigt il n'y a pas de survol, et le `pointerenter` émis après la frappe couvrirait la page qu'on vient d'appeler.
...
- **Absence** : `--cs-absence-fond`, la colonne ou la ligne d'une bible qui **ne porte pas le mot cherché** dans les résultats de recherche. ⛔ **Le fond d'un ENCART n'est pas un fond NU, et le confondre est une erreur qui se répète.** `--cs-danger-fond` est calibré pour vivre *sous* une bordure `--cs-danger-bord` et un libellé rouge : la teinte y accompagne ce que la forme dit déjà, et elle peut donc être un souffle. Posée seule — sans cadre, sans pictogramme, sans encre particulière — elle ne se distingue plus du crème de la page, et le signal disparaît (relevé de l'auteur sur le tableau polyglotte, 2026-09-08 : « le rouge du fond de la cellule n'est pas assez vif »). Un fond qui porte **seul** le sens prend donc un jeton à lui, dans la même famille mais à sa propre dose. ⚠️ Et la dose **se relit dans chaque thème au lieu de se recopier** : `color-mix` de l'aplat sur le sol, **34 %** au Clair où il fonce le crème en terre cuite, **48 %** en Cuir où il éclaire le brun en brique sombre — sur un sol sombre, un tiers de brique ne fait qu'un brun de plus. Encre du texte : 8,3 au Clair, 7,6 en Cuir.
...
## `--cs-mention` — la voix de l'éditeur quand elle tient la place d'un texte (2026-08-30)

**Troisième famille née d'un examen, et elle manquait depuis toujours.** « Absent de cette traduction », « Lacune du manuscrit », « Prendre une note sur Gn 1, 6 » : dans une grille de comparaison, ce n'est ni du texte de corpus ni de l'interface. C'est l'éditeur qui parle à la place du texte absent, et ce rôle n'avait pas de nom — la Polyglotte servait ses mentions en `--cs-texte-faible` (le rang le plus ténu de l'échelle de TEXTE, fait pour de l'ombre d'interface), la Polyglotte de la page Recherche en `#8aa593` écrit en dur.

⛔ **Le registre avait dérivé sur quatre axes à la fois**, relevé à l'audit des mises en forme : **quatre corps** pour une seule voix (9,5 · 10,5 · 11,5 · 12,5 px, tous des rangs valides de l'échelle — la garde ne voit pas les registres, seulement la grille) ; **deux polices dans la même colonne**, l'invite des notes en sérif et le message de connexion en sans, parce que la colonne des notes n'est pas une cellule de texte et tombait sur le `body` ; **trois teintes** pour un seul fait ; **deux libellés** pour un seul fait.

⚠️ **Le seuil de 4,5 s'applique ICI, et ce n'est pas une contradiction avec la note du Cuir plus haut.** « Un seuil absolu ne dit rien sur un thème » vaut pour ce qui est FAIT pour s'effacer — l'or des fleurons, le tan des étiquettes. Une mention d'absence n'est pas un ornement : elle est le seul contenu de sa cellule, et c'est elle qu'on cherche quand la case est vide. Mesuré : l'ancienne teinte rendait **2,14** sur `--cs-fond` et **1,87** sur `--cs-vert-pale`, le `#8aa593` de la Recherche 2,43 et 2,13. La nouvelle tient **4,70 au pire des six fonds**.
...
# Audit général du code (2026-09-02) — ce qui reste à faire, par rentabilité

Quatre relectures parallèles (données, sécurité, front, outillage), typage complet (0 erreur), lint (339 erreurs, 198 `any`, 73 `set-state-in-effect`), `npm audit` (9 avis, 7 hauts), mesures en ligne. Rapport complet publié le jour même ; ci-dessous le seul plan de travail, dans l'ordre où il paie.

- ✅ **Fait le jour même** : `hrefSur` (`app/lib/liensSurs.ts`) — la syntaxe `[texte](adresse)` des commentaires et publications posait l'adresse telle quelle dans `href`, un `javascript:` s'exécutait au clic ; région Vercel `dub1` ; passage fluide sur l'œuvre et la Bible ; écran d'attente racine.
- ⛔ **Next 16.2.9 porte neuf avis** (`npm audit`), corrigés dès 16.2.11 : contournement du proxy (site à locale unique, ce que nous n'avons pas), DoS par actions serveur, confusions de cache, divulgation d'endpoints. Mise à jour mineure (`npm install next@16.3.4 --package-lock-only`, déploiement surveillé). **Décision de l'auteur.**
- ⛔ **La clé de service est instanciée dans 33 fichiers**, dont six pages publiques (`essais`, `pericopes`, `histoire`, `traductions`, `essais/[id]`, `sitemap`). Les requêtes vérifiées filtrent bien (`statut = 'publie'`, `visible_public`), mais la surface est là. Un module serveur unique à écrire, réservé aux routes qui en ont besoin. `app/api/audience/vue` s'en sert comme sel de hachage faute d'`AUDIENCE_SEL`.
- ⛔ **Troncature silencieuse à 1 000 lignes** : `ModalLienBiblique.tsx` charge un livre entier de `versets_lecture` sans `.range()` (les Psaumes en ont ~2 460) ; tous les segments d'une œuvre (`oeuvre/[id]/page.tsx`, `OeuvreClient.tsx`) ; six lectures de `liens_bibliques` par chapitre ; `sitemap.ts`. **Six sites redécoupent `.in()` par nombre fixe** (`pericopes.ts`, `PanneauPatristique.tsx:940`, `SectionVerifications.tsx`, `SectionConstituerLiens.tsx`, `oeuvre/[id]/page.tsx:278`) contre l'interdit de `lotsPourClauseIn` ; 2 clauses sur 110 passent par le helper.
- ⚠️ **174 requêtes dont `error` n'est jamais lue**, dont le chapitre biblique lui-même (`app/page.tsx`, `return data || []`) : une erreur rend un chapitre vide sans message. Aussi `pericopes.ts` (lots), `RechercheClient.tsx:206` (recherche partielle rendue complète).
- ⚠️ **Filtres PostgREST composés avec la saisie** (`RechercheClient.tsx:436,439`, `ModalLienBiblique.tsx:145`, `SectionAjouterOeuvre.tsx:214`) : échapper `,` `.` `(` `)` `%` `_` par un helper unique. Compteurs publics (`essais/incrementer-vue`, `versets/incrementer-lecture`) sans session, débit limité en mémoire de processus (`rateLimiter.ts`, périmé sur Vercel).
- ⚠️ **Front** : `OeuvreClient` recalcule à chaque rendu `segMap`, `segMapApparat`, `groupeParCle`, `bornesGroupes`, `premierSegmentId`, `tocApparatLocal` (26 effets pour 14 mémoïsations) ; 81 effets posent un état dans leur corps, 8 sans dépendances, 16 `eslint-disable exhaustive-deps` ; `sansAccents` copié dix fois ; `detailsRefBiblique` identique client et serveur ; la bibliothèque recharge au montage les deux requêtes de sa page serveur ; 77 blocs cliquables sans rôle ni clavier, 17 boutons icône sans nom ; 72 exports d'`app/lib` sans usage.
- ⚠️ **Outillage** : `verification.yml` ne lance pas `next build` (seule étape qui parse feuille ET types de routes), le linter y est informatif et son `| tee` masquerait son code de sortie ; pas d'`npm audit` ; **zéro test** sur les 55 routes d'API, `proxy.ts` et `bibleEditionServer.ts` ; `.gitignore` laisse `work/` (37 Go), `livraisons/`, `outils/` à un `git add .` de l'historique ; deux verrous (`package-lock.json` versionné, `pnpm-lock.yaml` non) ; 25 rapports à la racine ; `scripts/` = 3 643 fichiers, 521 Mo, 484 scripts datés ; `sql/` mêle migrations hors journal et retours arrière ; `@next/third-parties` et `@types/dompurify` déclarés sans usage, `sharp` employé sans être déclaré ; l'empilement reste sans garde (151 `z-index`, 40 valeurs).
- ⚠️ **Ce fichier** : 4 117 lignes, 644 000 signes, 247 titres, chargés à chaque session ; la règle y est noyée dans le récit. Une quarantaine de milliers de signes de règles s'en détachent ; le reste relève d'un journal par chantier.
...
⛔ **ET LES FEUILLES EN LIGNE ONT LEUR GARDE DEPUIS LE 2026-09-07** : `app/lib/blocsStyleSansAccentGrave.test.ts` refuse tout accent grave dans un bloc `<style>{` … `}</style>`. Un accent grave y FERME le gabarit ; deux se referment l'un l'autre, le fichier se parse **sans une erreur**, l'expression rend un booléen que React n'imprime pas, et la feuille entière disparaît en silence. Payé trois fois — `globals.css` le 2026-08-28, la Polyglotte et la recherche le 2026-09-04, la page d'œuvre le 2026-09-07, où toute la composition de la lecture est tombée huit minutes en ligne. ⚠️ Le gabarit ÉPISSÉ (`…` + expression) reste licite : la garde ne relève qu'un accent grave qui ne borde pas un opérateur, et elle a été éprouvée dans les DEUX sens.
...
## ⛔ La TÊTE du volet se condense, et le seuil du libellé suit ce que le lien FAIT (2026-08-30)

Demande de l'auteur : « cette partie-là du volet de gauche prend trop de place, tu peux condenser ? » — la carte de traduction et le menu des manières de lire, entre le haut du volet et la barre « Livres | Sommaire ». ⛔ **Rien n'est retranché** : mêmes rubriques, mêmes options, même pastille, même référence d'édition. Seuls les blancs se referment, et une ligne entière est récupérée.

- ⚠️ **LE LIEN PARTAGE LA LIGNE DE L'ÉTIQUETTE.** « En savoir plus » occupait une ligne à lui pour une action secondaire, quand « Traduction » laisse les deux tiers de la sienne libres. Il y monte, au fer à droite, **aligné sur la LIGNE DE BASE** et non sur le milieu : l'étiquette pèse 8 px et le lien 9,5, et deux boîtes centrées l'une sur l'autre feraient flotter la plus petite au-dessus de la ligne de l'autre.
- **Le reste est du rythme vertical** : rembourrage de la carte (`10/9` → `7`), gouttière (3 → 2), rembourrage du bloc des axes (`9/10` → `6/7`), écart entre les deux axes (9 → 6), gouttière de rubrique (3 → 1), rangée d'option (`3px` → `2px`, interligne 1,35 → 1,3). ⛔ La rangée mesure alors **17 px pour un texte de 11,5**, la proportion d'une rangée de la liste des livres à quelques pixels de là (22 pour 13,5) ; ne pas descendre plus bas, la pastille cesserait de se lire comme une cible.
- ⛔ **Le seuil du libellé passe de 200 à 230 px, parce que le lien ne fait plus la même chose.** Sur sa propre ligne, il passait à deux puis trois lignes sous 200 ; sur celle de l'étiquette, qui lui prend 53 px, « En savoir plus sur cette traduction » réclame **218 px de volet** (mesuré). Le seuil est porté à 230 pour la douzaine de pixels qui sépare une mesure d'une marge. ⚠️ **Un seuil trop bas n'aurait pas fait DÉBORDER le libellé, il l'aurait COUPÉ** : le lien ne se laisse pas comprimer (`flexShrink: 0`) et la carte est en `overflow: hidden` — le défaut n'aurait paru qu'entre deux largeurs de volet, donc sur une fenêtre et pas sur la voisine. ⛔ Un libellé ne se coupe pas en JavaScript : il faudrait mesurer à chaque rendu.
- **Mesuré sur planche** (`tmp/planche-volet-condense.tsx`, avec les objets de style RÉELS et un balayage de 190 à 400 px) : la tête passe de **256 à 206 px** à 200 px de volet et de **270 à 219 px** à 320, soit un cinquième rendu à la liste des livres. Aucun débordement de la ligne de tête sur toute la plage, marge minimale **13,3 px**. ⚠️ La planche ne peut pas importer `EncartTraduction` : la fiche qu'il ouvre tire `app/lib/supabase`, qui ouvre un client navigateur dès son import (même piège que `pericopesRecherche`).

### ⛔ Un modèle de PAGE ne se pose pas tel quel dans un VOLET (2026-08-30)

Relevé de l'auteur dans le même souffle : « revois aussi Livres | Sommaire ; c'est pas gracieux ainsi ». La barre venait d'être mise au modèle commun le 28 août, et le modèle est dessiné pour une mesure de **46 à 52 rem** ; posé sans reprise dans un volet de **200 px**, il était devenu l'objet le plus aéré du volet — **35 px de haut pour un mot de 11,5**, quand les rangées d'option à trois pixels de là venaient de tomber à 17.

⛔ **La variante `cs-onglets--volet` ne change NI l'identité, NI le dessin** : mêmes libellés centrés à parts égales, même sans du site, même trait vert sous l'onglet retenu, même largeur réservée d'avance en graisse 600. Elle vit dans `globals.css` **à côté du modèle**, jamais en styles en ligne : ce n'est pas une sixième barre recomposée, c'est le modèle à une autre mesure, et il n'y en a toujours qu'un. Elle est passée par `className`, dont c'est désormais le second office (documenté sur le composant).

Trois traits, et trois seulement :

- **le SÉPARATEUR s'en va** — il sépare des cases sur une barre qui en porte trois ou quatre ; entre deux mots dans 200 px, un filet d'un pixel sur quatorze, qui ne touche ni le haut ni le bas, est un détail qu'on voit avant de lire les mots qu'il sépare ;
- **le rembourrage passe de 8 à 4 px**, ce qui rapproche AUSSI le trait vert de son mot : il courait douze pixels dessous, posé sur le filet de la barre, et faisait dans la même bande un troisième trait horizontal ;
- ⛔ **la barre REPREND LA LARGEUR DE LA COLONNE** — voir ci-dessous, c'est le vrai défaut.
...
### ⛔ Une marge AUTOMATIQUE en axe transversal annule l'étirement d'un enfant de flex (2026-08-30)

Relevé de l'auteur dans la foulée : « peut-être mettre une séparation plus nette avant le sommaire ; et étendre la largeur des onglets Livres et Sommaire à la largeur de la colonne ». **Les deux remarques n'en font qu'une, et c'est un défaut, non un goût.** Mesuré aussitôt : la barre ne faisait que **106 px dans un volet de 200**, centrée, et ses deux onglets n'étaient même pas égaux — **43 px pour « Livres », 63 pour « Sommaire »**, alors que `flex: 1` promet des parts égales.

- **La cause** est `margin-left: auto` / `margin-right: auto` du modèle, qui sert à CENTRER une barre de page sous le `max-width` de ce qu'elle commande. Une marge automatique en axe **transversal** absorbe l'espace libre et **annule l'étirement** d'un enfant de flex : dans une page, dont le conteneur est un bloc ordinaire, elle centre ; le volet, lui, est une **colonne flex**, et elle y réduisait la barre à son contenu. ⚠️ Les quatre barres de page sont hors d'atteinte, vérifié une à une : toutes vivent dans des blocs.
- ⛔ **On ne retire pas les marges du MODÈLE**, qui en a besoin : la variante les neutralise (`margin-left: 0; margin-right: 0`), et `flex: 1` rend alors aux deux onglets leurs **99 px chacun**.
- ⚠️ **Et c'est ce qui explique la « séparation » demandée.** Le filet de la barre — celui qui divise la tête du volet de ce qu'elle commande — ne traversait pas la colonne, puisqu'il ne fait que la largeur de la barre.

⛔ **RECTIFICATION : le premier remède avait retiré le filet du DESSUS, et le diagnostic était faux.** La barre paraissait enfermée entre deux filets et se lisait comme une bande posée en travers du volet ; ce qui faisait la bande n'était pas ce filet mais la LARGEUR, le seul trait plein du voisinage étant alors celui d'au-dessus. La largeur rendue, le filet revient, la séparation redevient franche, et il ne coûte **qu'un pixel de hauteur**. `filetBas` d'`EncartTraduction` et le drapeau qui l'accompagnait sont retirés avec lui. *Corollaire de méthode : quand on croit tenir la cause d'un défaut de mise en page, on MESURE la boîte avant de retirer un trait.*

⚠️ **Le CUIR ne se pose que sur la RACINE.** L'épreuve avait d'abord posé `data-theme="sombre"` sur une colonne : la feuille le déclare en `:root[data-theme="sombre"]` et rien ne se déclenchait, si bien que la colonne « Cuir » rendait le Clair sans le dire. Une épreuve sur la vraie feuille se rejoue donc ENTIÈRE par thème (`tmp/epreuve-onglets-volet.tsx`, `?theme=sombre`), et les deux captures se composent ensuite.

⚠️ **Et un « avant » reconstruit par surcharge doit porter `!important` sur ce que la page pose EN LIGNE.** Le filet des axes est un style en ligne : la règle de l'épreuve perdait contre lui, l'« avant » n'a d'abord pas montré le défaut qu'il devait montrer, et seule la mesure (`borderBottomWidth` à `0px` des deux côtés) l'a dit.
...
✅ **`app/loading.tsx` existe à la racine depuis le 2026-09-02**, et ce paragraphe disait jusque-là qu'il ne fallait pas l'écrire, de peur qu'il remplace toute la page à chaque changement de chapitre. La crainte était FAUSSE, et elle s'est éprouvée avant sur la page d'œuvre : un `loading.tsx` ne paraît qu'en changeant de ROUTE ; d'une adresse à l'autre de la même page (un autre chapitre, une autre bible), le routeur garde la page courante. Demande de l'auteur : « quand je clique sur Bible classique ou Polyglotte, j'aimerais que la page se charge, même avant le texte ». L'écran dessine donc la Bible en creux sur `/` (ses trois volets aux largeurs réelles, vides) et un mot d'attente différé ailleurs, l'œuvre et la publication gardant le leur. ⚠️ Corollaire qui compte autant que l'écran : le routeur PRÉCHARGE désormais la page jusqu'à cet écran au survol d'un lien, si bien que le châssis paraît sans attendre le serveur. Contrôlé en ligne le jour même : le changement de chapitre ne le montre pas, l'arrivée depuis l'accueil le montre.
...
- **Elle ne masque pas la page** : voile à 4 %, texte lisible dessous, `pointer-events: none`. ⛔ Jamais un jeton d'ENCRE pour ce voile — sur le Cuir il est presque blanc, et le voile deviendrait un rideau (charte).
- ⛔ **Elle se centre sur le BLOC DE TEXTE, jamais sur l'écran** (demande de l'auteur, 2026-09-03). Elle était `fixed` sous la barre, volets compris, et l'anneau tombait au milieu de l'écran, à côté de la colonne qu'il annonçait. Elle couvre désormais son PARENT (`position: absolute; inset: 0`), qui porte `position: relative` : l'enveloppe de la lecture sur la Bible (`BibleLayout`, passée de `display: contents` à `flex: 1` pour avoir une boîte), une enveloppe autour de `<main>` sur l'œuvre (`OeuvreClient`, allumée par `navigation || niv1Loading`), le corps du tableau sur la Polyglotte. ⚠️ Un bloc qui défile avec la page fait dix écrans : l'anneau vit dans un enfant `sticky` sous la barre, haut d'un écran au plus, qui suit le défilement. Mesuré sur calque (`tmp`, 1280 × 800) : centre de l'anneau à (613, 428) en haut, au milieu et au bas d'un `<main>` de 5 700 px, soit le centre de la colonne et de la part visible. ⛔ Sur l'œuvre, elle ne peut pas vivre DANS `<main>` : `.lecture-sortie` efface tout le bloc, et l'anneau s'effacerait avec le texte.
- **Les écrans d'attente de route sont dessinés EN CREUX, celui de l'œuvre aussi** (2026-09-03) : sommaire et volet biblique aux largeurs de leur premier rendu, vides, et le mot centré dans le bloc de texte entre les deux. Les pièces (barres, volets, le mot et son délai) vivent dans `app/lib/attenteEnCreux.tsx`, partagées avec `app/loading.tsx` ; ⚠️ aucun crochet dans ce module, qui sert un composant client et un composant serveur.
- **Tout ce qui attend a la même apparence, celle de la Bible** (demande de l'auteur, 2026-09-06 : « uniformiser l'apparence des chargements sur le modèle de la page Bible classique »). Vingt-cinq « Chargement… » écrits à la main (cinq corps, deux encres, trois polices, points tapés ou composés, sans délai) sont remplacés par trois pièces de `app/lib/attenteEnCreux.tsx` : `MotAttente` (le mot différé de 450 ms, là où un bloc VIDE attend son premier contenu), `Anneau` (l'anneau de `MarqueAttente` et `MarqueAttenteVolet`, là où un contenu DÉJÀ LÀ se remplace ; les deux anneaux en CSS de la barre de recherche et des infobulles de note prennent son pas de 0,7 s et sa piste `--cs-bord`) et `EcranAttente` (la page entière réduite au mot : `app/loading.tsx` hors Bible, et la première lecture des pages client : compte, profil, prélèvements, péricope, bienvenue). La publication a son écran en creux (`app/essais/[id]/loading.tsx`, volet de 15 rem). ⛔ Un « Chargement… » écrit ailleurs qu'avec ces pièces rouvre la dérive. Hors périmètre, à dessein : les 18 mentions de l'administration (`app/admin`), le lecteur Bible 899 (chantier de l'auteur), et les mentions de cellule de la Polyglotte (`MENTION_ATTENTE`), qui sont la voix de l'éditeur et non une attente.

## La Polyglotte attend et charge comme la Bible (2026-09-03)

Relevé de l'auteur : « la page Polyglotte n'a pas d'indicateur de chargement ; revoir aussi l'efficacité de son chargement ». Le tableau se vidait à chaque changement de chapitre, sans rien dire, puis se remplissait.

- ⛔ **L'attente se DÉDUIT, elle ne s'allume pas.** La demande est une valeur (`Portee` : livres, traductions, chapitre ou livre entier, couche 899) et `porteeChargee` est posée AVEC les données ; on attend tant que `couvre(porteeChargee, demande)` est faux. Aucun témoin « en cours » à éteindre, donc rien qui reste allumé sur une réponse perdue ni ne s'éteigne sur la réponse d'une demande périmée — un numéro de demande jette les réponses qui ne sont plus la dernière. Un livre entier chargé COUVRE chacun de ses chapitres : y revenir ne coûte rien.
- **Ce qui est rendu est ce que les données PORTENT** (`livresRendus`, `chFiltre`), non ce qu'on demande : le chapitre d'avant reste sous la marque jusqu'à l'arrivée du suivant, même parti que la Bible.
- ⚠️ **Une erreur est LEVÉE, non rendue en liste vide** : une liste vide se lit « absent de cette traduction ». Elle se retient dans une RÉFÉRENCE, pas dans une dépendance de l'effet — sans quoi l'échec relancerait la requête en boucle — et « Réessayer » la lève.
- **`fetchPaged` part d'UNE page qui rapporte son compte** (`count: 'exact'` sur une requête qui rend des lignes) : un chapitre coûte un aller-retour au lieu de deux en cascade ; un livre entier en coûte deux, les pages restantes partant ensemble. La Bible 899 part dans la MÊME vague que le canon et le texte.
- **La découverte des traductions migrées se fait par SONDE** (`limit 1`), non par compte exact : celui-ci parcourait l'index entier de chaque traduction (36 000 lignes pour la Vulgate, 13 ms) pour n'en retenir que « plus de zéro ». ⛔ Pas par `livres_par_traduction` : la vue balaie la table entière (487 ms mesurés), dix sondes parallèles coûtent moins qu'elle.
- **`points_sensibles` ne se charge que pour l'administrateur** : rouge, rose et « Lignes problématiques » ne paraissent que chez lui.
- **Le passage d'un texte à l'autre y est FLUIDE, comme sur l'œuvre et la Bible** (demande de l'auteur, 2026-09-03) : `data-passage` sur l'enveloppe du corps, `cs-lecture-colonne` sur le corps, `ordonnerBlocsVisibles` avec `.poly-row, .poly-surnum-row, h2`. Le DÉPART est le moment où `enChargement` passe à vrai (les lignes d'avant sont encore dans le document, c'est ce qui donne quelque chose à effacer) ; l'ARRIVÉE, celui où `porteeChargee` change sous un passage en sortie, dans un effet de mise en page. Un autre chapitre ou un autre livre remonte le corps sous l'en-tête collant ; la même portée rechargée (une colonne changée) garde sa place ; un verset visé fait son propre défilement. ⚠️ Le haut de la lecture est le BAS de l'en-tête collant, non la barre : les lignes qui passent dessous ne sont pas visibles.
- **Ce qui est chargé reste en CACHE, par traduction et par chapitre** (2026-09-03 : `cacheCanon`, `cacheTexte`, `cache899`, au niveau du module et bornés). Changer une colonne ne charge que la colonne ; revenir à un chapitre ne coûte rien ; le livre entier, rangé sous « * », couvre chacun de ses chapitres ; la Bible 899 se garde BRUTE, la couche s'appliquant à la lecture. ⛔ Un chapitre servi du cache l'est PENDANT le rendu (`assemblerDepuisCache`, même patron que le retour à la première page de la Bibliothèque) : l'attente n'est jamais vraie, rien ne s'efface pour reparaître aussitôt, et l'arrivée ne joue que le défilement, comme l'échange de bible en mémoire sur la page Bible. Une même requête ne part pas deux fois (`partager`), et un verset corrigé se corrige aussi dans le cache.
- **Le chapitre SUIVANT se met en cache d'avance**, 400 ms après que le courant est rendu, et seulement en vue d'un chapitre — le livre entier des Psaumes à chaque lecteur qui n'en lit qu'un serait de l'égress pour rien. Et le chapitre SURVOLÉ dans le sommaire, après 150 ms (`onPreparerChapitre` de `NavLivres`, même patron que les modes de lecture). Un préchargement qui échoue ne dit rien : la demande réelle le redira.
...
## Le passage d'un texte à l'autre sur la page Bible (audit du 2026-09-02, corrigé le jour même)

Même question que pour la page d'œuvre (voir « Le passage d'un texte à l'autre est FLUIDE »), posée à la Bible, Fillion en tête. Mesuré **en ligne** AVANT correction, depuis le navigateur de l'auteur, sur Matthieu 5 à 8 : durée de la charge RSC et poids décodé, et ce que devenait la POSITION de lecture (`scrollTop` du défileur `.overflow-y-auto.flex-1`, verset en tête de fenêtre).

**Ce qui est en place depuis le 2026-09-02** (`BibleLayout`, autour de `useAvantDeNaviguer` ; `attenteNavigation.tsx` ; `app/lib/passageTexte.ts`, partagé avec l'œuvre) :
- ⛔ **le départ et l'arrivée se jouent dans le MÊME composant**, `PageBible` restant monté d'une adresse à l'autre. `ProvisionAttente` appelle `surDepart` avant chaque `router.push` (posé par `useAvantDeNaviguer`), et pousse en `scroll: false` ; l'arrivée se reconnaît au changement d'une **clé de lecture** (livre, chapitre, bible, regard, texte seul, graphie, pièce), dans un `useLayoutEffect`, donc avant la peinture ;
- **changer de chapitre remonte le défileur, et lui seul** ; changer de vue au même chapitre (colonne ↔ regard, bible ↔ bible, avec ↔ sans commentaires) **repose le verset en tête de fenêtre à la hauteur qu'il avait**, retrouvé par `verset-N` en une colonne et `data-canon-id` en regard, l'axe canonique étant commun ; on repose quatre fois dans la première seconde tant que le lecteur n'a pas bougé (gravures et polices) ;
- **l'effacement et l'apparition par blocs** portent sur la seule `.cs-lecture-colonne` (l'en-tête, titre et menu, ne bouge pas), par `data-passage` sur l'enveloppe de la lecture et le sélecteur `SELECTEUR_BLOCS_BIBLE` ; ⚠️ un échange de bible EN MÉMOIRE (deux canoniques) ne joue rien, la colonne ayant déjà changé ; une navigation sans arrivée (`?verset=` au même chapitre, retour arrière) rend son texte quand `enAttente` retombe ;
- **la lecture en regard charge ses deux membres ET l'appareil ensemble** (`app/page.tsx`, `chargeePromise`), l'appareil recevant l'axe canonique en promesse comme en une colonne.

⚠️ Non traité, et consigné : le préchargement répété des liens de la barre (`prefetch={false}` couperait aussi le préchargement au survol, doc Next `link.md`), et le coût serveur lui-même.

**Ce que la mesure d'avant disait**, pour mémoire :
...
Les quatre points relevés (remonter au changement de chapitre ; reposer le verset d'une vue à l'autre ; effacer et faire paraître par blocs ; paralléliser le regard) sont ceux du dispositif décrit en tête de section. ⚠️ Le verset ne voyage PAS par l'adresse : `?verset=` sélectionne et sort du regard, et il ne fallait pas lui faire dire autre chose. Il voyage dans une référence du composant, qui ne se démonte pas ; une adresse rechargée ne reprend donc rien, et c'est voulu.
...
## La citation sortie, et les guillemets d'une locution coupée (2026-08-28)

- ⛔ **`.citation-sortie` a quitté le bloc `<style>` d'`OeuvreClient` pour `globals.css`.** La forme y vivait pour les ŒUVRES seules ; les introductions et apparats des bibles n'en avaient aucune, et l'Introduction générale de Fillion coulait dans son fil une citation de Stolberg de **438 signes**, guillemets compris. Une seule écriture, deux surfaces. ⛔ Ne restent dans `OeuvreClient` que les règles propres aux œuvres — surbrillance de survol, bloc structurel. *(⚠️ Aucun accent grave dans ce bloc `<style>` : il vit dans un littéral de gabarit. Une paire de backticks dans un commentaire CSS y casse le fichier entier — payé le 2026-08-28.)*
- **La règle est celle des œuvres**, `detecterCitationSortie` (charte § 3.8, cinquième règle) : isolée par un deux-points, longue de 400 signes, terminant son paragraphe. Elle vaut ici pour les paragraphes `kind: commentary` des blocs de nature **`introduction`** et **`notice`** seulement. ⛔ Pas pour un commentaire de péricope ou de verset : on y cite en une ligne, et le retrait l'y noierait. **Portée : 1 paragraphe sur les 3 221 du corpus** (`ig:p26`) — la règle est étroite, et c'est voulu.
- **`debutCitation`**, ajouté à `citationSortie.ts` : la position du contenu cité dans le texte SOURCE, pour que la page Bible — qui pose ses locutions marquées par OFFSET, là où la page d'œuvre les pose par marqueur — puisse les reporter sur la citation détachée. ⛔ `null` dès que la francisation a déplacé un signe (`“` → `« `, deux caractères pour un) : mieux vaut ne rien reporter que reporter de travers.
- ⛔ **On ne sort la citation que si TOUT ce qu'elle porte peut la suivre** : une locution à cheval sur la coupure la fait rester au fil du texte. Perdre une italique en silence vaudrait moins que de renoncer au retrait.
- ⚠️ **Les deux moitiés se rendent SANS `bloc`** : ses offsets de source valent pour le paragraphe entier et ne veulent plus rien dire une fois coupé. Les appels s'y posent par leur texte d'ancrage — les 66 notes de l'Introduction générale le portent toutes.
- ⛔ **Une locution entre guillemets n'ouvre sa paire qu'UNE fois.** Un appel de note tombé en son milieu la coupe en fragments, et chaque fragment reprenait sa paire : « les hommes de » « Dieu » là où l'édition écrit « les hommes de Dieu ». `envelopperSpan` reçoit désormais ses bornes (`ouvre`, `ferme`), et `rendreContenuAncre` retient les locutions déjà ouvertes.
...
## Le passage d'un texte à l'autre est FLUIDE, et il ne ramène pas au début (2026-09-02)

Demande de l'auteur, sans toucher aux données : pas de retour au début du texte quand le texte change ; le texte affiché s'efface de façon progressive et l'autre se reconstruit de façon fluide ; et le chargement va plus vite si possible. Tout ce qui traverse la coupure entre le composant qu'on quitte et celui qui arrive vit dans **`app/lib/passageTexte.ts`** (module pur pour sa part testable, 6 tests) ; le reste est dans `OeuvreClient` autour de `naviguer`, dans `page.tsx` autour de `resoudrePassage`, et dans `globals.css` (§ « Passage d'un texte à l'autre »).

- ⛔ **L'ADRESSE emporte la position de lecture** : `?niv1=` (le niveau qu'on lisait) et `?groupe=` (le groupe d'alignement du paragraphe en tête de fenêtre) ou, à défaut, `?cle=` (une clé de segment du texte original : celle que la copie lue portait en `cle_original`, ou la sienne propre si c'est l'original qu'on quitte). Le serveur retrouve le segment (`texte_alignement_membres` pour le groupe ; `segment_key` puis `segment_metadata->>original_segment_key` pour la clé), ouvre le texte sur SON niveau, et le client se pose dessus. ⚠️ Une reprise ne SÉLECTIONNE pas le segment (`cibleReprise`), à la différence d'un lien profond `?segment=` ; et elle ne vise que le CORPS, jamais l'apparat.
- ⚠️ **Le niveau nommé par l'adresse ne vaut que s'il existe dans le texte ouvert.** Mesuré en base le 2026-09-02 : sur onze œuvres à plusieurs textes, les clés de niveau 1 ne coïncident qu'une fois sur deux (« Liber I » d'un côté, « Livre premier » de l'autre). C'est l'alignement qui fait retomber au même passage ; le niveau n'est qu'un repli, et à défaut des deux on ouvre le premier niveau, comme avant. ⛔ Pas de repli par RANG (« le troisième niveau de l'autre texte ») : il tomberait à côté sans le dire, et un lecteur qui croit lire le même passage est pire qu'un lecteur ramené au début.
- **La MÉMOIRE DU MODULE emporte ce que l'adresse ne dirait pas sans l'enlaidir** : le défilement et la hauteur du paragraphe en tête (`annoncerBascule` / `reprendreBascule`). Une navigation de Next ne recharge pas le document, le module survit donc au démontage ; au chargement d'une adresse il est vide, et il n'y a pas d'arrivée. ⚠️ Une bascule se PÉRIME au bout de vingt secondes : une navigation interrompue ne doit pas s'appliquer au montage suivant venu. ⛔ Elle se CONSOMME dans le `useLayoutEffect` d'arrivée, jamais dans un initialiseur : le composant qu'on quitte se rend encore pendant l'attente, et il ne doit pas reprendre ce qu'il vient d'annoncer. L'initialiseur ne fait que LIRE (`basculeEnAttente`), pour que la classe d'arrivée soit là dès la première peinture.
- **`router.push(url, { scroll: false })`**, sinon le routeur remonte en haut. ⚠️ Une œuvre SŒUR change de route, et le `loading.tsx` remplace alors la page d'un coup : le départ y est différé de `DUREE_SORTIE_MS`, le temps de l'effacement, que rien ne verrait sinon. Un autre texte de la même œuvre part tout de suite : le routeur garde la page jusqu'à ce que l'autre soit prête. Le mot de l'écran d'attente ne vient qu'au bout de 450 ms (`cs-attente-paraitre`), une arrivée rapide ne montrant rien.
- **L'effacement et l'apparition se font PARAGRAPHE PAR PARAGRAPHE, de haut en bas**, et seulement sur ce qui est à l'écran : `ordonnerBlocsVisibles` donne son rang aux blocs FEUILLES visibles (`data-cs-bloc`, `--cs-ordre`, plafonné à dix), les animations de `globals.css` font le reste, et `prefers-reduced-motion` les éteint. ⛔ Aucune transformation sur `<main>` lui-même, seulement une opacité : il deviendrait le bloc conteneur des cellules d'actions posées en `fixed`.
- ⚠️ **Une navigation qui n'aboutit pas rend son texte au composant** : `sortie` retombe quand `navigation` s'éteint après avoir été vu allumé. C'est un `setState` dans un effet, et il est assumé : la fin d'une transition est un fait extérieur, pas une valeur qu'on recopie.
- **Le même passage joue pour les changements INTERNES** (demande de l'auteur, 2026-09-02) : changer de division par les flèches ou le sommaire (`changerNiv1`), tourner une page de pagination (`changerPage`). Le départ marque et efface (`departLocal`) ; le remplacement n'est appliqué qu'à la FIN de l'effacement (`finDeSortie`), d'un seul tenant, sinon la première page de l'ancienne division reparaissait entre-temps ; l'arrivée se joue dans un effet de mise en page sur le compteur `arrivee`, qui remonte d'abord à la barre du niveau puis marque, avant la première peinture. ⛔ Les flèches ‹ › n'emploient plus `conserverPosition` : elles gardaient la page de pagination et le défilement, si bien qu'une division plus courte que la page retenue s'ouvrait VIDE (`pages[pageActuelle] ?? []`). Changer de division remonte en haut, comme sur la Bible ; l'option ne sert plus qu'au rechargement après une correction d'administration.

**Ce que le chargement y gagne, sans une requête de plus.** Le passage visé et la première tranche du niveau partent AVANT la vague 2 (`promessePassage`, `promesseTranche`), la vague 2 ne servant plus qu'à confirmer que le niveau existe ; en texte entier, le corps part lui aussi dès la vague 1 (`promesseTexteEntier`). Les versets et la projection bilingue, qui ne dépendent que de la tranche, partent ensemble au lieu de se suivre. Et `chargerTrancheTexte` ne demande plus de `count: 'exact'` : elle regarde si le plafond est atteint, et retire alors la dernière ligne pour rester un PRÉFIXE (la tranche partielle fait donc 999 segments, non plus mille). ⛔ Une promesse lancée avant une vague et lue après porte son `catch` dès sa création : un rejet survenu pendant l'attente ferait sinon tomber le processus avant même qu'on le lise.

⚠️ **Contrôle du 2026-09-02** : types et 1 480 tests passés en empruntant les `node_modules` du worktree `bible-patristique-fillion-deploy` par une jonction posée puis retirée (`C:\Corpus Scriptura\node_modules`), les `node_modules` du dépôt étant ce jour-là un magasin pnpm sans liens. L'effet visuel n'a pu être vu qu'en ligne, après déploiement.
...
- ⛔ **La page de titre est celle de l'ÉDITION AFFICHÉE** (demande de l'auteur, 2026-09-08). L'identité — traducteur, adresse — se prend à `versionActive`, **son silence compris** : le repli champ par champ (`versionActive?.x ?? oeuvre.x`) faisait annoncer « Traduction par intelligence artificielle… » sur le latin de Bondurand, qui n'a pas de traducteur (dix-neuf textes du corpus étaient dans ce cas, tous en langue originale), et composait l'adresse de DEUX éditions (« Paris, Corpus Scriptura, 2026 » : Paris venait de Picard 1887). Une adresse se prend entière ou pas du tout ; l'œuvre ne parle qu'à défaut de version active, ou pour la version PAR DÉFAUT sans adresse propre. La règle vit dans `identiteEdition` (`versionTextuelle.ts`) et sert aussi la fiche d'édition, la citation et l'extraction. Doctrine : charte **§ 5.5**.
  - **Deux éditions à l'écran, deux mentions.** En bilingue, `versionEnRegard` nomme le texte établi AVANT la traduction (« Texte latin d'après l'édition de Paris, Alphonse Picard, 1887 »). ⚠️ Garde : `ensembleBilingue` — une colonne tirée du repli `segments.texte_original` n'est pas une autre édition, il n'y a rien de plus à nommer.
...
## ⛔ L'ÉCHELLE COMPTE CINQ RANGS, et le plafond doit tous les admettre (2026-08-31)

Doctrine : charte `parametres.charte_ia`, **§ 7.4**, sous « L'alinéa poétique se LIT ». Règles de code :

- ⛔ **`RANG_MAX` vaut 4, il valait 3, et c'était un rang de trop peu.** Une édition qui rentre ses vers au quart de pouce dispose de CINQ positions — le fer, puis `Em1` à `Em4`. ⚠️ **Un plafond ne borne pas une échelle, il ÉCRASE** : `Math.min(rang, RANG_MAX)` fait retomber sur le dernier rang tout ce qui dépasse, et deux niveaux que l'édition distingue se composent alors au même retrait.
- **Il ne se plafonne QU'À DEUX endroits, tous deux dans `compositionVers.ts`** : la fin de `niveauxAlinea` et `retraitVers`. Les quatre surfaces suivent d'elles-mêmes, `styleLigneDeVers` et `ComparaisonTraductions` passant tous deux par `retraitVers`. ⛔ Ne jamais réécrire un plafond ailleurs : ce serait rouvrir la dérive que la réunion du 29 août venait de fermer.
- **Le cas est le mètre XIV du Livre quatrième chez Mirandol**, seul poème du corpus où les quatre rentrées coexistent : 16 vers au fer, 5 à 0,25 pouce, 10 à 0,50, 4 à 0,75 et 6 à 1,00. Les six derniers se composaient comme les quatre précédents. ⛔ Les cinq niveaux sont attestés par le témoin et les 1 092 vers sont renseignés : **on ne rabat pas la donnée pour satisfaire un rendu**.
- ⚠️ **Le plafond faisait un SECOND travail, et il faut le compter avant de le hausser** : il bornait aussi le rabattage d'une océrisation bruitée. Chez Ceriziers 1646, dont les mesures sont continues, la construction des paliers monte jusqu'au **rang 6** ; le passage de 3 à 4 y déplace **99 vers sur 1 213, dans 13 poèmes**, d'un pas vers la droite, et change la décision de l'ombre de lettrine sur un poème. Mirandol n'en déplace que les six vers du mètre en cause. Relevé : `tmp/releve-plafond.mts`, qui importe la VRAIE fonction plutôt qu'une copie.
- ⚠️ **Le plafond agit AVANT `ombreDeLettrine`**, et peut donc changer sa décision : deux rangs de tête rabattus sur la même valeur cessent de « revenir plus à gauche ». Ce n'est pas une raison de les intervertir — le rendu écrase d'abord, corrige ensuite — mais c'en est une de les mesurer ensemble.
- ⚠️ **La largeur est MESURÉE, non ressentie**, comme celle des colonnes de la lecture en regard. `tmp/mesure-rang4.mjs` rend les 3 231 vers de Boèce un par un, sans enroulement, retrait compris. Le rang 4 vaut **7,5 em**, soit 97,5 px au corps de la lecture ; les six vers de rang 4 du mètre XIV demandent **221 à 243 px** quand la plus étroite colonne française en offre 354. Le nouveau plafond n'ajoute **aucun** enroulement en lecture ordinaire (500 px) ni en bilingue (354 px) ; il en ajoute **un** en traductions parallèles (403 px) et **six** sur mobile (362 px), tous chez Ceriziers. ⚠️ Ce n'est pas un hasard : une édition rentre les vers COURTS, et un retrait profond tombe donc sur une ligne brève.
- **Planche avant/après** : `tmp/planche-rang4.mjs`, qui rend le poème aux quatre mesures avec une règle d'un cran de 1,5 em en fond.

✅ **La recollation de Mirandol est FAITE (2026-08-31)**, et ce paragraphe disait l'inverse jusqu'à cette date : les 1 092 vers du corps portent tous `indent_inches` — 944 à 0, 23 à 0,25, 35 à 0,50, 57 à 0,75, 33 à 1,00 —, sans valeur manquante. ⚠️ Les **7 vers de l'apparat** n'en portent pas, et c'est hors du chantier : ne pas les renseigner d'office.
...
- **Posées au RENDU, jamais dans la donnée** (même doctrine que l'espacement, charte §3.2) : `cesurerLatin` s'applique au `texte_original` d'`OeuvreClient`, à la colonne en langue originale de `ComparaisonTraductions` et — depuis le 2026-08-30 — à la **colonne latine de la Polyglotte** (`texteCesure`, qui aiguille sur `lang`). La fonction est idempotente et ne change pas une lettre : `sansCesures(cesurerLatin(t)) === t`, garanti par test.
- ⛔ **La Polyglotte l'avait manqué, et c'est là qu'il manquait le plus.** Ses cellules sont justifiées (`.poly-texte-cell` : `text-align: justify`, `hyphens: auto`), le français y a son dictionnaire et le grec `cesurerGrec` — la Vulgate clémentine (36 046 versets) était donc la SEULE colonne justifiée sans aucun point de coupe, dans la page où les colonnes sont les plus étroites du site. Relevé à l'audit des mises en forme du 2026-08-30. ⚠️ La césure vient AVANT l'enrichissement, sans danger : la regex ne vise que des suites de lettres, jamais `**`, `++`, `^^` ni `<i>`, dont le nom de balise n'a qu'une lettre.
- ⛔ **Le garde-fou de copie ne connaît AUCUNE langue.** `copierSansCesures` (`app/lib/grec.ts`) retire tout U+00AD de la sélection, d'où qu'il vienne ; il s'appelait `copierSansCesuresGrecques` du temps où le grec était seul césuré, et ce nom faisait croire qu'une colonne latine césurée aurait besoin du sien. ⚠️ Il se pose sur CHAQUE cellule : la ligne surnuméraire de la Polyglotte ne le portait pas, et laissait déjà fuir les tirets grecs.
- ⛔ **La Polyglotte de la page RECHERCHE n'a pas reçu la césure latine, et c'est délibéré.** Elle a pourtant la même colonne justifiée, mais son texte passe par un surligneur qui travaille **par indices** (`surligneParts`) : un tiret conditionnel posé au milieu d'un mot empêcherait le terme cherché d'y être trouvé. ⚠️ Le grec y est déjà césuré avant surlignage — le même défaut s'y trouve donc, non traité.
...
- ⛔ **Le deux-points reçoit l'insécable pleine chasse — il ENTRE dans la conversion depuis le 2026-08-30.** Ce paragraphe disait l'inverse : le deux-points « n'entre pas dans la conversion », au motif que la règle de l'Imprimerie nationale lui donne déjà une insécable pleine chasse. Le motif valait tant que la DONNÉE la lui donnait, et c'était vrai de Sacy (11 081 versets), Crampon (8 608), Segond (7 578) et la Vulgate (18 211). Mais l'**AELF n'en porte aucune** — 10 954 versets à l'espace ordinaire, sécable — et la **Bible 899** non plus (556). « Laisser intact » y revenait donc à laisser le deux-points **passer à la ligne**, ce que la règle interdit précisément ; et l'AELF était la seule colonne de la Polyglotte où le cas se produise. On convertit désormais le TYPE, comme pour les autres signes : `ESPACES` → U+00A0 devant `:`, et la fine reste réservée à `;` `!` `?` et aux guillemets. Une fine déjà posée devant un deux-points est ramenée à l'insécable (1 261 segments patristiques) — c'est le même travail d'harmonisation, et il va dans le sens de la règle.
- ⚠️ **On CONVERTIT, on n'AJOUTE toujours pas.** « mot: suite » reste tel quel, et `https://` n'a pas d'espace devant son deux-points : la règle ne peut donc pas abîmer une adresse. C'est ce qui garde le remplacement caractère pour caractère.
- ⛔ **La colonne BIBLE 899 était la seule à n'y pas passer.** `normaliserEspaces` est appelée à l'entrée de `rendreTexteEnrichi` — mais TR0009 ne passe pas par lui : il porte des marqueurs éditoriaux (`rendreMarqueurs899`), non de l'enrichissement, et arrivait donc **brut**. C'est corrigé à l'entrée du tokeniseur. ⚠️ Ne jamais mettre là une fonction qui change la longueur (`normaliserTypographieLecture` en change) : l'automate se lit par indices. Et `\s` couvrant U+00A0 et U+202F, les marqueurs restent reconnus après normalisation — éprouvé par test.
...
- ⛔ **Une décision automatique n'est jamais une validation humaine** (charte § 23.12) : `AUTO_ACCEPT` écrit dans la couche candidate avec `validation_humaine: false`, forcé au point d'écriture (`entreeCorrection`/`traceCorr`). `ocr0` reste immuable ; tout est annulable.
...
- **`security_invoker = true` est la règle**, pour que la RLS de l’appelant s’applique. Trois exceptions assumées, écrites dans les migrations : `classement_utilisateurs` (la politique de `profils` est « soi-même » ; en invoker on ne verrait plus le score des AUTRES lecteurs), `v_bible899_verse_recomposed` (chantier en cours, ses segmentations non publiées disparaîtraient) et `essais_publies` (elle existe pour CACHER `user_id` d’une publication anonyme, ce qu’une politique de ligne ne sait pas faire ; la table `essais` ne se lisant plus qu’en propriétaire, en invoker la vue ne rendrait rien ; `security_barrier` en plus, pour qu’un filtre d’appelant ne descende pas sous son `where`).
...
- ⚠️ **La borne se passe en ÉLÉMENT**, jamais en nombre : `ancrer(el, cle, { borne })`. La Polyglotte passe la cellule de tableau elle-même, les traductions en regard remontent à `[data-colonne-comparaison]`. Une colonne étant aussi large que sa ligne, la cellule y passe toujours au-dessus — c'est le résultat voulu, non un effet de bord.
- ⛔ **La cellule porte une `key`, et le composant la pose LUI-MÊME** (`String(ancre.cle)`). C'est la règle du 3 septembre, ci-dessus : un objet qui se déplace d'une cible à l'autre voit l'état de ses boutons passer avec lui. Un appelant peut l'oublier ; le composant, non.
- ⚠️ **`donnees` est le second paramètre de type, et il est FACULTATIF.** La Polyglotte s'en sert parce que la donnée d'une cellule vit au fond de quatre boucles imbriquées et que la retrouver au-dehors demanderait quatre index de plus. Les deux autres surfaces retrouvent leur segment dans une `Map` et ne le passent pas. ⛔ La CLÉ reste comparable — c'est elle qui identifie la cible, jamais cet attirail.
- ⚠️ **La mesure passe par `useMesureAvantPeinture`**, l'alias de module que la bibliothèque emploie déjà : `useLayoutEffect` au navigateur, `useEffect` au rendu serveur. ⛔ L'alias DOIT être une constante de module nommée « use… », sinon la règle des hooks d'ESLint ne le reconnaît pas — et c'est aussi ce qui éteint `set-state-in-effect`, la mesure du document étant l'usage même de l'outil.
- ⚠️ **Le suivi au défilement écoute en CAPTURE sur la fenêtre** : un événement de défilement ne remonte pas, mais il descend, et c'est la seule façon d'entendre un défileur interne (la colonne de la page Bible, le corps de la Polyglotte) sans savoir lequel c'est. Une image par `requestAnimationFrame`, pas davantage.
- ⛔ **`sansSurvol` vient de `useSansSurvol`, jamais de `useEstMobile`.** L'axe est la CAPACITÉ DU POINTEUR : une tablette de 1024 px en paysage n'a pas de souris et `mobile` y est faux (charte, « LE DOIGT »). Les trois surfaces migrées y gagnent le tactile qu'elles n'avaient pas.
- ⛔ **UNE CELLULE EN PORTAIL NE REÇOIT PLUS LE CSS DE SA PAGE.** `.poly-act { opacity: 0 }` et `.seg-wrapper .seg-btn-action` exigent un ancêtre que le portail n'a pas : laissées en place, elles n'auraient RIEN atteint. Retirées avec ce qu'elles gardaient. ⚠️ Le piège vaut pour toute migration vers un portail, et il est SILENCIEUX : ni les types, ni les tests, ni la relecture de la feuille ne le voient.
- ⚠️ **`BTN_STYLE`, `VERSET_ACTION_BTN`, `BTN` et `ACT_BTN` ne sont plus que des noms** : les quatre pointent sur `STYLE_BOUTON_ACTION`. Les retirer demanderait de toucher leurs appelants ; les garder ne coûte rien et ne peut plus diverger.

- ⛔ **TROIS CÔTÉS, non deux : `positionCellule` sait descendre.** Quand ni la droite ni le dessus ne sont libres — une ligne au ras d'un en-tête collant —, la cellule passe DESSOUS. ⚠️ Elle a besoin du `bottom` du rectangle pour cela : sans lui (la forme d'origine, `{ top, right }`), elle garde le bornage au sommet. Le composant passe le `DOMRect` entier et un `pied` (le bas de la fenêtre), si bien que le troisième côté n'est retenu que s'il reste visible.
- ⚠️ **Le défaut ne s'est vu qu'à la MESURE, sur la page servie.** Les dix-sept cas d'alors passaient tous, contre-exemple du bridage compris : le trou tenait à ce que le `sommet` n'était qu'un garde-fou contre la barre de navigation, et qu'aucune surface ne lui passait le bas d'un en-tête collant. La Polyglotte le lui passe, et une branche jusque-là rare est devenue le CAS ORDINAIRE — le premier verset visible de la page. Recette : `dispatchEvent(new MouseEvent('mouseover', { bubbles: true }))` sans `relatedTarget`, puis comparer la boîte de `[data-cellule-actions]` aux `Range.getClientRects()` du texte survolé ; le compte de fragments recouverts doit valoir ZÉRO.

- ⛔ **LA LECTURE EN REGARD D'UNE ŒUVRE EST UNE GRILLE, ELLE AUSSI**, et elle emploie pourtant la cellule de la lecture ordinaire : c'est ce qui l'avait fait croire hors de cause. À droite d'un segment français il y a la colonne du latin. `colonneDuSegment` (OeuvreClient) retrouve la colonne comme l'enfant DIRECT de `[data-grille-bilingue]`, sans marquer les trois formes que le français y prend — paragraphe, bloc de vers, citation en versets —, car marquer trois formes fait trois endroits où l'oublier. ⚠️ **Une surface qui emploie déjà la bonne cellule n'est pas pour autant en règle** : ce qui décide est l'ESPACE qu'on lui donne, et il se lit dans la mise en page, non dans le composant.
- ⚠️ **« Traductions parallèles » est ÉTEINT** (`COMPARAISON_ACTIVE = false`, OeuvreClient) : le calcul bridé de `ComparaisonTraductions` n'atteignait donc aucun lecteur. Il est corrigé quand même — un mode qu'on rallumera ne se rallume pas avec son défaut —, mais il n'a pas pu être éprouvé en ligne. À faire le jour où il reparaît.

- ⛔ **DEUX MINUTEURS, ET ILS NE FONT PAS LE MÊME OFFICE.** `GRACE_SURVOL_MS` (400) laisse le temps d'aller du texte jusqu'aux boutons ; `DELAI_REANCRAGE_MS` (140) exige un temps de POSE avant qu'une cellule déjà ouverte ne se déplace vers une autre cible. Sans le second, la cellule s'enfuit : posée au-dessus du texte, elle occupe la ligne d'au-dessus, qui est survolable — monter vers les boutons la faisait remonter d'un rang. ⚠️ Rien n'attend quand rien n'est ouvert, et `basculer` (le tap) ne s'attend jamais.
- ⛔ **LA FERMETURE NE VISE PAS UNE CLÉ**, elle ferme ce qui est ouvert. Traverser un voisin appelle `relacher` avec la clé du VOISIN, et un minuteur retenu sur cette clé-là ne fermerait jamais la cellule qu'on vient de quitter. C'était un défaut latent de la première écriture.
...
- **Libellé du traducteur** : toute la mise en forme vit dans `app/lib/traducteurs.ts` (module pur, testé `traducteurs.test.ts`), ré-exportée par `PageTitre.tsx`. Elle sert la page de titre, la bibliothèque, les notices du catalogue et le sélecteur de version. Ne pas recomposer « Traduction par … » ailleurs.
  ...
  - ⛔ **Une mention de RÉGIME n'est pas un nom non plus** (relevé de l'auteur, 2026-09-08). « Traduction IA — Corpus Scriptura » (Dhuoda, seule œuvre du corpus à la porter) donnait « Traduction **par Traduction IA** — Corpus Scriptura » en page de titre. Le site la rédige : « **Traduction par intelligence artificielle sous la direction de Corpus Scriptura** », et en bas de casse sans « trad. » dans une ligne bibliographique, comme une formule de direction. ⚠️ Même tic ailleurs : `labelCourtVersion` prenait le dernier mot du champ pour un patronyme et intitulait la colonne française du texte en regard « Scriptura 2026 » en face de « Bondurand 1887 » ; `estTraductionMachine()` est là pour tout appelant qui découpe un nom. Doctrine : charte **§ 16.7**.
...
- ⛔ **Un appel ne prend PAS l'alinéa du bloc qui le porte** (2026-09-07). L'autre revers de l'`inline-block` : `text-indent` s'hérite, et un `inline-block` est un conteneur de BLOC — il applique donc le retrait à sa propre première ligne, la seule qu'il ait. Dans une ligne de vers, dont le retrait de suite vaut `-1.15em` (`styleLigneDeVers`), le chiffre était tiré **19 px à GAUCHE** de sa boîte, par-dessus les lettres qui le précèdent, tandis que la boîte, vidée de sa chasse, se refermait sur ses **2 px** de rembourrage (12,86 px une fois corrigée) et laissait le texte d'après se recoller par-dessus. Mesuré sur le *Manuel* de Dhuoda : les **83** appels de la division des Prolégomènes étaient tous illisibles, l'appel imprimé dans le mot. `styleAppelNote` pose donc `textIndent: 0`. ⚠️ Le mal n'est pas propre au vers — le retrait suspendu des bibliographies (`-1.1em`) et l'alinéa positif des essais (`0.9em`) l'atteignaient de même : la remise à zéro appartient à la FORME de l'appel, pas au bloc de vers.
...
**Les copies de la même liste.** `NATURES_CORPS` et `NATURES_APPARAT` (`app/lib/oeuvreSelects.ts`) décident ce qui paraît. Mais la même liste vivait recopiée dans la requête d'apparat du rendu serveur, dans celle du rechargement client, et — les plus difficiles à voir — **dans les fonctions `get_niv1_texte` et `get_niv1_list`, en base**. Chacune pouvait dériver seule.
...
**Règle** : une liste de natures ne s'écrit QUE dans `app/lib/oeuvreSelects.ts`. Tout autre endroit qui en a besoin l'importe, ou — s'il vit en SQL — est **relu par une garde** qui le compare au fichier. `oeuvreSelects.test.ts` refuse une égalité sur une seule nature d'apparat dans les deux requêtes de la page et relit les dernières migrations qui définissent `get_niv1_texte` et `get_niv1_list` pour exiger le miroir exact de `NATURES_CORPS`.
...
⚠️ Le partage entre les deux listes est celui de l'**auteur** et de l'**éditeur**, non celui du texte et de l'appareil : `apparat_auteur` se lit au corps, à sa place ; `apparat_editeur` se lit à l'apparat. `ref_niv1` est un rattachement partagé, pas une clé de déduplication entre surfaces : un segment d'apparat reste dans l'apparat même si le corps porte le même niveau 1. Inversement, `get_niv1_list` ne projette que le corps et exclut toujours `espace_textuel = 'apparat_critique'`.
...
- **Le style se donne AU SURVOL, et le clic l'épingle** (proposition de l'auteur, 2026-08-29). Toutes les légendes ensemble ne tenaient pas : posées au haut de leur unité, elles se recouvraient dès qu'une unité était plus courte que sa notice — 30 px contre 150. Une seule à la fois, celle qu'on désigne, et le problème disparaît au lieu d'être contourné. ⛔ *(Une règle de manchettes qui les faisait glisser vers le bas a existé une heure ; elle est tombée avec la refonte, et c'est mieux ainsi.)*
- ⚠️ **Deux blancs réglés sur la planche même** (2026-08-29). Sous un **titre T4**, le blanc se referme : ce qui suit lui appartient. ⛔ Raccourcir la marge du titre ne suffit PAS — les marges verticales se fondent et la plus grande l'emporte, celle du bloc suivant. On ferme donc des deux côtés, par une règle de voisinage `:has`. Mesuré : **36 px → 16,8**. Et la **gouttière de la manchette** passe de 1,25 à 0,9 rem (**20 px → 14,4**) : plus large, elle détachait le repère de son développement au lieu de le lui faire coiffer.
- ⛔ **La désignation n'écrit RIEN dans le texte** : ni classe ni style sur les nœuds de l'épreuve, mais un rectangle peint par-dessus, hors du flux et sans événements. Toucher aux nœuds, ce serait modifier la composition que la planche montre. L'unité survolée se retrouve par délégation d'événement — on remonte jusqu'à l'enfant direct de la colonne, dont le rang donne l'unité.
...
- **Une note peut appartenir au français, au latin, ou aux deux** (`applies_to` / `applies_to_member_id`). En lecture bilingue, les blocs communs à l'édition se rendent **hors des colonnes**, avant ou après elles, à la mesure du bloc de lecture (voir ci-dessous, 2026-09-03) ; ils ne sont jamais dupliqués dans chaque colonne.
...
- **Lecture « Latin-français » de la page Bible** (`?bilingue=1`). Elle ne passe pas par `TexteBible` : `BibleLayout` rend `LectureBilingueBible`, qui reprend le même châssis — en-tête, navigation de chapitre, zone de défilement — pour que le passage d'un mode à l'autre ne déplace rien à l'écran. Le corps est `BibleBilingue`, et toute la décision vit dans `app/lib/bibleEditionBilingue.ts`, module pur et testé.
  ...
  - ⛔ **Un bloc du corps IGNORE les colonnes** (décision de l'auteur, 2026-08-20). Commun à l'édition ou propre à une langue, il sort des colonnes. Les introductions et commentaires de Fillion n'ont pas d'équivalent latin : les enfermer dans la colonne française laissait en face une colonne vide de leur hauteur. L'appartenance (`applies_to`) reste une donnée de PROVENANCE, elle n'est plus une consigne de mise en page.
  - ⛔ **MAIS IL NE PREND PAS TOUTE LEUR LARGEUR** (décision de l'auteur, 2026-09-03, revenant sur la pleine largeur du 20 août : « toute la largeur, c'est trop, pas naturel pour un corps de texte ; il faut, pour ces styles-là, réduire la largeur maximale »). Mesuré sur un écran de 2 560 px, racine 22 : le paragraphe d'introduction de la Genèse faisait **124 signes par ligne** sur les 52 rem des deux colonnes, contre 83 sur les 31,25 rem de la lecture simple ; tout l'appareil — introductions, commentaires, titres, gravures — était composé sur 52 rem. `BibleBilingue` a d'abord enveloppé chaque bloc, chaque gravure et la série des notes dans `.cs-bible-regard` (`surMesure`), bornée à `--mesure-bloc` ; voir les deux points suivants. ⚠️ Une gravure y retrouve sa taille : sa part se calcule sur son conteneur, et sur 52 rem une planche hors-texte dépassait son fichier. ⛔ Pas sur mobile, où les colonnes sont empilées. ⚠️ Deux colonnes de commentaire à la manière du fac-similé (multicol) ont été maquettées le même jour et écartées au profit de la mesure bornée : la pleine largeur elle-même était le défaut.
  - ⛔ **LE RETRAIT DÉSIGNE LE VERSET, EN REGARD AUSSI** (décision de l'auteur, 2026-09-03, le soir, devant le premier résultat : « les versets dépassent trop ; élargir le corps du texte, et réduire la largeur des versets bibliques, harmonieusement »). Les versets prennent la mesure de la PAGE (`--mesure-page`, 38,75 rem, dans `LectureBilingueBible`) au lieu des 52 rem des œuvres, et se posent sur l'AXE DU TEXTE : la grille réserve la gouttière d'actions de 2,375 rem comme l'en-tête, si bien que le titre et le corps partagent enfin leur axe (mesuré avant : 1 176 contre 1 203 px, et le passage simple ↔ bilingue déplaçait le texte sans déplacer le titre). L'appareil est bordé par le FER DU TEXTE des versets : en regard, `.cs-bible-bloc` vaut la page moins, de chaque côté, la colonne du numéro et sa gouttière — `--regard-numero` (1,6 rem) et `--regard-numero-gouttiere` (0,3125 rem), nommées dans `:root` et employées par `BibleBilingue` pour composer la rangée, une seule écriture ; une gravure et la série des notes prennent l'enveloppe `.cs-bible-regard`, de même mesure. Les numéros pendent dans la marge, à gauche comme à droite, et il n'y a qu'un fer. Mesuré sur 2 560 px : versets 1 144 → 853 px, appareil 688 → 768 (34,9 rem, environ 90 signes par ligne), fers confondus au pixel. ⚠️ Un numéro plus large que sa colonne (« 27, 58 », « 150, 6 ») pousse le texte de sa rangée de quelques pixels : la colonne est `auto`, et c'est voulu, un numéro ne se coupe pas. ⚠️ La planche hors-texte déborde de l'enveloppe jusqu'à la page par des marges négatives PROPRES (`.cs-bible-regard > .cs-bible-gravure--hors-texte`) : celles de l'axe, calculées sur `--mesure-bloc`, la décaleraient. ⚠️ La lecture en regard des ŒUVRES reste à 42 rem (2026-08-30, les vers de Boèce) : la Bible pose des commentaires entre ses versets, elle n'a pas la même contrainte.
  - ⛔ **UN BLOC NE S'ENVELOPPE PAS EN REGARD** (2026-09-03, le soir). L'enveloppe posée l'après-midi autour de chaque bloc a coupé en silence les règles de voisinage de la mesure étroite, qui portent sur des FRÈRES (`.cs-bible-bloc.cs-bible-title--t6 + .cs-bible-bloc`, `.cs-bible-bloc.cs-bible-block--title + .cs-bible-bloc.cs-bible-block--title`…) et qui faisaient les blancs de la lecture en regard depuis toujours. Relevé de l'auteur : « un peu trop d'espace » sous « 1. Le premier jour (1, 3-5) » et « 2. L'œuvre des six jours ». Mesuré : **1,5 rem au lieu de 0,5** sous le titre de péricope (l'introduction qui suit reprenait sa marge propre), **4 rem au lieu de 2,25** entre les deux titres (le second rouvrait son rang entier). La mesure se pose donc sur le bloc lui-même, qui reste frère de ses voisins ; seules une gravure et la série des notes gardent l'enveloppe, qu'aucune règle de voisinage ne nomme. C'est le § 35.17.3 pris par un quatrième bout : **une enveloppe est une surface de plus, et une règle de blanc ne la connaît pas** — avant d'envelopper un bloc, relire les règles `+` et `:has(+ …)` qui le concernent.
  - ⛔ **UN BLOC DE SUITE NE ROUVRE PAS LE BLANC DE SON RANG** (relevé de l'auteur, 2026-09-03, sur l'introduction de la Genèse : « les blancs entre les paragraphes de même style sont trop importants »). Fillion range une introduction ou un commentaire en autant de blocs que de paragraphes, et chacun rouvrait le blanc de son rang : **50 px en regard** (le « changement d'unité » de `.cs-bible-block--commentary`), **63 en lecture simple** (1,1 + 1,5 rem, additifs sur la grille), là où deux paragraphes d'un même bloc s'écartent de 0,25 rem. Mesuré dans le corpus public : 1 517 paires de `commentaire` I5, 152 de I6, 44 + 26 de I3 (introductions), dont 191 + 8 + 70 sous la même ancre. La suite est un fait de DONNÉE — `estSuiteDuBloc` (`bibleHierarchieSemantique.ts`, pur, testé) : même rang, même nature après résolution du registre, second bloc sans intitulé — portée par `data-suite` sur le bloc (`BlocEditorialBible`, prop `suite`, posée par `BibleBilingue` et `TexteBible`, qui seuls connaissent le bloc d'avant ; une gravure intercalée rompt la suite). La feuille retire au bloc de suite sa marge haute et à son prédécesseur sa marge basse, sur les TROIS surfaces ; reste le 0,25 rem du dernier paragraphe. ⛔ Ne pas étendre aux titres ni aux blocs à intitulé : une manchette ouvre une unité, et c'est le blanc d'unité qu'elle mérite.
  - ⚠️ **Le blanc sous une sous-section (T4) vaut 0,6 rem sur la grille, non 0,4 comme la feuille l'écrivait, et la fratrie ne donnait que 0,2** : le titre ET le bloc qui le porte reçoivent la marge basse du rang, et un bloc de grille est un contexte à lui (rien n'y fusionne) quand la fratrie fusionne tout. Relevé de l'auteur en regard, sous « Les sources de la Genèse » : « il faut plus de blanc après ce style ». La règle de fratrie passe à 0,6 rem ; les deux surfaces se valent. ⚠️ Le même écart existe sous T6 (0,5 sur la grille, 0,25 en fratrie) et entre deux titres qui se suivent (2,25 contre 1,75) ; laissé tel quel, l'auteur ayant jugé ces blancs-là en regard le même jour.
...
- ⛔ **LE PAPIER SE NETTOIE CHIRURGICALEMENT** (charte § 49.16). Ni l'étalement
  ...
  `nettoyerLePapier` / `dilater` dans `reduire-planches.mjs` — la dilatation du
  masque est SÉPARABLE, sinon 49 lectures par pixel sur une planche de 2 959 px.
  Moucheture 0,47 et encre ×0,95 contre 0,77 / ×0,87 pour le plancher dur : il gagne
  sur les DEUX critères, et c'est cela qui le choisit. Nettoyage à PLEINE résolution,
  avant la réduction.
- ⚠️ **L'œil ne voit pas une perte PROPORTIONNELLE.** Le plancher dur avait été
  contrôlé à l'œil sur les quatre cas extrêmes et paraissait intact ; il manquait un
  dixième de l'encre. La mesure est `tmp/planches-temoin.mjs` : densité d'encre du
  fichier servi contre celle du master réduit à la même taille, échelle neutre.
...
### L'affichage des gravures, revu avec l'auteur (2026-09-02)

Sept points relevés en ligne, cinq réglés, deux qui relèvent de la donnée.

- **La légende IMPRIMÉE passe devant l'éditoriale** (`app/page.tsx`, `caption: printed ?? editorial`). ⚠️ Sans effet visible tant que les 32 planches du tome I n'ont pas de `printed_caption` transcrite : leur `editorial_caption` est une description d'atelier (« Scène de caravane orientale ; planche tournée. ») qui double la légende gravée dans la planche et dit le TRAVAIL (« tournée ») au lecteur. Marc (tome VII) et 1 Samuel (tome II) portent l'imprimée. ⛔ C'est une correction de DONNÉE, côté GPT : transcrire les légendes imprimées du tome I, et sortir « planche tournée » de la légende vers `metadata`.
- **Le nom accessible ne se répète plus** : la légende nomme la figure (`aria-labelledby` vers le `figcaption`) ; le texte alternatif ne reste que s'il DÉCRIT (Marc : « Un souverain assis regarde… »), et la légende le décrit alors en second (`aria-describedby`). Un `alt` égal à la légende s'efface.
- **Une vignette s'agrandit aussi**, par le même `GravureAgrandissable`, le masque rendu en grand avec l'encre du thème. ⛔ **Jamais au-delà de sa taille de fichier** (décision de l'auteur : « pas immense si la qualité ne le justifie pas ») : le fichier fait 1,67 fois l'affichage, l'agrandissement est modeste, et c'est voulu.
- ⛔ **ON NE ROGNE PAS LES PLANCHES : ELLES LE SONT DÉJÀ.** L'audit du jour affirmait qu'un quart de la planche de Genèse 24 partait en papier. FAUX, et prouvé par une planche de contrôle avant/après : sur les 32 planches du tome I, l'encre occupe 96 à 100 % du fichier servi. Un rognage à la boîte d'encre a été écrit dans `reduire-planches.mjs` puis retiré le jour même. Ce qui les fait paraître petites est leur format PAYSAGE (28 sur 32) dans les 500 px du bloc : **une planche hors-texte prend désormais la mesure de la PAGE** (`.cs-bible-axe > .cs-bible-gravure--hors-texte`, écran large seulement, `!important` contre la part en ligne), centrée sur l'axe du texte comme les trois grandes gravures du site. Mesuré : 500 → 620 px à la racine 16. ⚠️ Cette règle est la seule exception au « conteneur = le bloc » de la section suivante, et elle est nommée.
- **Chargement différé natif** (`loading="lazy"`, `decoding="async"`) sur les `<img>` des planches et photogravures, `width`/`height` posés. ⚠️ Pas sur les MASQUES : une image de masque CSS n'a pas de différé, et un différé par script laisserait la vignette vide au premier écran le temps de l'hydratation ; elles pèsent 30 à 55 Ko, trois par chapitre au plus.
- **Cache d'un an, adresses versionnées.** Les 242 objets du seau `bible-illustrations-web` portent `cacheControl: max-age=31536000` (métadonnée `storage.objects`, passée de 3600), et `reduire-planches.mjs` dépose désormais avec `cacheControl: '31536000'`. ⛔ Ce cache n'est sûr que parce que l'adresse porte l'empreinte du fichier : `adresseVersionnee(public_uri, web_sha256)` (`app/lib/bibleEdition.ts`, testée) ajoute `?v=<12 hex>`. Une illustration remplacée par la chaîne (qui reporte `sha256` en base) est une adresse neuve, et l'ancienne peut rester en cache un an sans nuire. ⚠️ Toute chaîne qui déposerait un fichier SANS reporter `sha256` servirait l'ancien pendant un an : le report n'est plus un ornement. ⚠️ `detourer-gravures.mjs` n'a pas reçu `cacheControl` (fichier en chantier, non versionné ce jour) : à faire au prochain passage.
- ⛔ **`reduire-planches.mjs --televerser` exige `--seulement <tome>`** : la table `bible_edition_assets` range en `plate` vingt-trois gravures de 1 Samuel servies par la chaîne des VIGNETTES (4.8.0) à leur largeur imprimée (316 à 633 px) ; sans filtre, le script les regonflait toutes à 880 px. Vu à la mesure, avant tout dépôt.
- **`srcset` — testé, non retenu.** Le service de transformation du seau répond (`/storage/v1/render/image/public/...?width=213`) : 37 Ko contre 56 pour une vignette à 1×, rien au-dessus de la taille du fichier, qui reste le plafond. Le gain ne porte que sur les écrans 1× et se paie en « origin images » au-delà du quota du plan. Écarté : sobre d'abord ; le jour où l'on voudra des variantes, la chaîne les fabriquera en fichiers.
...
- ⛔ **La clé SQL PERDAIT ses capitales accentuées**, et depuis toujours : elle translittérait AVANT de passer en minuscules, si bien qu’une capitale accentuée traversait un jeu de départ tout en minuscules, puis tombait sous `[^a-z0-9]`. « Éditions du Cerf » se rangeait sous « ditions du cerf ». Deux graphies d’une même maison, l’une accentuée et l’autre non, ne se rejoignaient donc pas — l’office même d’une clé — et la base ne fusionnait pas ce que l’écran annonçait. Corrigé par `sql/20260829_cle_editeur_capitales_accentuees.sql` : **71 noms atteints par référentiel**, aucune fiche confondue, aucune fusion nouvelle (mesuré avant application). ⚠️ `internal.normaliser_cle_bibliographique`, qui calcule `editeurs_valeur.cle_normalisee`, ne portait pas le défaut : elle nomme les deux casses dans son jeu de départ. Le défaut ne se voit qu’en comparant les deux écritures sur une capitale accentuée, ce qu’aucun test ne faisait ; il en existe un désormais.
- ⛔ **La résolution lit la forme ENTIÈRE avant de découper sur « ; » et « / »** (`normaliserNomEditeur`, `editeursDuSegment`). Découper d’abord rendait introuvable toute variante qui porte un point-virgule : la variante était bien en base et n’avait AUCUN effet à l’affichage. Le découpage en co-éditeurs n’est qu’un repli. Même correction dans la file « à normaliser » de l’écran, qui faisait reparaître les deux moitiés d’une graphie répertoriée.
- **La route `/api/admin/editeurs` ne fusionne pas, elle RELÈVE et PROPAGE** : elle note ce qui va être absorbé (avant l’écriture, puisque après ces fiches n’existent plus) pour le dire à l’écran, et porte la déclaration dans `editeurs_valeur` quand l’autorité correspondante y existe. ⛔ Elle n’en crée aucune. Le code `ZE001` d’une collision rend un **409** avec le message du déclencheur, jamais un 500 muet.
- ⚠️ **Une note de valeur académique qui change de fiche EMPORTE sa provenance.** Le `coalesce` gardait celle de la fiche d’accueil : « Éditions du Cerf » s’est retrouvée notée 2 par la consolidation du 5 août sous la signature de l’inventaire qui l’avait créée. Corrigé le jour même, la seule fiche atteinte avec.
- **Passe rétroactive** : `scripts/editeurs-fusion-autorites-20260829.mjs` (`--dry` pour le plan seul). Elle ne décide rien — elle réécrit les fiches et laisse les déclencheurs faire —, écrit l’état d’avant dans `sql/rollback_fusion_autorites_editeurs_20260829.sql`, et prend son plan dans la base (`variantes_editeurs_disputees`, `propagation_editeurs_a_faire`, `autorites_editeurs_a_fusionner`) plutôt que de recoder une clé.
- ⚠️ **Une fonction de plan qui calcule la clé DANS sa condition de jointure franchit les 8 s de PostgREST** : le planificateur ne peut que comparer chaque fiche à toutes les autres. Les clés se calculent une fois, en CTE, et les jointures redeviennent des jointures de hachage.
...
Doctrine : charte `parametres.charte_ia`, **§ 12.1** (l'original embarqué est une forme héritée, en extinction) et **§ 12.2** (alignement n'est pas paragraphage). Corrigés le 2026-08-24 par `scripts/charte-corriger-bilingue-alignement-2026-08-24.mjs`, puis le 2026-09-07 par `scripts/charte-alignement-nest-pas-paragraphage-2026-09-07.mjs` ; chacun applique la même table de remplacements aux DEUX exemplaires de la charte et refuse d'écrire si l'un des motifs n'y figure pas exactement une fois.

⛔ **Un texte en langue originale n'existe qu'à UN SEUL endroit : dans ses propres `segments`, sous son propre `id_texte`.** Règle posée par l'auteur — « je veux une seule occurrence de chaque traduction ; il faut que chaque traduction latine soit considérée comme une œuvre, mais aussi qu'à partir d'un même texte on puisse aligner avec la traduction ». La correspondance vit dans `texte_alignement_ensembles` / `texte_alignements` / `texte_alignement_membres`.

Toute la règle vit dans `app/oeuvre/[id]/bilingueAlignement.ts` (module pur, 47 tests), employé par le rendu serveur ET par le rechargement client — un seul chargeur, `chargerProjectionBilingue`, sinon les deux dérivent.

- ⛔ **ALIGNEMENT N'EST PAS PARAGRAPHAGE, ET LE PARAGRAPHE NE SUFFIT PAS NON PLUS.** Les deux moitiés de la règle se sont chassées l'une l'autre en un jour, le 2026-09-07. Le **paragraphe COMPOSE** : lui seul pose un filet, un blanc et un alinéa, sur la clé éditoriale entière — `espace_textuel`, `ref_niv*`, `paragraphe`, segments rangés par `rang` et joints par `join_before`. Le **groupe MET EN REGARD** : lui seul tient les deux colonnes en face l'une de l'autre, et il lui faut pour cela son propre rang de grille. `repartirGroupes` découpe donc aux DEUX.
- ⛔ **LA COUTURE : les rangs d'un même paragraphe se TOUCHENT** (`.para-bilingue--couture`) — ni blanc de paragraphe, ni retrait, ni `<p>` ; seul le dernier rang ferme le paragraphe (`BlocEnRegard.clot`). ⚠️ Deux classes au sélecteur, sinon `.para-bilingue > p` l'emporterait. ⚠️ Il reste une coupure de LIGNE à chaque empan, et elle est irréductible : aucune écriture CSS ne fait couler un texte d'un rang de grille au suivant en gardant deux colonnes accordées.
- ⛔ **DEUX RANGS DE SÉPARATEUR, et l'empan garde le sien** (2026-09-07, le soir). Cousu sans AUCUNE marque, l'empan cessait d'être visible et la page en regard redevenait un mur — relevé de l'auteur le jour même, et il est juste : *un empan qu'on ne voit pas ne met plus rien en regard.* Le filet PLEIN ferme le paragraphe, le filet PÂLE marque l'empan. Mesuré sur la composition réelle : **0,55 de teinte et 27,2 px de blanc** contre **0,22 et 5,76 px** — un rapport de 2,5 sur la teinte, de 4,7 sur le blanc. ⚠️ La COULEUR seule est reprise (`border-bottom-color`) : le style et l'épaisseur viennent de `.para-bilingue`, et `.para-bilingue--vers`, qui pose `none`, garde la main — une strophe ne se sépare toujours pas par un filet. Valeur choisie sur planche (`tmp/planche-filet-empan.html`), puis vérifiée par mesure.
- ⚠️ **Les deux échecs, pour n'y pas revenir.** Découper au GROUPE seul faisait de chaque empan un paragraphe : les 76 groupes du Discours 38 rendaient 76 filets et 76 blancs sur un corps qui n'en a que deux, la plupart ouverts en minuscule (« ces choses là… », « aussi ont faict les Juifs… »). Découper au PARAGRAPHE seul fondait ces 76 empans en un rang unique : plus un filet, mais plus rien en regard non plus. ⛔ Et l'on ne « corrige » pas une minuscule en base pour masquer un défaut d'affichage.
- ⚠️ **Contrôle sur les données réelles** (`tmp/controle-blocs-bilingues.mts`, qui rejoue les deux découpes avec les vraies fonctions), en rangs puis en filets : Discours 38, **77 rangs pour 3 filets** — l'alignement est rendu tel quel, les faux paragraphes ont disparu ; Cité de Dieu, 2 398 pour 1 064 ; Boèce, 1 142 pour 475 ; Hexaéméron, 325 pour 194 ; Confessions inchangées, 943 pour 943. Là où l'alignement est plus GROSSIER que le paragraphage, la découpe rend les coupures que l'édition demande : Heptateuque, 786 rangs dont 133 à colonne vide.
- ⚠️ **Le tri par `rang` est celui de `paragraphesDe`** : le rang de grille subdivisant un paragraphe, `rang` y est bien l'ordre de lecture, et il ne peut plus mêler deux moitiés de groupe. Côté original, c'est toujours `member_order` qui fait foi. ⛔ `repartirGroupes` juge l'appartenance d'une borne sur les ids du RANG, jamais par la position `ids[0]` : un bloc trié par `rang` et des bornes prises dans l'ordre de lecture ne se comparent pas par index.
- ⚠️ **`fondreOriginaux` ne sert plus qu'au POÈME refait** (`fusionnerBlocsDeVers` réunit les rangs de vers voisins et leurs groupes) : un rang de prose ne porte qu'un groupe. Il joint néanmoins les VERS par un saut de ligne et la PROSE par `join_before` — d'où `BlocOriginal.joinBefore`, le liant du premier segment du groupe —, la jonction se jugeant bloc par bloc sur `toutVers`, car un poème peut mêler les deux.
...
### ⛔ Un groupe qui enjambe deux BLOCS ne compose son original QU'UNE fois

Relevé en ligne le 2026-08-24, sur la Didachè. Les sections se rendent séparément, chacune sous son titre, et la découpe en blocs ne peut donc pas réunir deux paragraphes français que sépare un titre. Le groupe `PAR:003` couvre pourtant les deux : son grec se recomposait **dans chaque section traversée**, une fois en regard de « Et voici l'enseignement… », une fois en regard de « Abstiens-toi… ». Le lecteur voyait le même grec deux fois de suite. ⚠️ Depuis que le bloc est le PARAGRAPHE (2026-09-07), le cas ne demande plus deux sections : un empan à cheval sur deux paragraphes suffit, et c'est ce que la donnée fait encore là où l'alignement est plus grossier que le paragraphage.

`bornesDesGroupes` donne le PREMIER et le DERNIER segment de chaque groupe dans l’ordre de lecture, et il en faut bien deux :

- le **premier** porte l’original ; `repartirGroupes` ne compose donc un groupe que dans le bloc qui porte sa borne de tête. Les blocs suivants gardent leur grille, colonne de droite vide : le français ne doit pas reprendre toute la largeur au milieu d’un empan, sans quoi la mise en regard se défait à l’œil. Même règle que la lecture bilingue de la Bible, où un créneau qu’une édition ne porte pas reste vide. ⛔ C'est `BlocEnRegard.couvert` qui garde cette grille, jamais la présence d'un original — et `originalEnRegard` ne retombe alors PAS sur la copie `texte_original`, qui redirait ce que la colonne porte déjà plus haut.
- le **dernier** porte le FILET (`BlocEnRegard.clot`). Celui-ci marque l’appariement empan par empan : tiré entre deux blocs d’un même groupe, il annonce une frontière que l’alignement ne reconnaît pas, et les deux moitiés d’un empan se lisent comme deux empans. `.para-bilingue--suite` l’ôte aux autres et resserre le blanc au lieu de l’ouvrir. ⚠️ Le paragraphe, lui, se sépare quand même : le `<p>` garde son propre blanc.

⚠️ **Les bornes se comptent sur TOUTE la page, jamais sur un seul groupe structurel** : c'est ce qui permet à `repartirGroupes` de savoir qu'un empan vient de plus haut, quand la page rend groupe structurel par groupe structurel.

⛔ **Et « TOUTE la page » comprend les INTRODUCTIONS** (2026-09-07, le soir). Elles sont hissées en tête, hors des groupes structurels, donc absentes de `segmentsFiltres` : leur empan n'avait AUCUNE borne, et `repartirGroupes` appliquait son repli — composer dans chaque rang touché —, juste tant qu'on ne sait rien du groupe, ruineux ici. Le chunk de départ d'un argument étant le SEGMENT et non le paragraphe (« un argument fait bloc à lui seul »), l'original se recopiait autant de fois que le groupe compte de segments : sur le *Manuel* de Dhuoda, dont les **94** segments français sont tous des introductions, l'épigramme tient un seul groupe de **quatorze** vers, et la colonne latine portait quatorze fois la strophe entière — chaque vers latin quatorze fois. `bornesGroupes` se compte donc sur `[...introsEnTete, ...segmentsFiltres]`, c'est-à-dire EXACTEMENT ce que la page rend, dans l'ordre où elle le rend, et la boucle des arguments lit la MÊME liste au lieu d'en refiltrer une seconde : deux listes qui doivent s'accorder finissent par diverger. ⚠️ Les introductions ne sont rendues que sur la PREMIÈRE page — les compter ailleurs viderait la colonne d'un empan que la page compose pourtant.

⚠️ **Le défaut ne se voyait ni aux types, ni aux tests, ni dans la donnée** : les 57 groupes sont complets et sans orphelin, et la projection est juste. Il naît de la rencontre entre l'alignement et le découpage du RENDU, et il a fallu regarder la page.
...
⛔ **La teinte des Pères est LE MAROQUIN DE LA PAGE D’ACCUEIL (`#5a2a26`)** — charte § 38.6, décision de l’auteur du 2026-09-04 : « utiliser plutôt le maroquin de la page d’accueil pour les résultats liés à la patristique ». Le carton `.ac-patristique` d’`AccueilCards` porte cette reliure depuis le 31 août, et une matière ne se nomme pas de deux façons selon l’écran. Elle est à 123° du vert et à 53° de l’or (ΔE 35,6 à 47,8 entre les trois familles), et à ΔE 30,9 de `--cs-danger-fonce`, qui partage sa teinte (31°) mais non sa clarté ni sa chroma.

⚠️ **Elle est plus PROFONDE que ses deux sœurs**, L* 23,6 contre 41,3 et 41,9, blanc 11,71 sur la rubrique contre 6,15 et 6,02. C’est le caractère du maroquin, non un écart à corriger : sur l’accueil, les cartons de la Bible et des Pères sont tous deux à L* 23,5. ⛔ Ne pas l’éclaircir pour l’aligner sur elles, ce serait le reprendre à l’accueil, d’où il vient. ⚠️ Le pourpre de rubrique qu’il remplace (`#8a2846`) n’était pas faux ; il n’était nulle part ailleurs sur le site.

⚠️ **Les deux exemplaires restent LITTÉRAUX et se renvoient l’un à l’autre** : un carton est une gamme dessinée, un jeton une teinte de rôle, et la charte les tient séparés depuis que des jetons d’encre ont fait s’inverser les cartons en Cuir. ⛔ Reteinter le carton sans reteinter les jetons ferait dire deux choses au même corpus.
...
⛔ **En Cuir, les ENCRES ne peuvent pas se rabattre sur `--cs-vert`**, qui y vaut `#dcc08a` : les trois familles y seraient identiques. Les APLATS, eux, redeviennent du **cuir profond** comme la barre de navigation, et pour la même raison — trois bandes claires répétées sur toute la hauteur d’une page en seraient la seule tache de couleur. Chacun garde le penchant de sa famille sans en avoir la clarté ; blanc mesuré dessus : 11,97 · 12,71 · 12,79. C’est la dérogation que la charte accorde déjà aux catégories encodées par la couleur. ⚠️ **L’aplat des Pères EST le carton de l’accueil en Cuir**, sa teinte basse (`#4e2823`) : il tombe de lui-même dans la bande de ses sœurs (L* 21,1 contre 22,9 et 20,9), en portant deux fois leur chroma — ce que fait déjà l’accueil, dont le carton des Pères est plus coloré que celui de la Bible. ⛔ L’ENCRE, elle, ne peut pas être littérale, les deux teintes du carton étant sombres là où il faut une encre claire : c’est le même maroquin monté au rang d’encre (`#dda297`, L* 71,9, teinte 35°, 8,15 sur le sol, entre la sauge 8,87 et l’or 10,05).
...
⛔ **La balise ne passe PAS par le verrou de bêta, et il a fallu le poser** (`proxy.ts`, `LIBRES`). Le proxy renvoyait `POST /api/audience/vue` vers `/chantier` en 307. Tant que le site est fermé, cela ne se voyait pas : seuls des comptes autorisés tournent des pages, et leur balise passait le verrou avec leur session. Deux raisons de l'en sortir quand même. La mesure serait morte **en silence** le jour où le verrou change de forme à l'ouverture, et une balise redirigée ne se plaint à personne, pas même dans le journal. ⚠️ Le risque est borné et il faut le tenir tel : la route n'écrit que des agrégats anonymes, ne rend aucune donnée (204), est limitée en débit par empreinte, et ses lignes sont purgées. Ne rien y ajouter qui rendrait une donnée, ce serait ouvrir une lecture publique sur un site fermé.
...
## ⛔ Trois défauts trouvés en relisant la mesure (2026-08-31)

⛔ **« Visiteurs sur une PÉRIODE » ne peut pas exister sous ce parti, et le chiffre était faux.** Un `count(distinct empreinte)` sur trente jours comptait des **visiteurs-jours** : le sel tournant chaque jour, un lecteur qui revient trente fois produit trente empreintes. Ce n'est pas réparable — reconnaître quelqu'un d'un jour à l'autre est précisément ce que l'anonymat interdit — et la seule issue honnête était de **remplacer le chiffre par une moyenne quotidienne**, qui a un sens et qu'on peut nommer. ⚠️ Le compte PAR JOUR, lui, était juste et n'a pas bougé ; c'est le seul grain où le mot « visiteur » veut dire quelque chose. La même colonne a quitté le classement des pages, où elle portait le même défaut sous un autre nom. **Règle générale : une empreinte qui tourne borne le grain auquel on peut compter des personnes. Tout agrégat plus large que la rotation compte des occasions, pas des gens.**

⛔ **LA RACINE EST LA PAGE BIBLE, et la rubrique la rangeait sous « accueil ».** `app/page.tsx` sert la lecture biblique ; `/` sans paramètre redirige vers `/accueil` **avant tout rendu** (`app/page.tsx:106`), si bien qu'une vue de `/` ne peut venir que d'un chapitre lu. Toute la lecture biblique, c'est-à-dire la principale surface du site, était donc attribuée à une rubrique où personne ne serait allé la chercher. ⚠️ Le piège est que les deux lectures du chemin sont plausibles : il faut ouvrir `app/page.tsx` pour trancher, et le nom du fichier ne dit rien.

⛔ **Une COORDONNÉE de page n'est pas une saisie de visiteur.** Le retrait de la chaîne de requête, juste pour `/recherche?q=…`, fondait tous les chapitres bibliques en une seule ligne `/`. `PARAMETRES_CONSERVES` (`app/lib/audience.ts`) garde `livre` et `chapitre` sur la seule racine. ⚠️ Ce qui entre dans cette liste doit désigner une PAGE comme le fait un identifiant d'œuvre dans un chemin, jamais quelque chose que le visiteur a tapé. ⛔ Et surtout pas `trad`, `mode` ni `bilingue` : la charte tient traduction, graphie et lecture en regard pour des habits d'un même chapitre, et l'adresse canonique de la page Bible est `/?livre=X&chapitre=N`. Les compter à part compterait neuf fois la même page. ⚠️ **L'ORDRE de la liste blanche fait l'ordre du chemin écrit**, sinon `?chapitre=1&livre=GEN` et `?livre=GEN&chapitre=1` feraient deux lignes pour une page.

⛔ **`usePathname` ne suffit PAS sur ce site, et le défaut est silencieux.** On passe d'un chapitre à l'autre en ne changeant que la chaîne de requête : sans `useSearchParams`, l'effet de la balise ne se rejouait pas et toute une visite de la Bible ne comptait qu'une vue. ⚠️ Le crochet oblige à une frontière `Suspense` dans le gabarit, faute de quoi **toutes les pages prérendues du site basculent en rendu client**. Elle n'enveloppe qu'un composant qui rend `null`, donc elle ne peut pas laisser de charge hors flux comme celle du 2026-08-24. **Le contrôle n'est pas facultatif** : comparer `.next/prerender-manifest.json` avant et après (28 routes au 2026-08-31, inchangées).

⛔ **La page NOMME ce qu'elle montre, et c'est la jointure qui justifiait la mesure maison.** `audience_tableau_bord` résout en base ce que seule la base sait — œuvre avec son auteur, auteur, péricope, publication — et `app/lib/audienceLibelles.ts` nomme côté page ce que seul le code sait, les routes et la lecture biblique. ⚠️ Les jointures sont gardées par la FAMILLE tirée du chemin : sans elle, une clé d'œuvre irait se chercher parmi les auteurs. ⛔ Et la comparaison d'un identifiant numérique se fait **en texte** (`es.id::text = p.cle`) : convertir une clé qui n'en est pas un (`/essais/nouveau`) ferait échouer la requête entière.

⛔ **Les deux modules d'audience sont SÉPARÉS, et pas par goût du rangement.** `audience.ts` porte ce qui se décide à la COLLECTE et part dans le paquet de chaque visiteur ; `audienceLibelles.ts` porte ce qui se décide à la LECTURE et tire `LIVRES` derrière lui. Les réunir emporterait le canon entier dans la balise.

**Un silence de trois jours se SIGNALE, sans affirmer la panne.** Une collecte interrompue et une absence de visiteurs rendent exactement les mêmes zéros ; `derniere_vue` est le seul champ qui les sépare. ⚠️ Le bandeau dit que les deux se ressemblent et qu'il vaut d'aller voir : crier à la panne sur un site qui n'a simplement reçu personne ferait cesser de le lire, ce qui est le sort de toute alerte qui se trompe.

⚠️ **Ce qui reste ouvert, et qui a été relevé sans être traité** : `part_connectee` est calculée et n'est affichée nulle part ; la route d'écriture est publique et son plafond est haut (200 lignes par empreinte et par tranche de dix minutes) ; une page introuvable est comptée comme une vue ; et rien ne distingue une page OUVERTE d'une page LUE, ce qui est pourtant le seul signal qui compte sur une bibliothèque.

## ⛔ La référence d'édition paraît ENTIÈRE ou pas du tout, et la carte ne porte qu'un texte long (2026-09-02)

Décision de l'auteur, sous la fiche Fillion : « Ne pas tronquer le premier texte ; n'afficher, d'ailleurs, que le premier texte ; limiter le nombre de caractères affichés (ou même si ce paragraphe s'affiche) en fonction de la taille de l'écran (déjà en place, mais demande à être optimisé). » Le premier texte est la référence d'édition (`traductions.source_edition`), le second la notice du traducteur (`bio_courte`).

⛔ **Plus de `line-clamp` sur la référence.** Le 31 août son nombre de lignes MONTAIT avec le volet (2, 3, 4), ce qui laissait les plus longues coupées à toute largeur — Fillion compte **348 signes**, la plus courte 93. Désormais la feuille accorde un BUDGET de lignes (`--volet-ref-lignes` : 5 dès 260 px de volet, 7 à 300, 8 à 350), et `EncartTraduction` compose la référence dans une SONDE invisible de la largeur du volet, compte ses lignes, et ne rend le texte visible que s'il tient dans le budget. ⚠️ La mesure est en JavaScript parce qu'une requête de conteneur ne sait pas compter les signes d'un texte ; la politique (budget, seuils) reste dans `globals.css`, avec les autres seuils du volet, et ajouter une bible n'oblige à rien. ⚠️ La sonde est observée par `ResizeObserver` : elle change de hauteur quand le volet change de largeur, quand la police finit de se charger, et quand la feuille cesse de la cacher sous 260 px — trois cas, une seule mesure. ⛔ `false` au départ, donc au rendu serveur : la référence PARAÎT après la première mesure (sous `useLayoutEffect`, avant la peinture) ; partir de `true` l'aurait montrée puis retirée, et la liste des livres aurait sauté.

⛔ **La notice du traducteur a quitté la carte** (« n'afficher que le premier texte »). Entrée le 31 août au-dessus de 260 px, elle vit dans la fiche « En savoir plus », d'où elle venait ; `.cs-volet-bio`, `--volet-bio-lignes` et le champ `bio` de `TraductionEncart` sont partis avec elle. Le plafond de 8 lignes (109 px à 10,5 px d'œil) tient à la liste des livres, qui vit dessous.

*La leçon rejoint celle du 28 août : une référence tronquée n'est pas une référence courte, c'est une phrase cassée ; quand un texte ne tient pas, on le retire, on ne le coupe pas.* Charte : § 18, « Un texte long paraît ENTIER ou pas du tout » (`scripts/charte-reference-entiere-volet-2026-09-02.mjs`).

# Sécurité des données des utilisateurs (audit et correctifs du 2026-09-02)

Audit mené sur la base (politiques, droits, vues, fonctions) et sur les routes qui touchent aux comptes. Migration `supabase/migrations/20260902200000_securite_donnees_utilisateurs.sql`, appliquée le jour même ; module `app/lib/empreinteAnonyme.ts` (testé).

⛔ **N'IMPORTE QUEL COMPTE POUVAIT SE FAIRE ADMINISTRATEUR, et c'est éprouvé.** La politique UPDATE de `profils` ne bornait que la LIGNE (« la sienne »), jamais les colonnes ; `authenticated` a UPDATE sur la table entière ; et `estAdmin()` (`verifAdmin.ts`, qui garde toutes les routes `/api/admin/*` et `/admin`) comme le verrou de bêta (`proxy.ts`) lisent `profils.est_admin` et `profils.acces_beta`. En transaction annulée, depuis la place d'un lecteur ordinaire : `update profils set est_admin = true, acces_beta = true, pseudo = 'admin'` passait. ⚠️ `is_admin()` en SQL lit `admin_users`, non `profils` : les politiques RLS ne cédaient pas, les routes du site si. Le déclencheur `trg_profils_garde_colonnes` FIGE désormais `est_admin`, `acces_beta` et `points` pour tout écrivain qui n'est pas la clé de service (INSERT : à `false`/0 ; UPDATE : valeur d'avant, sans erreur, le reste de la ligne passe), et il impose au pseudo, quand il CHANGE, le format et la liste réservée de `creer-profil`, que `RubriqueCompte` contournait en écrivant `profils` directement. ⚠️ Deux pseudos existants sont hors format (« Sœur », « Compte test ») : ils restent, la garde ne juge qu'un changement. Éprouvé après migration : `est_admin`, `acces_beta`, `points` inchangés, `bio` modifiée.

⛔ **Un commentaire « supprimé » gardait son texte lisible par l'API.** L'écran remplace le texte par « a supprimé un commentaire », mais la ligne restait validée et `texte` intact : n'importe qui le relisait par PostgREST. `trg_effacer_texte_supprime` (sur `commentaires` et `essais_commentaires`) vide `texte` dès que `supprime` passe à vrai. La ligne reste, le fil des réponses tient.

- **`essais_commentaires`** se lisait avec `using (true)` : les commentaires EN ATTENTE de tout le monde étaient servis. Aligné sur `commentaires` : validé, ou le sien, ou administrateur.
- **`signalements`** : l'insertion acceptait n'importe quel `user_id` (with_check `true`, `anon` compris) ; désormais le sien ou nul, `authenticated` seul (la route passe par la clé de service et ne change pas).
- **Le réglage « Rang » du compte (`pub_rang`) n'était honoré que par `/api/profil/[pseudo]`** : les vues `lecture_utilisateurs` et `classement_utilisateurs`, en DEFINER et interrogées depuis le navigateur pour des tiers (volet patristique, commentaires d'œuvre et d'essai), l'ignoraient. Les deux vues portent maintenant `where coalesce(pub_rang, true) or id = auth.uid()` : un lecteur voit toujours sa propre ligne (page Parcours), les autres ne voient que ceux qui l'ont permis. ⚠️ `auth.uid()` se lit dans une vue DEFINER : c'est un réglage de session, non un droit. Sous la clé de service il vaut `null`, et la route du profil vérifie `pub_rang` avant d'interroger. Les droits INSERT/UPDATE/DELETE que `lecture_utilisateurs` portait encore sont révoqués.
- ⛔ **La page Confidentialité disait « votre adresse IP n'est enregistrée à aucun moment », et `messages_contact` l'écrivait en clair** (formulaire de contact, propositions d'œuvre). La colonne `ip` est supprimée ; `empreinte` la remplace, calculée par `empreinteAnonyme` (sel secret + jour + adresse + agent), la même règle que la mesure d'audience, dont la fonction a rejoint le module partagé. La limitation de débit se fait sur l'empreinte. `messages_contact` et `inscriptions_attente` perdent aussi tout GRANT pour `anon`/`authenticated` : la RLS sans politique fermait déjà, mais un verrou qu'on ne voit pas n'en est pas un.
- **Suppression du compte** : la base efface par `on delete cascade` profil, favoris, prélèvements, publications, appréciations, messages, progression, notes ; `commentaires`, `essais_commentaires` et `signalements` sont seulement DÉTACHÉS (`set null`) et gardaient le pseudonyme dans `auteur_nom`. La route l'efface (« Compte supprimé ») avant `deleteUser`, et refuse de continuer si cette écriture échoue.
- **Messagerie** : trente envois par dix minutes et par compte (`checkRateLimit`), il n'y avait aucune borne.

**Relevé sans être traité, à décider :**
- `commentaires_likes`, `essais_appreciations` et `catalogue_votes` se lisent en `true`, `user_id` compris : qui a aimé quoi est public. Le site lit ces tables depuis le navigateur pour compter ET pour trouver son propre vote ; ne montrer que des agrégats demanderait une vue ou une RPC et de toucher quatre composants.
- `messages_contact` n'a aucune durée de conservation (courriel et message gardés sans borne) ; `vues_pages` a la sienne.
- `GET /api/messagerie/[pseudo]` marque les messages comme lus au passage : un préchargement consomme l'accusé. Déplacer dans un POST demande de toucher le client.
- Aucun blocage ni sourdine entre lecteurs dans la messagerie.
- `notificationsClient.ts` lit `profils` pour des tiers avec la session du lecteur, dont la politique ne rend que sa propre ligne : les pseudos des autres n'arrivent jamais (défaut de FONCTION, pas de sécurité).
- Les politiques RLS des tables d'utilisateurs ne sont versionnées nulle part : elles vivent en base seulement. Un script « depuis la place d'un autre lecteur » (modèle `audit-lecture-anonyme.mjs`) reste à écrire.

## L'épreuve des droits d'un lecteur, et le miroir des politiques (2026-09-03)

⛔ **Une politique RLS ne se relit pas, elle s'ÉPROUVE.** `node --env-file=.env.local scripts/audit-droits-lecteur.mjs` se met à la place d'un compte ordinaire (deux comptes non administrateurs pris en base, « moi » et « autre ») et tente vingt-neuf écritures et lectures interdites, dans UN bloc `do` qui se termine par une exception : la transaction est annulée, rien n'est écrit, et le rapport revient dans le message de l'exception. Sortie 1 dès qu'une épreuve est en défaut. À rejouer après toute migration qui touche une table d'utilisateurs, et avant de croire une politique.

- ⚠️ **`exec_sql` ne peut pas servir** : « cannot set parameter "role" within security-definer function ». D'où `eprouver_sql`, sa jumelle en INVOKER (migration `20260903091000`), réservée à `service_role`, et **`grant authenticated to service_role`**, sans quoi le changement de rôle est refusé. La clé de service n'y gagne aucun droit qu'elle n'ait déjà.
- ⚠️ **Une table RLS sans politique ne REFUSE pas, elle rend zéro ligne.** C'est la première épreuve qui l'a montré : `admin_users` répondait « accepté » à une lecture, vide mais acceptée, parce qu'elle gardait tous ses GRANT. Un verrou qu'on ne voit pas n'en est pas un ; ses droits sont révoqués, comme ceux de `messages_contact` et `inscriptions_attente` la veille. Une épreuve « refusé » attend donc un `42501`, et une table qui doit être fermée se ferme par le GRANT, la RLS n'étant que le second verrou.
- ⚠️ **Une épreuve se relit avant d'être crue** : « se valider soi-même » a d'abord rendu ÉCRIT parce que le commentaire témoin était inséré déjà validé, et le déclencheur, qui ramène `valide` à sa valeur d'AVANT, le laissait donc vrai. Le témoin s'insère en attente.
...
# La Bible du XIIIe siècle a sa TRADUCTION MODERNE (TR0013), lue par le canon (2026-09-03)

GPT traduit le témoin BnF fr. 899 en français moderne : **TR0013**, « Bible française du XIIIe siècle — traduction critique moderne ». Relevé du 2026-09-03 : **14 livres, 9 972 versets** dans `versets_v2` (Genèse à 2 Rois, Ruth, Tobie, Judith, Esther), chacun avec son `canon_id` ; `est_privee = true` (administrateur seul, par la RLS de `versets_v2`) ; famille éditoriale `bible899-critical-modern-v1` avec TR0009 (témoin, `fro`, à gauche) et TR0013 (traduction, `fr`, à droite) ; 4 138 notes de verset, toutes en brouillon et non publiques ; aucun bloc de corps. ⛔ Les 15 couches `lecture-fidele` de `bible_source_unit_texts` n'ont RIEN à voir avec elle : ce sont les relectures de Fillion (TR0010/TR0011).

**Le chemin de lecture, écrit ce jour.** Ni la vue large `versets_lecture` (colonnes TR0001–TR0005) ni la cascade éditoriale (segments, unités, offsets) ne pouvaient la servir : son texte est déjà un verset par créneau. D'où un TROISIÈME chemin, `chargerVersetsCanoniquesV2` (`bibleEditorialServer.ts`), qui lit `versets_v2` par `canon_id` et rend la forme de `chargerVersetsEditoriaux`, si bien que `TexteBible`, la lecture en regard et le volet des livres n'ont rien appris de nouveau. Capacité `verse` de source `versets-v2` (`withCanonicalV2Capability`, `estVerseCanoniqueV2`).

- ⛔ **La découverte se fait dans la PAGE, sous la RLS du lecteur, jamais dans le catalogue mis en cache.** Le catalogue des capacités est partagé soixante secondes entre tous les lecteurs ; y semer une traduction privée la promettrait à tous, et le chapitre viendrait vide. `app/page.tsx` prend les membres de famille que le catalogue ne sait pas lire, demande à `livres_par_traduction` (vue en INVOKER, donc filtrée) lesquels portent du texte, et n'attend la réponse que là où elle sert : tout de suite si c'est cette bible qu'on demande, avec la vague des versets sinon. Coût : une requête, en parallèle.
- ⛔ **L'échange EN MÉMOIRE du menu ne vaut qu'entre COLONNES de la vue large** : `estVerseSurColonnes` remplace la double négation sur `estVerseEditorial` dans `BibleLayout`. Une traduction de `versets_v2` n'est pas plus chargée qu'une segmentation éditoriale.
- **En regard**, `chargerLectureBilingue` reçoit `membresCanoniquesV2` et charge ces membres par le nouveau chemin ; l'axe canonique reste commun. Le menu « Lecture » dit « Ancien français-français » : `fro` est entré dans `NOM_LANGUE`.
- **La numérotation native** (`num_TR0013`) ne se dit que lorsqu'elle DIFFÈRE du canon : sur une traduction alignée au même schéma, elle redirait le numéro.
- ⚠️ **Ce qui reste à GPT** : lever `est_privee` le jour de la publication (rien à changer ici, la RLS et `livres_par_traduction` suivent) ; publier les notes de verset (`is_public`, `validation_status`) ; des `notes` libres existent sur 32 versets et ne sont pas rendues. ⚠️ 34 versets sans `canon_id` (rubriques hors canon) ne paraissent pas dans la lecture par le canon : c'est le même parti que pour les MANUSCRIPT_EXTRA du témoin.
- ⛔ **La famille est en BROUILLON, et ses quatre tables n'ouvraient la lecture qu'au statut `published`, administrateur compris** : `v_bible_edition_catalog` rendait zéro ligne à l'auteur, la page ne trouvait aucun membre et retombait sur Sacy. Migration `20260903150000_bible_edition_admin_read_private` : une politique `is_admin()` sur `bible_edition_families`, `_members`, `_member_sources` et `_components`, sur le modèle de `bible_text_sources_admin_read_private`. Le public ne voit toujours que `published`. Vérifié en ligne sous la session de l'auteur : Genèse 3 en TR0013 seule (24 versets), puis en regard « Ancien français-français » (24 créneaux, témoin à gauche, traduction à droite).
- **« Avec / Sans les commentaires » ne paraît que si l'appareil EXISTE** (demande de l'auteur, 2026-09-03). `paratexteDisponible` ne se juge plus sur la seule existence de la famille : deux comptes en tête (`bible_editorial_body_blocks`, `bible_verse_notes`) sous la RLS du lecteur, qui ne rend que le publié, dans la vague des versets. La règle « on juge la FAMILLE, jamais le chapitre » tient toujours ; elle dit maintenant : la famille a-t-elle un appareil publié, où que ce soit. La famille 899, dont les 4 138 notes sont en brouillon, n'offre donc pas le choix ; Fillion (7 221 blocs publics) le garde.

# La MARQUE DE MÉCÈNE — un objet À CÔTÉ des hauts faits (2026-09-03)

Doctrine : charte `parametres.charte_ia`, **§ 40.7**. Règles de code :

- ⛔ **Elle n'entre dans AUCUNE table de hauts faits.** `hauts_faits`, `hauts_faits_obtenus`, `MESURES` et `FAMILLES` (`app/lib/hautsFaits.ts`) ne la connaissent pas et ne doivent jamais la connaître : elle ne vaut aucun point, n'a ni série ni degré, ne compte pas dans le rang et n'ouvre ni droit, ni accès, ni fonction. Le jour où l'on serait tenté de l'y ranger « pour tout mettre au même endroit », relire le § 40.4 : une case qui s'achète discrédite les vingt et une autres.
- **Deux objets, et la marque se DÉDUIT du registre.** `dons` est le registre (qui, quand, la référence de transaction, unique) ; `profils.mecene_depuis` est l'état, recalculé côté serveur sur la date du plus ancien don rattaché (`recalculerMarque`, `app/api/admin/dons/route.ts`). ⛔ Ne jamais poser `mecene_depuis` à la main : deux surfaces qui décrivent le même fait divergent au premier réglage.
- ⛔ **AUCUN MONTANT n'est reçu ni gardé**, ni dans la table, ni dans la route, ni à l'écran. PayPal tient ce livre ; un montant en base serait une donnée financière à protéger pour rien, et c'est aussi ce qui rend la marque indivisible.
- ⛔ **La colonne est ÉPINGLÉE dans `profils_garde_colonnes`.** La politique RLS de `profils` borne la LIGNE qu'un lecteur modifie, jamais la VALEUR qu'il y écrit : sans cette ligne, n'importe quel compte se décerne la marque par un `update` sur sa propre ligne, comme il se faisait administrateur avant le 2026-09-02. `pub_mecene`, lui, reste libre — le lecteur doit pouvoir se taire.
- ⛔ **`dons` n'a AUCUNE politique RLS**, et ses droits ne sont accordés à personne : elle ne se lit et ne s'écrit qu'avec la clé de service, depuis `/api/admin/dons`, qui vérifie l'admin. Un registre nominatif de dons n'a rien à faire dans un navigateur, fût-ce celui de l'administrateur.
- **La vue `mecenes_publics` est la SEULE porte côté navigateur.** Elle ne rend que des identifiants (`user_id`), et elle filtre déjà sur `pub_mecene` : le lecteur qui retire sa marque sort de la SOURCE, il ne compte pas sur chaque écran pour se taire. Les trois surfaces de commentaires s'y adressent de la même manière (`.in('user_id', ids)`), en parallèle de leurs requêtes existantes. ⚠️ Une lecture SERVEUR (page d'essai, liste des essais, messagerie, profil) passe par la clé de service et n'a donc pas ce filtre derrière elle : elle doit tester `pub_mecene` elle-même.
- **Le compte connecté porte `estMecene`** dans `contexteCompte`, deux colonnes de plus sur une requête qui part déjà : c'est ce qui fait qu'un commentaire qu'on vient de poser porte la marque sans attendre un rechargement.
- ⚠️ **La marque prend l'encre de sa surface**, par la propriété `couleur` de `MarqueMecene` : `var(--cs-or)` par défaut, `var(--cs-or-doux)` sur l'en-tête vert sombre d'un profil, `currentColor` sur une couverture de publication, dont la charte dit qu'elle n'a qu'une seule encre. ⛔ Ne pas poser l'or de la charte sur un fond sombre : il est mesuré pour un fond clair.
- ⚠️ **Le `<title>` du SVG fait l'infobulle ET le nom accessible.** Un `title` sur l'enveloppe ferait annoncer la marque deux fois.
- ⚠️ **L'interrupteur `pub_mecene` ne paraît QU'AUX MÉCÈNES** (`RubriqueCompte`). Montré à tous, il annoncerait une distinction que le lecteur n'a pas et ferait de la page du compte un catalogue de ce qui lui manque.
- **`admin_chercher_comptes(terme)` est SECURITY DEFINER et RÉSERVÉE à la clé de service.** L'adresse de connexion vit dans `auth.users`, hors de PostgREST : c'est sa raison d'être, et c'est aussi la raison des trois `revoke`. Sans eux, `authenticated` y lirait l'adresse de connexion de n'importe qui.
- ⚠️ **Le rattachement reste MANUEL, et la notification PayPal n'est pas écrite.** La colonne `reference` et son unicité l'attendent. Le jour où on l'écrira : vérifier la signature (postback IPN, ou signature du webhook REST), puis rattacher par `reference` d'abord, par adresse ensuite, et laisser au registre les dons qu'aucun compte ne réclame.

## ⛔ Le don qui S'INSCRIT SEUL — la notification PayPal (2026-09-03)

Doctrine : charte **§ 40.7**, sous « LE DON S'INSCRIT SEUL ». Route `app/api/paypal/webhook/route.ts`, lecture des charges dans `app/lib/paypalWebhook.ts` (pur, 21 tests), règles communes aux deux portes du registre dans `app/lib/mecenesServeur.ts`.

- ⛔ **La vérification passe par l'API de PayPal, jamais par le certificat qu'annonce la requête.** `paypal-cert-url` vient de qui envoie : aller le chercher soi-même, c'est faire confiance à l'adresse que fournit celui qu'on contrôle. On appelle `/v1/notifications/verify-webhook-signature` avec NOS clés, et la question disparaît. ⚠️ Trois variables d'environnement : `PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, `PAYPAL_WEBHOOK_ID` (plus `PAYPAL_ENV=sandbox` pour éprouver le montage). Sans les trois, `paypalConfigure()` est faux et la route ne fait RIEN — elle répond 200 pour que PayPal ne réessaie pas un montage qui n'existe pas.
- ⚠️ **Les codes de réponse ne sont pas décoratifs, PayPal RÉESSAIE.** 401 sur une signature refusée (un refus n'a pas à être rejoué), 503 quand PayPal est injoignable (une panne temporaire mérite un rejeu), 200 partout ailleurs — y compris sur un doublon (23505), qui n'est pas une anomalie mais le fonctionnement même du rejeu.
- ⛔ **L'IDENTITÉ DU DONATEUR N'EST PAS DANS L'ENCAISSEMENT.** `PAYMENT.CAPTURE.COMPLETED` porte la référence et le bénéficiaire, presque jamais le payeur ; c'est la COMMANDE (`supplementary_data.related_ids.order_id`, lue par `GET /v2/checkout/orders/{id}`) qui le porte. C'est le chemin ordinaire, non un repli. ⚠️ `PAYMENT.SALE.COMPLETED`, la forme ancienne, n'a ni commande ni payeur : son don s'inscrit sans compte.
- ⛔ **`payee` n'est lu NULLE PART**, et son absence de `CHEMINS_EMAIL` est la règle : c'est l'adresse du site, celle qui reçoit. La confondre avec celle du donateur rattacherait tous les dons au compte de l'auteur. Un test le tient.
- ⛔ **`compte_par_courriel` DÉCIDE, quand `admin_chercher_comptes` PROPOSE**, et c'est pourquoi ce sont deux fonctions. La première ne rend un compte que s'il est le SEUL à porter l'adresse, à l'égalité exacte : deux lecteurs peuvent avoir déclaré la même adresse de contact, et sans personne derrière la marque irait au mauvais. Les deux sont SECURITY DEFINER (l'adresse de connexion vit dans `auth.users`) et réservées à la clé de service.
- ⛔ **Le registre ne perd jamais un don** : le rattachement n'est pas un préalable à l'inscription. Un don sans donateur lisible s'inscrit avec sa seule référence et attend une main. ⚠️ Et la marque ne se recalcule que sur une inscription NEUVE : sur un rejeu, la date n'a pas bougé.
- ⚠️ **La route est HORS du verrou de bêta** (`proxy.ts`, `LIBRES`, entrée `/api/paypal`). Redirigée en 307 vers /chantier, elle mourrait en silence et PayPal renoncerait au bout de ses tentatives. Même raison que pour la balise d'audience, et même risque borné : elle ne rend aucune donnée et n'écrit rien avant vérification.
- **Le silence se voit** : `parametres['paypal_webhook_dernier']` date la dernière notification vérifiée, et l'onglet « Mécènes » l'affiche. ⚠️ Elle s'écrit pour TOUTE notification vérifiée, même celles qu'on n'inscrit pas : ce qu'on veut savoir est que la liaison vit, et un remboursement le prouve aussi bien qu'un don.
- ⛔ **L'ancienneté est comptée par le SERVEUR** (`lireEtatReception`), jamais au rendu : `react-hooks/purity` refuse une horloge lue pendant un rendu, et il a raison — un composant qui lit l'heure n'est pas idempotent.
- ⚠️ **`recalculerMarque` a quitté la route d'administration pour `mecenesServeur.ts`** : les deux portes du registre doivent poser la marque de la même façon, sinon un don rattaché à la main et un don reçu automatiquement ne donneraient pas le même résultat, et il faudrait deviner lequel a raison.
- ⛔ **Le lien de don n'est PAS touché.** Y glisser un jeton fermerait le troisième cas, mais le bouton est hébergé chez PayPal et rien ne dit qu'il laisse passer une variable ajoutée : un bouton de don cassé coûte infiniment plus. `marqueurDuDon` LIT le marqueur s'il vient et le range dans la note du registre ; ⛔ il ne le tient pas pour une preuve d'identité, un marqueur non signé se forgeant dans l'adresse du lien.
- ⚠️ **Ce que je n'ai pas pu éprouver** : aucune notification réelle n'est encore venue, PayPal n'étant pas configuré au moment du dépôt. Les charges des tests sont recopiées des formes documentées, non observées. Le premier don réel dira si le donateur se lit bien sur la commande.

### ⛔ La voie est l'IPN, et non le webhook (2026-09-03, même jour)

Le webhook exige un compte PayPal **Business** : le tableau de bord développeur refuse l'onglet « Live » à un compte personnel (« Upgrade your account to PayPal for Business to view live credentials ») et n'y sert que le bac à sable. Décision de l'auteur le jour même : « je ne passerai pas en compte Business ». C'est donc `app/api/paypal/ipn/route.ts` qui sert, et `app/lib/paypalIpn.ts` (pur, 19 tests) qui lit les messages.

- ⚠️ **Et c'est la meilleure des deux ici** : aucune clé d'API, rien de secret, et le message porte DIRECTEMENT `payer_email`, `first_name` et `last_name` — là où l'encaissement d'un webhook oblige à aller chercher la commande. ⛔ La route du webhook reste écrite et dort (`paypalConfigure()` faux) : le jour où le compte changerait de nature, il n'y aurait rien à réécrire. `voieDeReception()` arbitre, l'IPN d'abord.
- ⛔ **`PAYPAL_RECEVEUR` est le SEUL réglage, et il n'est pas facultatif.** La validation de PayPal dit qu'un message est authentique, jamais qu'il m'était destiné : un message authentique adressé à un autre marchand se valide tout aussi bien, et sans ce contrôle n'importe qui se donne un euro à lui-même et réexpédie le message pour prendre la marque. `messagePourMoi` compare `receiver_email` ET `business` (adresse secondaire). Sans la variable, `ipnConfigure()` est faux et la route répond **503** — PayPal rejoue pendant plusieurs jours, si bien que les dons de l'intervalle finissent par arriver au lieu d'être perdus.
- ⛔ **Le corps BRUT est réexpédié tel quel** (`corpsDeValidation`), jamais reconstruit depuis les paramètres décodés : PayPal compare caractère pour caractère, et un paramètre réordonné ou réencodé rend `INVALID`. C'est l'erreur classique de ce mécanisme, et elle ne se voit qu'à l'usage. Corollaire : `request.text()`, jamais `request.formData()`.
- ⛔ **`ipnpb.paypal.com`, et non `www.paypal.com`** : la validation par l'adresse ordinaire est abandonnée et rend des refus qu'on prend pour des messages faux. ⚠️ Et un `User-Agent` nommé est OBLIGATOIRE : sans lui PayPal refuse la validation, ce qui ressemble à une attaque là où il n'y a qu'un en-tête absent.
- ⛔ **PayPal attend un 200 NU.** Un corps, une redirection ou un délai le font conclure à un échec et rejouer ; répété, il coupe la notification. Les autres codes sont choisis pour le rejeu : 503 tant que la configuration manque, 500 sur un échec d'écriture (le don sera rejoué, donc pas perdu), 401 sur un message refusé (rien à rejouer).
- ⚠️ **`payment_date` s'écrit « 15:04:05 Sep 03, 2026 PDT »**, que `new Date()` ne lit pas de façon fiable — l'abréviation de fuseau lui échappe et le résultat varie selon le moteur. Le jour se lit à la main (`dateDuMessage`) ; l'heure n'intéresse pas le registre.
- ⚠️ **Un type de transaction ABSENT ne fait pas écarter** (`estTypeRetenu`) : le message est déjà vérifié, et un don qu'on n'inscrirait pas serait un don perdu. On n'écarte que ce qu'on reconnaît comme n'étant pas un paiement (`subscr_signup`).
- ⚠️ **PayPal donne l'IPN pour ancien et le retirera un jour**, sans un mot. C'est pourquoi l'onglet « Mécènes » date la dernière notification reçue. Ce n'est pas une raison de s'en priver ; c'en est une de savoir qu'on le surveille.

# Le module « Styles » de l'administration (2026-09-03)

Demande de l'auteur : « un petit module admin pour les styles, pour les identifier ou les changer ; il faut donc un nom de style propre, pas le nom technique, pour chaque style ; à partir de ce module, je veux pouvoir changer le style d'un segment, d'un verset, d'un titre, etc. » Onglet `/admin?onglet=styles` (`app/admin/SectionStyles.tsx`), route `app/api/admin/styles`, règles pures dans `app/lib/stylesLibelles.ts` (les noms) et `app/lib/stylesAttribution.ts` (ce qu'on écrit), toutes deux testées.

- ⛔ **UN NOM PROPRE PAR STYLE, DANS UN SEUL REGISTRE.** `stylesLibelles.ts` nomme chaque nature de `NATURE_VALIDES`, chaque style canonique du registre biblique et chaque rang des deux échelles ; `stylesLibelles.test.ts` refuse un nom orphelin comme un style sans nom. Le menu « Nature » du contrôle des œuvres lit désormais cette table (il portait la sienne, `naturesSegments.test.ts` la relisait dans le source du composant) : deux listes de noms divergeraient au premier ajout.
- ⛔ **On attribue, on ne crée pas** (charte § 7.6). Les menus n'offrent que le vocabulaire clos ; `separateur` (hérité) et `note_verset` (qui n'est pas un bloc du corps) ne s'attribuent pas. La base garde le dernier mot par ses verrous — `chk_segments_nature`, `trg_bible_style_semantique_connu` — et leur message est rendu tel quel sur la ligne.
- ⛔ **Le style d'un bloc biblique vit DANS un `jsonb`** (`metadata.semantic_style`, `semantic_level`) : l'écriture passe par la route, avec la clé de service, et `metadonneesAvecStyleBible` FUSIONNE au lieu de remplacer. Un titre reçoit le rang fixe du registre (T1 à T6) ; une nature d'information exige un rang déclaré (I1 à I6) ; `metadata.style_attribution` garde qui, quand, et ce que le bloc portait (charte § 14.2).
- ⛔ **Une famille éditoriale en brouillon ne se touche pas d'ici** : la route refuse tout bloc d'une famille dont le statut n'est pas `published`. C'est le chantier de la Bible 899, que GPT tient.
...
- ⚠️ **Les divisions de l'éditeur viennent de `get_niv1_list_global`**, RPC réservée au `service_role` : `get_niv1_list` appartient au lecteur et ne projette que le corps. Les segments sans niveau 1 se listent à part : une œuvre sans divisions en a quand même des segments.
