...
- ⛔ **La page des résultats appelle TROIS RPC, et trois seulement** : `recherche_versets_v2(p_termes text[], p_mode, p_scope)`, `recherche_segments_v2(p_termes, p_mode)`, `recherche_segments_original_v2(p_termes, p_exact)`, plus `lexemes_recherche(p_termes)` en mode famille. Un mot ou plusieurs passent par la même voie ; ⛔ plus de `ilike` du navigateur sur `segment_texte` ou `versets_lecture`, plus de boucle de graphies u/v i/j côté client, plus de liste `in('id_texte', …)` des textes par défaut. Modes : `prefixe` | `exact` | `famille`.
...
- Ce qui reste : les essais se cherchent en `ilike` brut (25 textes) ; le grec n'est pas désaccentué ; plafonds 6 000 / 5 000 ; `recherche_segments` et `recherche_versets(p_terme, p_scope)` d'avant restent en place, sans appelant dans le site.
