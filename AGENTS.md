...
- **Corps : `variante` de l'appel** — `corps` (prose, chapeaux, titres de niveaux 3-4 : brun `#8a6a3e` + pointillé, `0.60em`), `titre` (niveaux 1-2 : `currentColor` à 55 %, sans pointillé, `0.42em`), `frontispice` (page de titre : `0.30em`). Dans tous les cas l'appel **hérite `font-family` et `font-style`** : dans un chapeau en italique, il s'incline avec lui. Ne pas revenir à `fontStyle: 'normal'` ni à une police serif forcée.
