...
- ⛔ **La marge a son propre flux — règle des MANCHETTES** (`app/admin/styles/manchettes.ts`, 7 tests). Posées toutes à `top: 0` de leur unité, les légendes se recouvraient dès que l'unité était plus courte qu'elles : un paragraphe fait 30 px, sa notice 150. Chacune se cale donc au plus bas de deux repères — le haut de son unité, ou le bas de la précédente plus 14 px —, et ⛔ **ne remonte jamais au-dessus de son unité** : la marge peut prendre du retard sur le texte, jamais le devancer. Le composant MESURE, il ne décide pas. Sous 1 200 px la marge passe sous l'épreuve, en liste.
