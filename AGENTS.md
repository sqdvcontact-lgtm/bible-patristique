...
- **Rendu.** Frise auteur : `app/components/ModaleAuteur.tsx` (`FriseAuteur`). Frise générale : `app/histoire/page.tsx`. Classement des brins par `nature_lien` : `bibliographique`→Œuvres, `direct`→Vie, sinon Contexte (`portee` en repli). Dates en années brèves ; siècles via `app/lib/siecles.tsx` (jamais composés à la main). Ne jamais créer d'auteur ni d'association silencieuse depuis les données (charte §26.2–26.3).
